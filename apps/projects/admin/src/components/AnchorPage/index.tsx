...
const AnchorPage = (props: IProps) => {
  const { title, extra, anchors, customStyle, onBack, children } = props
          ...
          <div className={styles['anchor-page-header-main']}>
            <div className={styles['anchor-page-header-left']}>
              <div className={styles['anchor-page-header-heading']}>
                <ArrowLeftOutlined className={styles['anchor-page-header-back']} onClick={handleBack} />
              ...
              <div className={styles['anchor-page-header-content']}>
                <div className={styles['anchor-page-header-anchors']}>
                  {anchors.map((item, index) => (
                    <Anchor.Link
                      className={current && index === 0 ? 'ant-anchor-link-active' : ''}
                      key={item.key}
                      href={`#${item.key}`}
                      title={item.name}
                    />
