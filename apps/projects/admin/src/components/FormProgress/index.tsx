...
export type listFiledConfigItemType = {
  parentFiled: string // 父级字段名
  valueFiled?: string[] // 当前需要计算的值字段
  children?: listFiledConfigItemType // 子级
}
...
type PropsType = {
  ...
  listFiledConfig?: listFiledConfigItemType[] // 列表字段内部需要计算的字段配置
}
...
const FormProgress = ({ title, disabled, ignoreConfig = [], listFiledConfig = [] }: PropsType, ref: any) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    render(form: FormInstance<any>, otherValues = {}) {
      ...
      // 递归列表字段内部需要计算的字段配置
      const deepListFiledConfig = (filedList, filedConfig, level: number) => {
        filedList[filedConfig.parentFiled]?.forEach((i, idx) => {
          filedConfig.valueFiled?.forEach((v) => {
            values[v + `_${level}_${idx}`] = i[v] // 将需要计算的值拼一个特殊标识存储到 values 中
          })
          if (filedConfig.children) {
            deepListFiledConfig(i, filedConfig.children, level + 1)
          }
        })
      }
      ...
      // 根据配置进行递归扁平化到values中
      listFiledConfig.map((item) => {
        deepListFiledConfig(values, item, 1)
      })
      ...
      ignoreConfig?.forEach((item) => {
        ...
        if ((!item.key && !item.value && item.ignoreKey) || (item.key && values[item.key] === item.value)) {
