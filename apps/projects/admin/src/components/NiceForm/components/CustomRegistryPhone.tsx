...
const registryPhone = (props: any) => {
  const { dataSource = [], selectPh, inputPh } = props.props['x-component-props']
  ...
  return (
    <Row>
      <Col span={8}>
        <Select value={defaultValue.countryCode} onChange={(val) => handleChange('select', val)} placeholder={selectPh}>
          {dataSource.map((v, i) => {
            return (
