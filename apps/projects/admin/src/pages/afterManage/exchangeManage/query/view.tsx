...
import { getAftersalesReplaceGoodsPageByPlatform } from '@apps/apis'
import { EXCHANGE_OUTER_STATUS_TAG_MAP } from '../constants'
  ...
  const defaultColumns: RecordColumns<any>[] = [
    {
      ...
      render: (text, record) => (
        <>
          <EyeAuthButton url={`${pathname}/detail?id=${record.replaceId}`}>{text}</EyeAuthButton>
        </>
      ),
    },
    {
