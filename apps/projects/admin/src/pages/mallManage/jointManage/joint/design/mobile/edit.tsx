    ...
    try {
      ...
      if (appConfig && Object.keys(appConfig).length > 0) {
        ...
        const AVAILABLE_COMPONENTS = [
          ...
          MOBILE_DESIGN_COMPONENT.RecommendShop,
    ...
    >
      <div className={styles.wrapper}>
        ...
        <div className={styles.content}>
          {!isPreview && <MobileClientEditLeft isClient={false} />}
          <div className={styles['app-wrapper']}>
            <div className={styles['app-canvas-container']}>
              <MobileDesignPanel isPreview={isPreview} theme={theme} onlyEidt pageConfig={componentConfigs} />
            </div>
          </div>
          {!isPreview && <MobileSettingPanel shopId={shopId} property={1} environment={Number(environment)} />}
        </div>
      </div>
    </BrickProvider>
  ) : (
