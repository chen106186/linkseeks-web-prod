...
const useCpecialPage = () => {
  ...
  const [shopList, setShopList] = useState<Array<{ label: string; value: number }>>([])
  ...
  const handleDesign = (record: GetCommodityAdornTopicPagePageListResponseDetail) => {
    if (record?.shopEnvironment === WEB) {
      history.jump(
        `/marketingManage/cpecialPageManage/cpecialPage/design/edit?adornId=${record.id}&environment=${record?.shopEnvironment}&shopId=${record?.shopId}`,
      )
    } else {
      history.jump(
        `/marketingManage/cpecialPageManage/cpecialPage/design/mobile/edit?adornId=${record.id}&environment=${record?.shopEnvironment}&shopId=${record?.shopId}`,
      )
    }
  }
  ...
  const handleDesignPreview = (record: GetCommodityAdornTopicPagePageListResponseDetail) => {
    if (record?.shopEnvironment === WEB) {
      history.jump(
        `/marketingManage/cpecialPageManage/cpecialPage/design?adornId=${record.id}&environment=${record?.shopEnvironment}&shopId=${record?.shopId}`,
      )
    } else {
      history.jump(
        `/marketingManage/cpecialPageManage/cpecialPage/design/mobile?adornId=${record.id}&environment=${record?.shopEnvironment}&shopId=${record?.shopId}`,
      )
    }
  }
  ...
  const columns = StandardFormTable.createColumns<GetCommodityAdornTopicPagePageListResponseDetail>([
    ...
    {
      title: translate('web.resource.marketing.shiyongshangcheng'),
      key: 'shopName',
      searchField: {
        type: 'Select',
        name: 'shopId',
