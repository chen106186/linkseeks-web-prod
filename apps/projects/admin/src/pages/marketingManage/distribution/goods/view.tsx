...
import {
  getProductPlatformSocialDistributionGoodsPage,
  postProductPlatformSocialDistributionGoodsExamine,
  postProductPlatformSocialDistributionGoodsAuditStatus,
  postProductPlatformSocialDistributionGoodsBatchExamine,
} from '@apps/apis'
...
const SocialDistributionGoods: React.FC = () => {
  ...
  const handleStatus = async (record: any) => {
    console.log(record)
    await postProductPlatformSocialDistributionGoodsAuditStatus({
      id: record.id,
      isBan: record.auditStatus === 2 ? 1 : 0,
    })
    ref.current.reload()
  }
  ...
  const handleBatch = () => {
    if (!ref.current.selectionKeys.length) {
      message.warning('未选择任何分销商品')
      return
    }
    const products = ref.current.getSelectionItems().map((item) => item.id)
    setAuditIds(products)
  ...
  const columns: RecordColumns<any>[] = [
      ...
      render: (_text, record) => (
        <Space>
          <AuthButton type="custom" code="detail">
            <Link to={`/marketingManage/distribution/goods/detail?id=${record.id}`}>
    ...
    },
    {
      ...
      searchField: 'Input',
      render: (_text, record) => <>{record.memberName}</>,
    },
    {
      title: '申请审核时间',
      key: 'auditSubmitTime',
        ...
        <Space>
          {record.auditStatus > 1 ? (
            <AuthButton type="custom" code="status">
              <Popconfirm
                title={record.auditStatus === 2 ? '确定要下架吗？' : '确定要上架吗？'}
                okText="是"
                cancelText="否"
                onConfirm={() => handleStatus(record)}
              >
                <a>{record.auditStatus === 2 ? '下架' : '上架'}</a>
              </Popconfirm>
            </AuthButton>
          ) : record.auditStatus === 0 ? (
            <Link to={`/marketingManage/distribution/goods/detail?id=${record.id}`}>审核</Link>
          ) : (
            ''
          )}
        </Space>
  ...
  const fetchData = async (params: any) => {
    ...
    return new Promise((resolve) => {
      getProductPlatformSocialDistributionGoodsPage({ ...payload, auditStatus: activeKey }).then((res) => {
        if (res.code === 1000) {
          resolve(res.data)
        }
      })
