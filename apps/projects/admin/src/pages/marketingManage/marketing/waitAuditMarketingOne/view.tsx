...
import { getMarketingPlatformActivityPageExamineStep1 } from '@apps/apis'
import useSelectOptions from './services/hooks/useSelectOptions'

const WaitAuditMarketingOne: React.FC = () => {
  ...
  const columns: RecordColumns<any>[] = [
    ...
    },
    {
      ...
      fixed: 'left',
      render: (text, record) => (
        <EyeAuthButton url={`/marketingManage/marketing/waitAuditMarketingOne/detail?id=${record.id}`}>
          {text}
        </EyeAuthButton>
      ),
    },
      ...
      render: (text, record) =>
        record.exam && (
          <EditAuthButton>
            <Button
              type="link"
              onClick={() => history.push(`/marketingManage/marketing/waitAuditMarketingOne/edit?id=${record.id}`)}
            >
              审核
            </Button>
          </EditAuthButton>
  ...
  const fetchData = async (params: any) => {
    return new Promise((resolve) => {
      ...
      getMarketingPlatformActivityPageExamineStep1({ ...payload }).then((res) => {
        if (res.code === 1000) {
          resolve(res.data)
        }
      })
...
export default WaitAuditMarketingOne
