...
import { getMarketingPlatformActivityPageTobeSubmit, postMarketingPlatformActivitySubmitExamine } from '@apps/apis'
import useSelectOptions from './services/hooks/useSelectOptions'

const WaitSubmitMarketing: React.FC = () => {
  ...
  const fetchSubmitBatch = async (id?: number) => {
    let res: any = null
    if (id) {
      res = await postMarketingPlatformActivitySubmitExamine({ id: Number(id) })
    }
    if (res.code === 1000) {
      ref.current.reload()
    }
  ...
  const columns: RecordColumns<any>[] = [
    ...
    },
    {
      ...
      fixed: 'left',
      render: (text, record) => (
        <EyeAuthButton url={`/marketingManage/marketing/waitSubmitMarketing/detail?id=${record.id}`}>
          {text}
        </EyeAuthButton>
      ),
    },
  ...
  const fetchData = async (params: any) => {
    return new Promise((resolve) => {
      ...
      getMarketingPlatformActivityPageTobeSubmit({ ...payload }).then((res) => {
        if (res.code === 1000) {
          resolve(res.data)
        }
      })
...
export default WaitSubmitMarketing
