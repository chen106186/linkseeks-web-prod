...
import type { GetMemberValidateStep2PageResponseDetail } from '@apps/apis'
import { getMemberValidateStep2Page, postMemberValidateStep2Batch } from '@apps/apis'
import useSelectOptions from './services/hooks/useSelectOptions'

const { confirm } = Modal

const MemberPr2: React.FC = () => {
  const ref = useRef({} as ActionType)
  const selectData = useSelectOptions()

  const handleJumpAudit = (record) => {
    history.push(`/memberManage/memberPr2/edit?id=${record.memberId}&validateId=${record.validateId}`)
  }

  const defaultColumns: RecordColumns<GetMemberValidateStep2PageResponseDetail>[] = [
    {
      title: 'ID',
      key: 'memberId',
      dataIndex: 'memberId',
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <>
          <EyeAuthButton url={`/memberManage/memberPr2/detail?id=${record.memberId}&validateId=${record.validateId}`}>
            {text}
          </EyeAuthButton>
  ...
  const fetchData = async (params: any) => {
    ...
    const res = await getMemberValidateStep2Page(payload)
    if (res.code === 1000) {
      return res.data
    }
    ...
    confirm({
      ...
      onOk() {
        ...
        return new Promise<void>((resolve, reject) => {
          postMemberValidateStep2Batch(members)
            .then((res) => {
              if (res.code === 1000) {
                ref.current.reload()
                resolve()
