...
import { MEMBER_INNER_STATUS_TO_CONFIRM } from '@/constants/const/member'
import type { GetMemberValidateConfirmPageResponseDetail } from '@apps/apis'
import { getMemberValidateConfirmPage, postMemberValidateConfirmBatch } from '@apps/apis'
...
const MemberPrConfirm: React.FC = () => {
  const ref = useRef({} as ActionType)
  const selectData = useSelectOptions()

  const handleJumpAudit = (record) => {
    history.push(`/memberManage/memberPrConfirm/edit?id=${record.memberId}&validateId=${record.validateId}`)
  }

  const defaultColumns: RecordColumns<GetMemberValidateConfirmPageResponseDetail>[] = [
    {
      title: 'ID',
      key: 'memberId',
      dataIndex: 'memberId',
    ...
    },
    {
      ...
      render: (text: any, record) => (
        <>
          <EyeAuthButton
            url={`/memberManage/memberPrConfirm/detail?id=${record.memberId}&validateId=${record.validateId}`}
          >
            {text}
          </EyeAuthButton>
  ...
  const fetchData = async (params: any) => {
    ...
    const res = await getMemberValidateConfirmPage(payload)
    if (res.code === 1000) {
      return res.data
    }
    ...
    confirm({
      ...
      onOk() {
        ...
        return new Promise<void>((resolve, reject) => {
          postMemberValidateConfirmBatch(members)
            .then((res) => {
              if (res.code === 1000) {
                ref.current.reload()
                resolve()
        ...
        tableProps={{
          rowSelection: {
            getCheckboxProps: (record) => ({
              disabled: record.innerStatus !== MEMBER_INNER_STATUS_TO_CONFIRM,
            }),
          },
        }}
      />
