...
import type { GetMemberValidateCommitPageResponseDetail } from '@apps/apis'
import { getMemberValidateCommitPage, postMemberValidateCommitBatch } from '@apps/apis'
...
const MemberPrSubmit: React.FC = () => {
  const ref = useRef({} as ActionType)
  const selectData = useSelectOptions()

  const handleJumpAudit = (record) => {
    history.push(`/memberManage/memberPrSubmit/edit?id=${record.memberId}&validateId=${record.validateId}`)
  }

  const defaultColumns: RecordColumns<GetMemberValidateCommitPageResponseDetail>[] = [
    {
      title: 'ID',
      dataIndex: 'memberId',
      key: 'memberId',
    ...
    },
    {
      ...
      render: (text: any, record) => (
        <>
          <EyeAuthButton
            url={`/memberManage/memberPrSubmit/detail?id=${record.memberId}&validateId=${record.validateId}`}
          >
            {text}
          </EyeAuthButton>
  ...
  const fetchData = async (params: any) => {
    ...
    const res = await getMemberValidateCommitPage(payload)
    if (res.code === 1000) {
      return res.data
    }
    ...
    confirm({
      ...
      onOk() {
        ...
        return new Promise<void>((resolve, reject) => {
          postMemberValidateCommitBatch(members)
            .then((res) => {
              if (res.code === 1000) {
                ref.current.reload()
                resolve()
