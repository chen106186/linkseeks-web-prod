...
const OrderDeleveRecord: React.FC<OrderDeleveRecordProps> = () => {
  ...
  const { deliveries, deliveryDetails, orderKind, orderNo } = data
  ...
  const sideOrderCols: any[] = [
    { title: '收货批次', dataIndex: 'batchNo', align: 'center', key: 'batchNo' },
    { title: '收货单号', dataIndex: 'receiptNo', align: 'center', key: 'receiptNo' },
    {
      title: '收货时间',
    ...
    { title: '物流单号', dataIndex: 'logisticsNo', align: 'center', key: 'logisticsNo' },
    { title: '物流公司', dataIndex: 'company', align: 'center', key: 'company' },
    { title: '发货单号', dataIndex: 'deliveryNo', align: 'center', key: 'deliveryNo' },
    {
      title: '发货时间',
  ...
  const outOrderCols: any[] = [
    ...
    { title: data.orderType === 3 ? '商品名称' : '物料名称', dataIndex: 'name', align: 'center', key: 'name' },
    { title: '品类', dataIndex: 'category', align: 'center', key: 'category' },
    { title: '品牌', dataIndex: 'brand', align: 'center', key: 'brand' },
    { title: '单位', dataIndex: 'unit', align: 'center', key: 'unit' },
  ...
  const DeliverlyItem = ({ record }) => (
    <div className={style.deliverlyItem}>
      <div style={{ flex: 1 }}>
        ...
        <p className={style.nowrap}>发货时间：{record.createTime ? formatTimeString(record.createTime) : '-'}</p>
      </div>
      <div style={{ flex: 1 }}>
  ...
  return (
    <Space direction="vertical" style={{ display: 'flex', width: '100%', marginTop: 16 }} size={16}>
      ...
      <Card title="订单收货明细">
        <Table columns={sideOrderCols} dataSource={deliveryDetails} pagination={false} rowKey="batchNo" />
        {deliveryDetails?.length > 0
          ? deliveryDetails
              .slice()
              .sort((a, b) => (b.batchNo > a.batchNo ? 1 : -1))
              .map((item) => (
                <div className={style['deliveryDetailsBox']} key={item.batchNo}>
