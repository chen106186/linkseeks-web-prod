...
interface OrderLogisticsActionProps {
  action: 'shipment' | 'view'
  disabled?: boolean
}

const OrderLogisticsAction: React.FC<OrderLogisticsActionProps> = ({ action, disabled }) => {
  ...
  return (
    <>
      {action === 'view' ? (
        <Button className={styles.logisticsActionButton} type="link" onClick={showLogisticsDrawer}>
          查看物流
        </Button>
      ) : (
        <Button
          className={styles.shipmentTriggerButton}
          disabled={disabled}
          type="primary"
          onClick={() => setShipmentVisible(true)}
        >
          去发货
        </Button>
      )}
...
export default OrderLogisticsAction
