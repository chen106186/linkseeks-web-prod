...
import { AdvertItem } from '@apps/design-ui'
import { DragIcon, PlusIcon } from '@linkseeks/icons'
import { Button, RadioGroup, Table } from '@linkseeks/ui'
import { Col, Form, InputNumber, Popconfirm, Radio, Row, Slider, Space } from 'antd'
...
const CarouselBanner: React.FC<IProps> = (props) => {
  const { dataList, layoutType, showType = 'standard', componentHeight = 200, verticalMargin = 0 } = props
  ...
  return (
    <SettingPanel onCancel={handleCancel} onOK={handleConfirmSave}>
      ...
      >
        <Form.Item
          label={translate('web.resource.shop.zhanshiyangshi')}
          name="showType"
        ...
        >
          <RadioGroup>
            <Radio value="fullscreen">{translate('web.resource.shop.quanpingkuandu')}</Radio>
            <Radio value="standard">{translate('web.resource.shop.biaozhunkuandu')}</Radio>
          </RadioGroup>
        </Form.Item>
        <Form.Item
