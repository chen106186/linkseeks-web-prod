...
import { CaretDownOutlined, CaretUpOutlined, CheckSquareOutlined } from '@ant-design/icons'
import type { ColumnType } from 'antd/lib/table/interface'
import cx from 'classnames'
import ReturnEle from '@/components/ReturnEle'
import styles from '../products/index.less'
...
import {
  ...
  postProductCommodityPlatformCheckCommodity,
} from '@apps/apis'
...
const viewProducts: React.FC<{}> = () => {
  ...
  const loadPageData = (commodityId: any) => {
    getProductCommodityGetCommodity({ id: commodityId }).then((res) => {
      const { code, data } = res
      if (code === 1000) {
    ...
    getProductCommodityPlatformGetCommodityCheckRecord({ commodityId: commodityId }).then((res) => {
      const { code, data } = res
      if (code === 1000) {
        setCheckRecord(data)
      }
  ...
  const content = (
    <>
      <Descriptions colon={true} style={{ paddingLeft: 48 }}>
        ...
        <Descriptions.Item label="商品品类">{productDetail?.customerCategory?.category?.fullName}</Descriptions.Item>
  ...
  const handleOK = () => {
    checkForm.validateFields().then((values) => {
      postProductCommodityPlatformCheckCommodity({ id: productDetail?.id, ...values }).then((res) => {
        if (res.code === 1000) {
          handleCancel()
          setDisableCheck(true)
          loadPageData(productDetail?.id)
        }
      })
    })
  }
    ...
    >
      <Space direction="vertical" style={{ width: '100%' }} size={16}>
        ...
        <Card headStyle={{ borderBottom: 'none' }} title="商品图片">
          {
            // productDetail?.skuList[0].attributeAndValueList.length === 0 ? <>
      ...
      <Modal title="商品审核" open={visibleModal} onOk={handleOK} onCancel={handleCancel}>
