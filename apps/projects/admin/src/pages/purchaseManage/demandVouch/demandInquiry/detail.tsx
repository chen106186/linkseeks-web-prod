...
const DemandDetailed = () => {
              ...
              <>
                {data.transactionUurls.map((item: any, index: number) => (
                  <Typography.Link
                    ...
                    key={`link_${index + 1}`}
                    href={`/api/order/contractTemplate/downloadContract?contractName=${item.name}&fileUrl=${item.url}`}
                    target="_blank"
                  >
                    <LinkOutlined />
                    {item.name}
  ...
  const fetchDataSource = async () => {
    ...
    await getPurchasePlatformPurchaseInquiryDetails({ ...params }).then((res: any) => {
      if (res.code !== 1000) {
        history.goBack()
        return
      }
      ...
      if (data.areas) {
        data.areas.forEach((item) => {
          _areas.push(`${item.province}/${item.city}`)
        })
        setAreas(_areas)
      }
      if (data.shopIds) {
      ...
      <PeripheralLayout
        ...
        components={
          <Fragment>
            ...
            <MaterialLayout id={id} number={number} fetch={getPurchasePlatformPurchaseInquiryProductlistList} />
