...
import { postMemberManageRoleAdd, postMemberManageRoleUpdate } from '@apps/apis'
...
const useRoleSubmit = () => {
  ...
  const { runAsync: runAdd } = useRequestApi(postMemberManageRoleAdd, { manual: true })
  const { runAsync: runUpdate } = useRequestApi(postMemberManageRoleUpdate, { manual: true })
