...
import {
  ...
  postPurchaseRequisitionWebProcessSaveDefault,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        postPurchaseRequisitionWebProcessSaveDefault(params).then((res) => {
          if (res.code !== 1000) {
            setLoading?.(false)
            return
          }
        ...
        const fetchApi = processId ? postPurchaseRequisitionWebProcessUpdate : postPurchaseRequisitionWebProcessSave
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getPurchaseRequisitionWebProcessMemberPage({ processId } as any),
        getPurchaseRequisitionWebProcessGet({ processId }),
      ]).then((resArr) => {
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getPurchaseRequisitionWebProcessBaseList}
        />
        <ApplyMemberLayout
