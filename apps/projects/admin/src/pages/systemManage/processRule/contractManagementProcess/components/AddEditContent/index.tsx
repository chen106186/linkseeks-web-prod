...
import {
  ...
  postContractPlatformProcessSaveDefault,
  getContractPlatformProcessBaseList,
  getContractPlatformProcessGet,
  getContractPlatformProcessMemberPage,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        postContractPlatformProcessSaveDefault(params).then((res) => {
          if (res.code !== 1000) {
            setLoading?.(false)
            return
          }
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getContractPlatformProcessMemberPage({ processId } as any),
        getContractPlatformProcessGet({ processId }),
      ]).then((resArr) => {
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getContractPlatformProcessBaseList}
        />
        <ApplyMemberLayout
