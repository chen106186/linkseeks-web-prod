...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        const fetchApi = processId ? postProductPlatformMaterialProcessUpdate : postProductPlatformMaterialProcessSave
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getProductPlatformMaterialProcessMemberPage({ processId } as any),
        getProductPlatformMaterialProcessGet({ processId }),
      ]).then((resArr) => {
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getProductPlatformMaterialProcessBaseList}
        />
        <ApplyMemberLayout
