...
import {
  postOrderPlatformQualityProcessSaveDefault,
  postOrderPlatformQualityProcessUpdate,
  postOrderPlatformQualityProcessSave,
  getOrderPlatformQualityProcessBaseList,
  getOrderPlatformQualityProcessMemberPage,
  getOrderPlatformQualityProcessGet,
} from '@apps/apis'
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        postOrderPlatformQualityProcessSaveDefault(params).then((res) => {
          if (res.code !== 1000) {
            setLoading?.(false)
            return
          }
        ...
        const fetchApi = processId ? postOrderPlatformQualityProcessUpdate : postOrderPlatformQualityProcessSave
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getOrderPlatformQualityProcessMemberPage({ processId } as any),
        getOrderPlatformQualityProcessGet({ processId }),
      ]).then((resArr) => {
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getOrderPlatformQualityProcessBaseList}
        />
        <ApplyMemberLayout
