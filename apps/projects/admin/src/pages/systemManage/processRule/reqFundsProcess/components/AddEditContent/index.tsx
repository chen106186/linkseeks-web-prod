...
import { getMemberManageAllConsumerPage } from '@apps/apis'
import { VISIT_TYPE } from '@/constants'
import {
  getSettlementBusinessApplyAmountProcessGet,
  postSettlementBusinessApplyAmountProcessSave,
  postSettlementBusinessApplyAmountProcessSaveDefault,
  postSettlementBusinessApplyAmountProcessUpdate,
  getSettlementBusinessApplyAmountProcessMemberPage,
  getSettlementBusinessApplyAmountProcessBaseList,
} from '@apps/apis'
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        postSettlementBusinessApplyAmountProcessSaveDefault(params).then((res) => {
          if (res.code !== 1000) {
            setLoading?.(false)
            return
          }
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getSettlementBusinessApplyAmountProcessMemberPage({ processId } as any),
        getSettlementBusinessApplyAmountProcessGet({ processId }),
      ]).then((resArr) => {
        if (resArr.every((item) => item && item.code === 1000)) {
          const members = resArr[0]?.data?.map((_item) => ({ ..._item, mrId: `${_item.memberId}_${_item.roleId}` }))
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getSettlementBusinessApplyAmountProcessBaseList}
        />
        <ApplyMemberLayout
          disabled={type === VISIT_TYPE.VIEW || isEditDefault}
          fetchMemberApi={getMemberManageAllConsumerPage}
        />
      </Space>
    </ContentLayout>
  )
