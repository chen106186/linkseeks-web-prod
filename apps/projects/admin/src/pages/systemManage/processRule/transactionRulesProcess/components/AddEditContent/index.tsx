...
import {
  ...
  getOrderPlatformTradeProcessMemberPage,
  postOrderPlatformTradeProcessCreate,
  postOrderPlatformTradeProcessSaveDefault,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, title, type }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      if (isEditDefault) {
        ...
        postOrderPlatformTradeProcessSaveDefault(params).then((res) => {
          if (res.code !== 1000) {
            setLoading?.(false)
            return
          }
        ...
        const fetchApi = processId ? postOrderPlatformTradeProcessUpdate : postOrderPlatformTradeProcessCreate
  ...
  const getDetail = async () => {
    if (processId) {
      Promise.all([
        getOrderPlatformTradeProcessMemberPage({ processId, current: 1, pageSize: 100 } as any),
        getOrderPlatformTradeProcessGet({ processId }),
      ]).then((resArr) => {
    ...
    <ContentLayout ref={ref} form={form} title={title} type={type} onSubmit={handleSubmit}>
      <Space direction="vertical" size={16} style={{ display: 'flex', width: '100%' }}>
        ...
        <ProcessSelectLayout
          ...
          fetchApi={getOrderPlatformTradeProcessBaseList}
        />
        <ApplyMemberLayout
          disabled={type === VISIT_TYPE.VIEW || isEditDefault}
          fetchMemberApi={getMemberManageAllProviderPage}
        />
      </Space>
    </ContentLayout>
  )
