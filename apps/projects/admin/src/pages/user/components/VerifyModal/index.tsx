...
import { sendForgetSms } from '../../forget/services/feature'
...
const VerifyModal: React.FC<VerifyModalProps> = (props) => {
  const { account, accountInfo, visible, form, type, setVisible, onCheckTypeChange, onSmsSend, ...modalProps } = props
  ...
  const showSwitchBUtton = useMemo(() => {
    const switchText = type === 'phone' ? '切换邮箱验证' : '切换手机号验证'
  ...
  const handleStartSms = (form, _, setLoading, callback) => {
    if (onSmsSend) {
      onSmsSend(form, _, setLoading, callback)
    } else {
      const account = form.getFieldValue('checkAccount')
      if (account) {
        setLoading(true)
        sendForgetSms(
          type,
