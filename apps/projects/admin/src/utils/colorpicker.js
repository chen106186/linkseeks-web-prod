...
Colorpicker.prototype = {
  init(opt) {
    ...
    const body = opt.box ? document.getElementsByClassName(opt.box)[0] : document.getElementsByTagName('body')[0]
    ...
    this.elem_showModeBtn.addEventListener(
      'click',
      function () {
    ...
    this.elem_colorPalette.addEventListener(
      ...
      function (e) {
        if (e.target.tagName.toLocaleLowerCase() == 'p') {
          ...
          const rgb = colorStr.slice(4, -1).split(',')
          const rgba = {
            r: parseInt(rgb[0]),
            g: parseInt(rgb[1]),
            b: parseInt(rgb[2]),
          ...
          switch (_this.current_mode) {
            case 'hex':
              ...
              break
            case 'rgb':
              const inputs = _this.elem_wrap.getElementsByTagName('input')
  ...
  render: function () {
    const tpl = `<div style="position: fixed; top: 0px; right: 0px; bottom: 0px; left: 0px;"></div>
                ...
                this.rgba.r
                      ...
                      this.rgba.r
                    },${this.rgba.g},${this.rgba.b}); z-index: 2;"></div>
  ...
  hexToRgb: function (hex) {
    ...
    return { r: newHex >> 16, g: (newHex & 0x00ff00) >> 8, b: newHex & 0x0000ff }
  },
