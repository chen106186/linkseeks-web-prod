...
export function Avatar<T extends AvatarProps>(props: T): React.ReactElement {
  const { image, size = 32, shape = 'circle', update, onClick, list = [] } = props
  ...
  return (
    ...
    >
      ...
      {update && (
        <div
          className={`tui-kit-avatar-edit ${isEdit ? 'displayFlex' : ''}`}
          role="button"
          tabIndex={0}
          onClick={toggle}
        >
        ...
        >
          {list.map((item: string, index: number) => {
            ...
            return (
              ...
              >
                <img className="avatar-image" alt={item} src={item} />
              </li>
            )
          })}
        </ul>
