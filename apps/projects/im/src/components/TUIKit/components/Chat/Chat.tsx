...
import TUIChatEngine, { TUIChatService, IMessageModel, TUIStore, StoreName } from '@tencentcloud/chat-uikit-engine'
...
import { TUIMessageProps, TUIMessage as TUIMessageDefault } from '../MessageElement'
...
function UnMemoizedChat<T extends TUIChatProps>(props: PropsWithChildren<T>): React.ReactElement {
  const {
  ...
  return <TUIChatInner chat={chat} {...props} conversation={conversation} key={conversation.conversationID} />
}

function TUIChatInner<T extends TUIChatInnerProps>(props: PropsWithChildren<T>): React.ReactElement {
  const {
    chat,
    conversation,
    className,
  ...
  const chatStateContextValue =
    state &&
    useCreateTUIChatStateContext({
      chat,
      conversation,
      messageListRef,
      textareaRef,
  ...
  const { editLocalMessage, updateUploadPendingMessageList } = useHandleMessageList({
    chat,
    conversation,
  ...
  const { operateMessage, setAudioSource, setVideoSource, setHighlightedMessageId, setActiveMessageID } =
    useHandleMessage({
      // eslint-disable-next-line
      // @ts-ignore
      state,
          ...
          <ComponentProvider value={componentContextValue}>
            {children || (
              <>
                <ChatHeaderElement />
                <MessageList />
                <MessageInputElement extraPickers={extraPickers} />
              </>
            )}
          </ComponentProvider>
