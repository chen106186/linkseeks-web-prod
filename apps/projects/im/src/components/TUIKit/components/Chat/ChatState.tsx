...
import { handleEditMessage, handleUploadPendingMessage } from './utils'
...
export const chatReducer = (state: TUIChatStateContextValue, action: ChatStateReducerAction) => {
  switch (action?.type) {
    ...
    case CONSTANT_DISPATCH_TYPE.UPDATE_UPLOAD_PENDING_MESSAGE_LIST:
      return (
        state?.uploadPendingMessageList &&
        action.value && {
          ...state,
          uploadPendingMessageList: [...handleUploadPendingMessage(state.uploadPendingMessageList, action.value)],
        }
      )
