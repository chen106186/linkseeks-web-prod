...
function UnMemoizedChatSetting<T>(): React.ReactElement {
    ...
    activeConversation && (
      <div className={`tui-manage ${isH5 ? 'tui-h5-manage' : ''}`}>
        <div className="tui-manage-title">
          {isPC && <Icon onClick={close} type={IconTypes.CANCEL} width={9} height={16} />}
          {isH5 && <Icon onClick={close} type={IconTypes.BACK} width={9} height={16} />}
          <span style={{ marginLeft: '10px' }}>{t('TUIConversation.Conversation Information')}</span>
        </div>
        <div className="tui-manage-container">
          <div className="tui-manage-info">
            <div className="info-avatar">
              <Avatar
                size={64}
                image={profile?.avatar || (profile?.groupID ? defaultGroupAvatarWork : defaultUserAvatar)}
              />
            </div>
...
export const ChatSetting = React.memo(UnMemoizedChatSetting) as unknown as typeof UnMemoizedChatSetting
