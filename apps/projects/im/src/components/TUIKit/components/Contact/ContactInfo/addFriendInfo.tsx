...
export function UnMemoizedAddFriendInfo<T extends Props>(props: T): React.ReactElement {
  const { profile } = props
  ...
  const { addFriend } = useContactInfo()
      ...
      {!isSendedAdd ? (
        <div className="tui-contact-info-content">
          <div className="content-item-wording">
            ...
            <textarea
              className="content-item-wording-text"
              value={wording}
              onChange={(e: any) => {
                handleChange(e)
              }}
          ...
          <div className="content-btn-container">
            <div className="content-item-btn confirm-btn" role="button" tabIndex={0} onClick={addFriendHandler}>
              {t('TUIContact.Send application')}
            </div>
          </div>
        </div>
      ) : (
