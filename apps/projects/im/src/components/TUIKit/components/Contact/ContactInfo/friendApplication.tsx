...
export function UnMemoizedFriendApplication<T extends Props>(props: T): React.ReactElement {
  const { setActiveContact } = useUIManager('TUIContact')
  const { t } = useTranslation()
  const { application } = props
  const { userID, nick, avatar, type, wording } = application
  const { acceptFriendApplication, refuseFriendApplication } = useContactInfo()
      ...
      {type === TencentCloudChat.TYPES.SNS_APPLICATION_SENT_TO_ME && (
        <div className="tui-contact-info-content">
          <div className="content-btn-container">
            <div
              className="content-item-btn delete-btn"
              role="button"
              tabIndex={0}
              onClick={refuseFriendApplicationHandler}
            >
              {t('TUIContact.Refuse')}
            </div>
            <div
              className="content-item-btn confirm-btn"
              role="button"
              tabIndex={0}
              onClick={acceptFriendApplicationHandler}
            >
              {t('TUIContact.Agree')}
            </div>
          </div>
