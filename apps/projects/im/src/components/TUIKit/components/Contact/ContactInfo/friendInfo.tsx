...
export function UnMemoizedFriendInfo<T extends Props>(props: T): React.ReactElement {
  const { chat } = useUIKit()
  const { contactData, setActiveContact, setActiveConversation } = useUIManager('TUIContact')
  ...
  const { addToBlocklist, deleteFriend } = useContactInfo()
  useEffect(() => {
  ...
  const openC2CConversation = () => {
    ...
    showChats && showChats()
    TUIConversationService.switchConversation(conversationID).then((conversationModel: IConversationModel) => {
    ...
    <>
      ...
      <div className="tui-contact-info-content">
        ...
        <div className="content-item">
          ...
          <Switch className="content-item-text" onChange={addToBlocklistHandler} checked={isAddToBlocklist} />
        </div>
        <div className="content-btn-container">
          <div className="content-item-btn delete-btn" role="button" tabIndex={0} onClick={deleteFriendHandler}>
            {t('TUIContact.Delete friend')}
          </div>
          <div className="content-item-btn confirm-btn" role="button" tabIndex={0} onClick={openC2CConversation}>
