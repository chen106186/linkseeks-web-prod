...
function UnMemoizedContactList<T>(): React.ReactElement {
  ...
  const { isShowContactList, friendList, blocklistProfile, friendApplicationList } =
    useTUIContactContext('TUIContactList')
  const { acceptFriendApplication } = useContactInfo()
  ...
  const RenderContactList = ({ type, isShow, setShow, list, title }: RenderContactListProps) => {
    return (
      <>
        <div className="tui-contacts-title" role="button" tabIndex={0} onClick={() => setShow(!isShow)}>
      ...
      <div className={`tui-contacts-list ${isH5 ? 'tui-contacts-list-h5' : ''} `}>
        ...
        {iShowFriendApplication &&
          friendApplicationList?.map((item, index) => {
            const { userID, avatar, nick, wording, type } = item
            return (
              <div
                role="button"
                tabIndex={0}
              ...
              >
                ...
                <div className="tui-contacts-list-item-card">
                  <div>
                    <p className="tui-contacts-list-item-name text-ellipsis">{nick || userID}</p>
                    {wording !== '' && <p className="tui-contacts-list-item-text text-ellipsis">{wording}</p>}
                  </div>
                  {type === TencentCloudChat.TYPES.SNS_APPLICATION_SENT_BY_ME && (
                    <p className="tui-contacts-list-btn-text text-ellipsis">
                      {t('TUIContact.waiting for verification')}
                    </p>
                  )}
