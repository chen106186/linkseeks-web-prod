...
function useContactSearch() {
  ...
  const checkFriend = (info: any): Promise<any> =>
    chat
      .checkFriend({
        userIDList: [info?.userID],
        type: TencentCloudChat.TYPES.SNS_CHECK_TYPE_BOTH,
      })
      .then((res: any) => res?.data?.successUserIDList[0]?.relation === TencentCloudChat.TYPES.SNS_TYPE_BOTH_WAY)

  const isBlock = (userID: string) => blockList?.includes(userID)
