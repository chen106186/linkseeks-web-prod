...
export function ConversationCreateGroupDetail(props: ConversationCreateGroupDetailProps) {
  const { profileList, pageState, setPageState, onBeforeCreateConversation, onConversationCreated } = props
  ...
  const [groupName, setGroupName] = useState(name.length >= 15 ? `${name.slice(0, 12)}...` : name)
  ...
  return pageState !== PageStateTypes.GROUP_TYPE ? (
    <>
      <div className="tui-conversation-create-group-detail">
        ...
        <div className="create-group-box create-group-type" onClick={showGroupTypeInfo}>
          <Input
            ...
            suffix={<Icon onClick={showGroupTypeInfo} type={IconTypes.ARROW_RIGHT} width={7} height={12} />}
        ...
        <div className="create-group-illustrate">{t(`TUIConversation.${getDes()}`)}</div>
      ...
      <div className="tui-conversation-create-next-container">
        <div role="presentation" className="tui-conversation-create-next" onClick={next}>
          {t('TUIConversation.Create')}
        </div>
      </div>
    </>
  ) : (
    <ConversationGroupTypeInfo groupType={groupType} setGroupType={setGroupType} setPageState={setPageState} />
  )
}
