...
export function ConversationCreateUserSelectList(props: ConversationCreateUserSelectListProps) {
  ...
  const { getFriendListSortSearchResult } = useConversationCreate(conversationList, (newFriendListResult) => {
  ...
  const userSelectListChange = (e: any, profile: Profile, domList = []) => {
    ...
    if (checked) {
      ...
      selectList.splice(
        selectList.findIndex((item) => item.profile.userID === userID),
        1,
      )
    }
  ...
  return (
    <>
      ...
      />
      {isCreateGroup && <ConversationCreateSelectView selectList={selectList} setSelectList={setSelectList} />}
      {!isCreateGroup && (
