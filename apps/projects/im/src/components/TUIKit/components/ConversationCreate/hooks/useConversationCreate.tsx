...
export const useConversationCreate = (
  ...
  const queryFriendList = async (friendList: Friend[]) => {
    const frequentlyConversationProfile = conversationList
      .filter((item: IConversationModel) => item?.type === TencentCloudChat.TYPES.CONV_C2C)
      .slice(0, 5)
      .map((item: IConversationModel) => item?.userProfile)
