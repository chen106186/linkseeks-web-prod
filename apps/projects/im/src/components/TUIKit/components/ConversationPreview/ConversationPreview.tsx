...
function ConversationPreviewTimestamp(props: { conversation: IConversationModel }): string | JSX.Element {
...
function ConversationPreviewAbstract(props: { conversation: IConversationModel }): string | JSX.Element {
  const { conversation } = props
  const { myProfile } = useUIManager()
  return <div className="uikit-conversation-preview__abstract">{getLatestMessagePreview(conversation, myProfile)}</div>
}

function ConversationPreviewUnread(props: { conversation: IConversationModel }): string | JSX.Element {
  const { conversation } = props
  let content
  ...
  return <div className="uikit-conversation-preview__unread">{content}</div>
}

function ConversationPreview<T extends IConversationPreviewProps>(props: T): React.ReactElement {
  const {
    conversation,
    isSelected = false,
    enableActions = true,
...
function ConversationPreviewUI<T extends IConversationPreviewUIProps>(props: T): React.ReactElement {
  const {
    conversation,
    isSelected,
    enableActions,
  ...
  const { currentConversation, setCurrentConversation } = useConversationList()
    ...
    <div
      ...
      className={cs('uikit-conversation-preview', className, {
        ...
        'uikit-conversation-preview--active':
          isSelected || conversation.conversationID === currentConversation?.conversationID,
      ...
      {children || (
        <>
          <div className="uikit-conversation-preview__avatar">
            <Avatar image={conversation.getAvatar()} size={40} />
          </div>
          ...
          <div className="uikit-conversation-preview__external">
            ...
            {enableActions && isActionMenuActive ? (
              <ConversationActions conversation={conversation} {...actionsConfig} />
            ) : (
              LastMessageTimestamp
            )}
          </div>
