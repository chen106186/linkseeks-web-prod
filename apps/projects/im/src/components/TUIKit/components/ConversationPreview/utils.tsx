...
export const generateHighlightTitle = (
  ...
  const titleList = titleString.split(new RegExp(`(${highlightMatchString})`, 'gi')).map((item: string) => {
    return { text: item, isHighlight: item.toLowerCase() === matchLowerCaseString }
  })
  return titleList
}

export const getLatestMessagePreview = (conversation: IConversationModel, myProfile?: Profile) => {
  const { lastMessage, type } = conversation
  if (!lastMessage) {
    return ''
  }
  const { fromAccount, nick, nameCard, isRevoked } = lastMessage
...
export const getDisplayTitle = (
  ...
  const { name, nick, groupID, userID } = getMessageProfile(conversation)
  ...
  const handleTitle = (str: string) => {
    ...
    return (
      <div>
        {searchValue &&
          titleList.map((textItem: string, index: number) =>
