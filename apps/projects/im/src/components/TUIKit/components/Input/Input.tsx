...
import React, { useRef, useState, useImperativeHandle, InputHTMLAttributes } from 'react'
...
export const Input = React.forwardRef<InputRef, InputProps>((props: InputProps, ref): React.ReactElement => {
  const {
    className = '',
    inputClassName = '',
    placeholder,
  ...
  return (
    <div className={`${className} tui-kit-input-container`}>
      <div
        className={`tui-kit-input-box ${border && `tui-kit-input-border--${border}`} ${
          focused ? 'tui-kit-input-box--focus' : 'tui-kit-input-box--blur'
        }`}
      >
        {prefix}
        <input
