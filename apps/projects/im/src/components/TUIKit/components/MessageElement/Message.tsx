...
export interface TUIMessageProps extends TUIMessageBasicProps {
  ...
  CardElement?: React.ComponentType<{ message: Message }>
}
function TUIMessageWithContext<T extends TUIMessageProps>(props: PropsWithChildren<T>): React.ReactElement {
  const {
  ...
  const { MessagePlugins: ContextMessagePlugins, MessageContext: ContextMessageContext } =
    useComponentContext('TUIMessage')

  const { messageConfig } = useTUIChatStateContext('TUIMessage')
  ...
  const messageContextValue = {
    ...
    CustemElement: CardElement || messageConfig?.CardElement,
...
const MemoizedTUIMessage = React.memo(TUIMessageWithContext) as typeof TUIMessageWithContext

export function TUIMessage(props: TUIMessageProps): React.ReactElement {
  const { message } = props
  const { handleDelMessage } = useMessageHandler({ message })
  return <MemoizedTUIMessage handleDelete={handleDelMessage} {...props} />
}
