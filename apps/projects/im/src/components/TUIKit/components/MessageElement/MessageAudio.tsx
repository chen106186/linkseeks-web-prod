...
function MessageAudioWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  ...
  return (
    <div className="message-audio">
      <div className={`message-audio-content message-audio-${message?.flow}`}>
        <Icon
          className={`${message?.flow} ${playClassName ? 'playing' : ''}`}
          type={IconTypes.VOICE}
          width={16}
          height={16}
          onClick={play}
        />
...
const MemoizedMessageAudio = React.memo(MessageAudioWithContext) as typeof MessageAudioWithContext

export function MessageAudio(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageAudio {...props} />
}
