...
function MessageBubbleWithContext<T extends MessageBubbleProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { message, children, Context, Plugins } = props

  const [PluginsShow, setPluginsShow] = useState(false)

  const { messageReply, replyMessage, sender } = useMessageReply({ message })
  ...
  return (
    <div className="meesage-bubble">
      ...
      >
        ...
        >
          {messageReply && (
            <div className="meesage-bubble-reply-main" role="menuitem" tabIndex={0} onClick={handleReplyMessage}>
        ...
        {Plugins && <div className="message-plugin">{PluginsShow && <Plugins message={message} />}</div>}
      </div>
      <div className="message-bubble-status icon">
        {message?.status === MESSAGE_STATUS.FAIL && <i className="icon-fail" />}
        {message?.status === MESSAGE_STATUS.UNSEND && <Icon width={10} height={10} type={IconTypes.PROGRESS} />}
      </div>
    </div>
  )
}

const MemoizedMessageBubble = React.memo(MessageBubbleWithContext) as typeof MessageBubbleWithContext

export function MessageBubble(props: MessageBubbleProps): React.ReactElement {
  return <MemoizedMessageBubble {...props} />
}
