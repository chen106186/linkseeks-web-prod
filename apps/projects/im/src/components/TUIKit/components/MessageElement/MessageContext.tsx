...
const components: any = {
  ...
  [TencentCloudChat.TYPES.MSG_CUSTOM]: MessageCard,
...
function MessageContextWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { message } = props
...
const MemoizedMessageContext = React.memo(MessageContextWithContext) as typeof MessageContextWithContext

export function MessageContext(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageContext {...props} />
}
