...
function MessageCustomWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  ...
  const handleContext = (data: any) => {
    if (data.data === 'Hyperlink') {
      ...
      if (extension?.item) {
        return extension?.item.map((item: any) => (
          <a target="_blank" key={item.value} href={item.value} rel="noreferrer">
            {item.value}
          </a>
        ))
      }
      if (extension?.hyperlinks_text) {
        ...
        return (
          <>
            {extension.title}{' '}
            <a target="_blank" key={hyperlinks?.value} href={hyperlinks?.value} rel="noreferrer">
              {hyperlinks.key}
            </a>
          </>
        )
    ...
    if (
      botMessage?.chatbotPlugin === 1 &&
      botMessage?.src === 15 &&
      (botMessage?.subtype === 'welcome_msg' || botMessage?.subtype === 'clarify_msg')
    ) {
      return (
...
const MemoizedMessageCustom = React.memo(MessageCustomWithContext) as typeof MessageCustomWithContext
