...
function TUIMessageDefaultWithContext<T extends TUIMessageProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { message, MessageContext: propsMessageContext, MessagePlugins: propsMessagePlugins, className } = props

  const { MessageContext: contextMessageContext, MessagePlugins: contextMessagePlugins } =
  ...
  return (
    <div
      ...
      `}
    >
      {message?.type === TencentCloudChat.TYPES.MSG_GRP_TIP && <MessageTip message={message} />}
      {message?.type === TencentCloudChat.TYPES.MSG_GRP_SYS_NOTICE && <MessageSystem message={message} />}
      {message?.isRevoked && <MessageRevoke message={message} />}
      {message?.type !== TencentCloudChat.TYPES.MSG_GRP_TIP &&
        message?.type !== TencentCloudChat.TYPES.MSG_GRP_SYS_NOTICE &&
        !message?.isRevoked && (
          <div className={`${message?.flow}`} key={message?.ID}>
            {prefix}
            ...
            <main data-message-id={message?.ID} className="content">
              ...
              <MessageBubble message={message} Context={MessageContextUIComponent} Plugins={MessagePlugins}>
...
const MemoizedTUIMessageDefault = React.memo(TUIMessageDefaultWithContext) as typeof TUIMessageDefaultWithContext

export function TUIMessageDefault(props: TUIMessageProps): React.ReactElement {
  const { message } = props
  return <MemoizedTUIMessageDefault {...props} />
}
