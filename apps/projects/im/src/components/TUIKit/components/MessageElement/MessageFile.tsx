...
function MessageFileWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  ...
  return (
    <a
      ...
      target="_parent"
      className={`bubble message-file bubble-${message?.flow} ${
...
const MemoizedMessageFile = React.memo(MessageFileWithContext) as typeof MessageFileWithContext

export function MessageFile(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageFile {...props} />
}
