...
function MessageMergerWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { context, message, children } = props

  return (
    <div
      className={`bubble message-merger bubble-${message?.flow} ${
      ...
      <ul className="message-merger-list">
        {context?.abstractList.length > 0 &&
          context?.abstractList.map((item: string, index: number) => {
            ...
            return (
              <li className="message-merger-item" key={key}>
                {item}
              </li>
            )
          })}
...
const MemoizedMessageMerger = React.memo(MessageMergerWithContext) as typeof MessageMergerWithContext

export function MessageMerger(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageMerger {...props} />
}
