...
export function MessageName<T extends MessageNameProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { className, CustomName, message, showType } = props

  const show =
    (showType === messageShowType.ALL || message?.flow === showType) &&
