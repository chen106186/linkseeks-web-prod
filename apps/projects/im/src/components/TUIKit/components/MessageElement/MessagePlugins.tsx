...
export function MessagePlugins<T extends MessagePluginsProps>(props: PropsWithChildren<T>): React.ReactElement {
  const {
  ...
  const RevokeElement = useMessagePluginElement({
    ...
    message,
    isShow:
      pluginConfig.revoke.isShow && message?.status === MESSAGE_STATUS.SUCCESS && message.flow === MESSAGE_FLOW.OUT,
  ...
  const defaultPlugins = [RevokeElement, DeleteElement, ResendElement, CopyElement]

  const plugins = (propsPlugins || contextPlugin?.plugins || defaultPlugins).filter((item) => item)

  const MoreIcon = propsMoreIcon || contextPlugin?.MoreIcon || (
    <Icon className="icon-more" width={16} height={16} type={IconTypes.MORE} />
  )
