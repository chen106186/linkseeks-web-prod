...
function MessageRevokeWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { message } = props
  ...
  return (
    <div className="bubble message-system message-revoke">
      ...
      {message?.flow === MESSAGE_FLOW.OUT && message?.type === TencentCloudChat.TYPES.MSG_TEXT && (
...
const MemoizedMessageRevoke = React.memo(MessageRevokeWithContext) as typeof MessageRevokeWithContext

export function MessageRevoke(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageRevoke {...props} />
}
