...
function MessageSystemWithContext<T extends MessageContextProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { message } = props
...
const MemoizedMessageSystem = React.memo(MessageSystemWithContext) as typeof MessageSystemWithContext

export function MessageSystem(props: MessageContextProps): React.ReactElement {
  return <MemoizedMessageSystem {...props} />
}
