...
export const useMessageHandler = (props: MessageHandlerProps) => {
  const { message, handleError } = props

  const { editLocalMessage, operateMessage, revokeMessage } = useTUIChatActionContext('useDeleteHandler')
          ...
          if (handleError) {
            ...
            const text =
              message.flow === MESSAGE_FLOW.OUT
                ? t('TUIChat.The message recall exceeded the time limit (default 2 minutes)')
                : t('TUIChat.Error revoke Message')
