...
export function handleShowLastMessage(item: Conversation) {
  ...
  const showUnreadCount =
    conversation.unreadCount > 0 && conversation.messageRemindType === TencentCloudChat.TYPES.MSG_REMIND_ACPT_NOT_NOTE
  ...
  if (
    ...
    lastMessage.type === TencentCloudChat.TYPES.MSG_GRP_TIP
  ) {
...
export function formatTime(secondTime: number) {
  ...
  if (time >= 3600) {
    ...
    if (seconds > 60) {
      minite =
        parseInt(`${seconds / 60}`, 10) < 10 ? `0${parseInt(`${seconds / 60}`, 10)}` : parseInt(`${seconds / 60}`, 10)
