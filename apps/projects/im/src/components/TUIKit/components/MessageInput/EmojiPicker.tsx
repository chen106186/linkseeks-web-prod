...
export function EmojiPicker(): React.ReactElement {
  ...
  const { onSelectEmoji, sendFaceMessage } = useTUIMessageInputContext('TUIMessageInputDefault')
    ...
    <div className="emoji-picker input-plugin-popup">
      ...
      >
        <ul className="face-list">
          {index === 0 &&
            emojiName.map((item: string, emojiIndex: number) => {
              const key = item + emojiIndex
              return (
                <li role="menuitem" className="face-list-item" key={key} onClick={handleSelectEmoji}>
                  <img src={`${emojiUrl + emojiMap[item]}`} alt="" data-data={item} />
                </li>
              )
            })}
        </ul>
