...
export function Forward<T extends TUIForwardToProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { handleForward: propsHandleForward } = props
  ...
  return message && conversationList.length > 0 ? (
    <Model onClick={handleClose}>
      <div
        role="button"
        tabIndex={0}
        className="tui-forward"
        onClick={(e) => {
          stopPropagation(e)
        }}
      >
        <header className="tui-forward-header">
          <Icon type={IconTypes.CANCEL} width={16} height={16} onClick={handleClose} />
        ...
        <main className="tui-forward-main">
          {searchValue && (
            <ul className="tui-forward-list">
              ...
              {searchResult.length === 0 && <p className="no-result">{t('TUIChat.No Result')}</p>}
            </ul>
          )}
            ...
            <ul className="tui-forward-list">
              {RecentList.map((item: any) => (
                <li key={item.conversationID} className="tui-forward-list-item">
                  ...
                  <Checkbox onChange={handleCheckboxChange} id={`${item.conversationID}`} value={item} />
                </li>
              ))}
            </ul>
          )}
        ...
        <footer className="tui-forward-footer">
          ...
          <button type="button" className="button" onClick={handleForward} disabled={selectList.length === 0}>
            {t('TUIChat.Forward')}
          </button>
        </footer>
      </div>
