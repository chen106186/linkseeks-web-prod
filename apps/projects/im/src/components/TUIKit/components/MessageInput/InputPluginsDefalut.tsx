...
export function InputPluginsDefalut(props): React.ReactElement {
  const { extraPickers } = props
  const { sendUploadMessage, pluginConfig: propsPluginConfig } = useTUIMessageInputContext('TUIMessageInputDefault')
  const { t } = useTranslation()

  const { TUIMessageInputConfig } = useTUIChatStateContext('TUIMessageInput')

  const propPlugins = propsPluginConfig?.plugins || TUIMessageInputConfig?.pluginConfig?.plugins || []
  const showNumber = propsPluginConfig?.showNumber || TUIMessageInputConfig?.pluginConfig?.showNumber || 1
  ...
  const ExtraPickers = extraPickers
    ? extraPickers.map((v) => {
        const { children, ...props } = v
        return (
          <div className="input-plugin-item" {...props}>
