...
import { UnknowPorps, useComponentContext, useTUIChatStateContext, useUIManager } from '../../context'
...
function TUIMessageInputProvider<T extends TUIMessageInputProps>(props: PropsWithChildren<T>): React.ReactElement {
  const { children, disabled: propsDisabled, focus: propsFoces, pluginConfig, textareaRef: propsTextareaRef } = props
  ...
  return <TUIMessageInputContextProvider value={messageInputContextValue}>{children}</TUIMessageInputContextProvider>
}
...
function UnMemoizedMessageInput<T extends TUIMessageInputProps>(props: PropsWithChildren<T>): React.ReactElement {
  ...
  const InputPlugins = PropInputPlugins || ContextInputPlugins || InputPluginsDefalut || EmptyStateIndicator
      ...
      <TUIMessageInputProvider {...props}>
        ...
        <div className="tui-message-input-main">
          <div className="tui-message-input-box">
            <InputPlugins extraPickers={extraPickers} />
            <Input />
          </div>
