...
export const useMessageInputState = (props: TUIMessageInputProps) => {
  ...
  const { sendUploadMessage } = useUploadPicker({
    state,
    dispatch,
  })
  const { handleChange, handleSubmit, handleKeyDown, handlePasete, insertText, setText, setCursorPos } =
    useMessageInputText({
      state,
      dispatch,
      textareaRef,
  ...
  const { onSelectEmoji, sendFaceMessage } = useEmojiPicker({
    state,
    dispatch,
    textareaRef,
    insertText,
