import React, { useCallback, ChangeEventHandler, MutableRefObject } from 'react'
...
export const useMessageInputText = (props: useMessageInputTextProps) => {
  const { state, dispatch, focus, textareaRef, sendUploadMessage } = props
  ...
  const insertText = useCallback(
    (textToInsert: string) => {
      dispatch({
        type: CONSTANT_DISPATCH_TYPE.SET_TEXT,
        getNewText: (text: string) =>
          `${text.slice(0, state?.cursorPos?.start || 0)}${textToInsert}${text.slice(state?.cursorPos?.start || 0)}`,
      })
      dispatch({
