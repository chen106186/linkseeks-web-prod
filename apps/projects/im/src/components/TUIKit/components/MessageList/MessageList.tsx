...
import { useTUIChatStateContext, useTUIChatActionContext, useComponentContext } from '../../context'
...
function TUIMessageListWithContext<T extends MessageListProps>(props: PropsWithChildren<T>): React.ReactElement {
  const {
  ...
  const highlightedMessageId =
    propsHighlightedMessageId || TUIMessageListConfig?.highlightedMessageId || contextHighlightedMessageId
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!isCompleted && parentElement && parentElement?.clientHeight >= ulElement?.clientHeight) {
  ...
  return (
    <div className={`message-list ${propsClassName} ${!firstRender ? 'hide' : ''}`} ref={messageListRef}>
      ...
      <InfiniteScroll className="message-list-infinite-scroll" hasMore loadMore={loadMore} threshold={1}>
        <ul ref={setUlElement}>
...
const MemoizedTUIMessageListContext = React.memo(TUIMessageListWithContext) as typeof TUIMessageListWithContext
