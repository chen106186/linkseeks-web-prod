...
function useMessageListElement<T extends MessageListElementProps>(props: PropsWithChildren<T>) {
  const { enrichedMessageList, TUIMessage, intervalsTimer } = props
  const { language } = useUIKit('TUIConversation')

  return useMemo(
    () =>
      enrichedMessageList?.map((item: Message, index: number) => {
        ...
        const isShowIntervalsTimer =
          intervalsTimer && preMessageTImer !== -1 ? currrentTimer - preMessageTImer >= intervalsTimer : false
        return (
