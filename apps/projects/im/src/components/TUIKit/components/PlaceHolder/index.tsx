...
export const PlaceHolderStringTypes = {
  ...
  [PlaceHolderTypes.NO_CONVERSATIONS]: '没有可用的会话',
...
export function PlaceHolder({ className = '', type, iconSize = 42, searchString = '', retry }: PlaceHolderProps) {
  return (
    <div className={cs(className, 'tui-place-holder')}>
      {PlaceHolderIconTypes[type] && (
        <Icon className="tui-place-holder__icon" type={PlaceHolderIconTypes[type]} width={iconSize} height={iconSize} />
      )}
      {PlaceHolderStringTypes[type] && (
        <div className="tui-place-holder__label">
          {searchString ? `${PlaceHolderStringTypes[type]}  '${searchString}' ` : PlaceHolderStringTypes[type]}
        </div>
      )}
