import React, { forwardRef, PropsWithChildren, useImperativeHandle, useLayoutEffect, useRef, useState } from 'react'
...
function PluginsWithContext<T extends PluginsProps>(
  ...
  const { plugins = [], showNumber, MoreIcon, className = '', style, customClass = '', root, handleVisible } = props
  ...
  const pluginHandleVisible = (data: any) => {
    if (handleVisible) {
      const { width, height } = pluginRef?.current?.children[1].getBoundingClientRect() || {}
  ...
  return (
    ((showPicker && showPicker.length > 0) || (elements && elements?.length > 0)) && (
      <ul className={`plugin ${className}`}>
        {showPicker &&
          showPicker?.length > 0 &&
          showPicker.map((Item, index: number) => {
