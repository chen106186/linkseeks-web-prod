...
export function Popup<T extends PopupProps>(props: React.PropsWithChildren<T>): React.ReactElement {
  const { show, close, children, className, style, root, handleVisible } = props
  ...
  useLayoutEffect(() => {
    const io = new IntersectionObserver(
      ([change]) => {
        const { boundingClientRect: changeBoundingClientRec, rootBounds: changeRootBounds, intersectionRatio } = change
    ...
    <>
      {show && (
        ...
        <div
          ...
          tabIndex={0}
          style={style}
          className={`popup ${className} ${isSetPos && 'popup-show'}`}
          ref={popup}
        >
          {children}
        </div>
