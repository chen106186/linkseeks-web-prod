...
function TUIProfileDefaultWithContext<T extends TUIProfileDefaultProps>(
  ...
  const { userInfo, className, update } = props
  ...
  let editListMap: any = [
    ...
    },
    {
      ...
      value: t(
        userInfo?.gender &&
          gender[userInfo?.gender]?.replace(
    ...
    },
    {
      name: 'AllowType',
      value: t(
        `TUIProfile.${allowType[userInfo?.allowType || '']?.replace(
    ...
    <div className={`${className} tui-profile`}>
      <header className="tui-profile-header">
        <Icon
          ...
          type={IconTypes.BACK}
          onClick={() => {
            setTUIProfileShow && setTUIProfileShow(false)
          }}
        />
...
const MemoizedTUIProfileDefault = React.memo(TUIProfileDefaultWithContext) as typeof TUIProfileDefaultWithContext
