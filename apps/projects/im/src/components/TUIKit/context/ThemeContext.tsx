import React, { useState, useContext, createContext, useLayoutEffect, PropsWithChildren } from 'react'
...
function ThemeProvider(props: PropsWithChildren<Partial<ThemeProviderProps>>) {
  const { theme: _theme = 'light', colors: _colors, children } = props

  if (supportedThemes.indexOf(_theme) === -1) {
    throw new Error(
      `Theme ${_theme} is not supported, please check the supported themes list: [${supportedThemes.join(', ')}]`,
    )
  }
