import React, { useState, useContext, useCallback, PropsWithChildren, useEffect } from 'react'
import type { Conversation, Profile, Friend, FriendApplication } from '@tencentcloud/chat'
import { TUIStore, StoreName, type IConversationModel } from '@tencentcloud/chat-uikit-engine'
...
function UIManagerProvider(props: PropsWithChildren<Partial<UIManagerProviderProps>>) {
  const { customClasses, activeConversation, children } = props
  ...
  return <UIManagerContext.Provider value={value}>{children}</UIManagerContext.Provider>
}
...
export { UIManagerProvider, useUIManager, UIManagerContextType }
