...
const ActivePurchase: React.FC<ActiveStoresProps> = (props) => {
  ...
  return (
    <div className="active_stores">
      <div className="active_stores_title">
        <label>{translate('web.resource.mall.huoyuecaigoushang')}</label>
      </div>
      <div className="active_stores_list">
        {shopList &&
          shopList.map((item, index) => (
            <div className="active_stores_list_item" key={`${item.id}_${index}`}>
              <div className="active_stores_logo">
                <ImageBox width={32} height={32} src={item.logo} />
              </div>
              <div className="active_stores_name">
                {renderRank(index + 1)}
                <a href={`${path}/${item.id}`}>{item.name || item.memberName}</a>
              </div>
            </div>
          ))}
      </div>
