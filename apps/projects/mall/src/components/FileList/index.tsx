...
export const FileItem: React.FC<FileItemProps> = (props) => {
  ...
  const renderByFileType = useMemo(() => {
    ...
    const fileType = getFileType(file)
    if (IMAGE_FILE_TYPE.includes(fileType) && imagePreview) {
      return (
        <Image
          width={64}
          height={64}
    ...
    return (
      <div className={styles['file-list-item-main']}>
        ...
        <a className={styles['file-list-item-filename']} href={file} target="_blank">
  ...
  return (
    <div className={cx(styles['file-list-item'], className)} style={style}>
      {renderByFileType}
      {deleteAble && <DeleteOutlined className={styles['file-list-item-btn-del']} />}
    </div>
  )
}
...
const FileList: React.FC<FileListProps> & { Item: typeof FileItem } = (props) => {
  ...
  return (
    <div className={cx(styles['file-list'], className)} style={style}>
      {fileList &&
        fileList.length > 0 &&
