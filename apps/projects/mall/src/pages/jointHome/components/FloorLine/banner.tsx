...
export class FloorBanner extends React.Component<FloorBannerProps, {}> {
  renderComponent = () => {
    const { className, advertList, linkdisable, getPrefixCls, ...others } = this.props
          ...
          {advertList &&
            advertList.map((item) => (
              <div className={styles['floor-line-banner-item']} key={item.id}>
                <span onClick={() => openLink(item.link, linkdisable)} className={!linkdisable ? styles.link : ''}>
                  <img className={styles['floor-line-banner-item-img']} src={item.picUrl} alt={item.name} />
                </span>
              </div>
            ))}
        </Carousel>
