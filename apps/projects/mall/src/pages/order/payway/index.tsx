...
import { DELIVERY_TYPE_LOGISTICS, DELIVERY_TYPE_LOGISTICS_AND_SELF } from '@/types/order'
...
const PayWay: React.FC<PayWayProps> = (props) => {
  ...
  const getCreditState = (info: any): boolean => {
    ...
    //   if (![DELIVERY_TYPE_LOGISTICS, DELIVERY_TYPE_LOGISTICS_AND_SELF].includes(deliveryType)) {
    //     result = false
    //   } else {
    //     result = true
    //   }
  ...
  return visible ? (
    <div className={styles.payway}>
      <div className={styles.common_title}>
        <span>{translate('web.resource.mall.payType')}</span>
      </div>
      {payWayList.map(
        (item, index: number) =>
              ...
              <ul className={styles.payway_pay_list}>
                {item.payChannels.map((childItem, childIndex: number) => (
                  ...
                  >
                    ...
                    {childItem.payChannel === 15 && balanceInfo && (
                      <span>
                        ({translate('web.common.currencySymbol')}
                        {balanceInfo?.availableAmount})
                      </span>
                    )}
                  </li>
                ))}
      ...
      {payWayList && payWayList.length > SHOW_PAYWAY_LENGTH && (
        <div className={cx(styles.more_btn, styles.pad_l_150)} onClick={() => setExpand(!expand)}>
