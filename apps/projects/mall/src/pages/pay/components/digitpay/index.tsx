...
interface DigitPayWayPropsType {
  orderId: number[]
  onChange: Function
  queryParam: any
}

const DigitPayWay: React.FC<DigitPayWayPropsType> = (props) => {
  ...
  return (
    <Spin spinning={pageLoading}>
      <div className={styles.common_title}>
        <span>{translate('web.resource.mall.shuzirenmingbizhifu')}</span>
      </div>
      <div className={styles.wechat_payway}>
        <p className={styles.wechat_payway_title}>{translate('web.resource.mall.shiyongjianhangappsaoyisao')}</p>
        <div className={styles.wechat_payway_imgbox}>
          {payUrl && <img src={payUrl} />}
          {timeOutStatus && (
