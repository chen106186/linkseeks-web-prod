...
const CellItem: React.FC<CellItemProps> = (props: CellItemProps) => {
  ...
  const renderLabel = () => (isTextNode(label) ? <Text className="cell-list-item-label">{label}</Text> : label)

  return (
    <View className={`${'cell-list-item'} ${border && 'cell-list-item__border'}`} onClick={handlePress}>
      <View className="cell-list-item-head" style={customHeadStyle}>
        {icon || customIcon ? (
      ...
      {label && <View className="cell-list-item-label-wrap">{renderLabel()}</View>}
    </View>
  )
}
