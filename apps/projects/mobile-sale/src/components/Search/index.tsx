...
const Search: React.FC<SearchProps> = React.forwardRef((props: SearchProps, ref: any) => {
  ...
  const {
    ...
    showAction,
    background,
    actionText = intl.formatMessage({ id: 'common.handle.confirm', defaultMessage: '取消' }),
    focus,
    editable,
    clearable,
    placeholder = intl.formatMessage({ id: 'common.input.keyword', defaultMessage: '请输入搜索关键词' }),
    leftIcon,
    customLeftIcon,
    customAction,
    searchOnClearAction,
  ...
  return (
    <View
      ...
      onClick={handleClick}
    >
      <View className={classNames('search-content', { 'search-content__round': shape === 'round' })}>
        {!!label && <Text className="search-label">{label}</Text>}
        <View className="search-control">
          {!customLeftIcon ? (
            <Icons name={leftIcon} size={20} color="#C0C4CC" className="search-control-left-icon" />
          ) : (
            customLeftIcon
          )}
          <View
