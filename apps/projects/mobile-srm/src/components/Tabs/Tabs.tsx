...
const Tabs: React.FC<TabsProps> = (props) => {
  ...
  const tabItems = tabList.map((item, idx) => {
    ...
    return (
      <View className={itemCls} id={`tab-${idx}`} key={`tabs-item-${idx}`} onClick={(e) => handleClick(idx, e)}>
        {item.title}
  ...
  return (
    <View className="tabs" style={heightStyle}>
      {scroll ? (
        <ScrollView horizontal className="tabs__header" style={heightStyle} scrollWithAnimation>
          {tabItems}
        </ScrollView>
      ) : (
        <View className="tabs__header">{tabItems}</View>
      )}
      <View className="tabs__body">
        <View className="tabs-transform" style={heightStyle}>
