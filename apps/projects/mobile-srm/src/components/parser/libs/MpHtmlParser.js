  ...
  if (!cfg.ignoreTags[node.name]) {
    ...
    for (var i = 0, len = styles.length; i < len; i++) {
      ...
      if (
        ...
        style += `;${key}:${value}`
      else if (!styleObj[key] || value.includes('import') || !styleObj[key].includes('import')) styleObj[key] = value
    }
    if (node.name == 'img') {
  ...
  while (1) {
    ...
    if (this.section().toLowerCase() == name) {
      ...
      if (name == 'pre') {
        ...
        return (this.i = j)
      } else if (name == 'style') this.CssHandler.getStyle(this.data.substring(j + 1, this.i - 7))
      else if (name == 'title') this.DOM.title = this.data.substring(j + 1, this.i - 7)
      ...
      (function f(ns) {
        for (var i = 0, n; (n = ns[i]); i++) {
          ...
          if (node.c && n.name[0] == 't') {
            n.c = 1
            style +=
              ';display:table-' + (n.name == 'th' || n.name == 'td' ? 'cell' : n.name == 'tr' ? 'row' : 'row-group')
          }
