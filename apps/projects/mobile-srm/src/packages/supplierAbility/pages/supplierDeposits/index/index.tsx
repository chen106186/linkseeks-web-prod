...
import {
  getMemberMobileDepositGradePage,
  getMemberMobileDepositPageConditions,
  GetMemberMobileDepositVerifyPageResponseDetail,
} from '@apps/apis'
...
import { getDepositsRefreshStorage } from '../../../common/utils/pageStockRefreshUtil'
...
type SupplierDepositsIndexRouteParams = {
  /**
   * 内部状态
   */
  innerStatus?: string
...
type MemberItemType = GetMemberMobileDepositVerifyPageResponseDetail & {}

const SupplierDepositsIndex: React.FC = () => {
  const router = useRouter<SupplierDepositsIndexRouteParams>()
  const {
  ...
  const verifyBtnTxtMap: Record<number, string> = useMemo(
    () => ({
      [MEMBER_INNER_STATUS_TO_VERIFY_DEPOSITORY_DETAIL]: '审核',
      [MEMBER_INNER_STATUS_TO_VERIFY_DEPOSITORY_QUALIFICATION]: '审核',
      [MEMBER_INNER_STATUS_TO_INSPECT_DEPOSITORY]: '入库考察',
      [MEMBER_INNER_STATUS_TO_CLASSFIY_DEPOSITORY]: '入库分类',
      [MEMBER_INNER_STATUS_TO_DEPOSIT_GRADE_ONE]: '审核',
      [MEMBER_INNER_STATUS_TO_DEPOSIT_GRADE_TWO]: '审核',
      [MEMBER_INNER_STATUS_TO_COMFIRM_DEPOSITORY]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_DETAIL_NOT_PASSED]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_QUALIFICATION_NOT_PASSED]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_INSPECTION_NOT_PASSED]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_CLASSIFICATION_NOT_PASSED]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_GRADE_ONE_NOT_PASSED]: '审核',
      [MEMBER_INNER_STATUS_DEPOSITORY_GRADE_TWO_NOT_PASSED]: '审核',
    }),
    [],
  )
  ...
  const getPageItems = () => {
    getMemberMobileDepositPageConditions().then((res) => {
      if (res.code === 1000) {
  ...
  useDidShow(() => {
    const listRefresh = getDepositsRefreshStorage()
  ...
  return (
    <PageLayout>
      <View className="supplier-deposits">
        <CustomFilterModal
          renderHeaderComponent={
            <View id="topbar">
              <NavBar title="供应商入库审核" />
        ...
        <View className="supplier-deposits-scroll">
          <ScrollView
            onScrollToLower={handleLoadMore}
            refresherTriggered={refreshing}
            onRefresherRefresh={() => refreshList()}
            className="supplier-deposits-scrollView"
            refresherEnabled
            scrollY
          >
            <View className="supplier-deposits-list">
              {list.map((item) => (
                <View key={item.validateId} className="supplier-deposits-list-item">
                  <MemberCounter
                    data={{
                      name: item.name,
                    ...
                    customRenderFootRight={
                      <Space>
                        {verifyBtnTxtMap[item.innerStatus] ? (
...
export default SupplierDepositsIndex
