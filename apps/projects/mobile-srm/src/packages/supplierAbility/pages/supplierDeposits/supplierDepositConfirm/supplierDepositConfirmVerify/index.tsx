...
import { getMemberMobileDepositConfirmDetail, postMemberMobileDepositConfirm } from '@apps/apis'
...
type SupplierDepositConfirmVerifyRouteParams = {
  /**
   * 会员审核id
   */
  validateId?: string
...
const SupplierDepositConfirmVerify: React.FC = () => {
  const router = useRouter<SupplierDepositConfirmVerifyRouteParams>()
  const {
    params: { validateId },
  } = router
  ...
  const fetchSupplierDepositDetails = () => {
    ...
    getMemberMobileDepositConfirmDetail({
      validateId,
    })
      .then((res) => {
        if (res.code === 1000) {
  ...
  const handleVerifyConfirm = (values: VerifySubmitValueType) => {
    ...
    // 暂无渠道信息
    postMemberMobileDepositConfirm({
...
export default SupplierDepositConfirmVerify
