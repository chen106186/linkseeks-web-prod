...
import { getMemberMobileDepositGradeOneDetail, postMemberMobileDepositGradeOne } from '@apps/apis'
...
type SupplierDepositGradeOneVerifyRouteParams = {
  /**
   * 会员审核id
   */
  validateId?: string
...
const SupplierDepositGradeOneVerify: React.FC = () => {
  const router = useRouter<SupplierDepositGradeOneVerifyRouteParams>()
  const {
    params: { validateId },
  } = router
  ...
  const fetchSupplierDepositDetails = () => {
    ...
    getMemberMobileDepositGradeOneDetail({
      validateId,
    })
      .then((res) => {
        if (res.code === 1000) {
  ...
  const handleVerifyConfirm = (values: VerifySubmitValueType) => {
    ...
    // 暂无渠道信息
    postMemberMobileDepositGradeOne({
...
export default SupplierDepositGradeOneVerify
