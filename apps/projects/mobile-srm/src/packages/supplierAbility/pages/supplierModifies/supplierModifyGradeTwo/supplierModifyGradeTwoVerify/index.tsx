...
import { getMemberMobileModityGradeTwoDetail, postMemberMobileModityGradeTwo } from '@apps/apis'
...
type SupplierModifyGradeTwoVerifyRouteParams = {
  /**
   * 会员审核id
   */
  validateId?: string
...
const SupplierModifyGradeTwoVerify: React.FC = () => {
  const router = useRouter<SupplierModifyGradeTwoVerifyRouteParams>()
  const {
    params: { validateId },
  } = router
  ...
  const fetchSupplierDepositDetails = () => {
    ...
    getMemberMobileModityGradeTwoDetail({
      validateId,
    })
      .then((res) => {
        if (res.code === 1000) {
  ...
  const handleVerifyConfirm = (values: VerifySubmitValueType) => {
    ...
    // 暂无渠道信息
    postMemberMobileModityGradeTwo({
