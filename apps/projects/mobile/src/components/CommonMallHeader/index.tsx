  ...
  const _renderNavBar = () => {
    return (
      <MallNavBar
        ...
        extra={
          isSelf && (
            <Icons name="ArrowDownFill" size={24} onClick={handleMoreLink} color="#5A2A12" className="exchange_icon" />
          )
        }
      />
    )
    ...
    <View className={cx(`common-mall-header`, STYLE_THEME_LIST[styleTheme])} id="mallHeader">
      ...
      <View className="searchContainer">
        <View className="search" onClick={handleSearch}>
          <Icons customStyle={{ margin: '0 8px' }} name="Search" color="#8F7564" size={20} />
      ...
      {isShowCategory ? (
        <View className="categoryContainer">
          ...
          >
            <Icons name="Menu" color={styleTheme === 0 ? '#8F7564' : '#FFFFFF'} size={24} />
          </View>
        </View>
      ) : null}
    </View>
