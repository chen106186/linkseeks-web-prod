...
import './index.scss'
import PopupIndex from '@/packages/basicSetting/pages/webView/popupIndex'
type FunctionItem = {
  /**
   * 标题
   */
...
const SkuPopup = React.forwardRef<SkuPopupRefHandle, SkuPopupProps>((props: SkuPopupProps, ref) => {
  ...
  useEffect(() => {
    if ([1, 2].includes(productInfo?.adoptionType)) {
      getManageContentNoticeFindWithOutContent({ id: productInfo.adoptionAgreementId }).then((res: any) => {
        if (res.code === 1000) {
          setColumnTypeList(res.data.filter((item) => item.id == productInfo.adoptionAgreementId))
        }
      })
    }
  }, [productInfo])
  ...
  const handleDjs = () => {
    console.log('adoptionType', productInfo.adoptionType)
    if ([1, 2].includes(productInfo?.adoptionType)) {
      setVisible(true)
      setSecond(5)
  ...
  return (
    <View>
      <Popup key="skuPopup" visible={visible && !visible1} onClose={handleClosePopup} closeable>
        <View
          className="sku"
              ...
              {!customRenderProductContent ? (
                <>
                  <View className="sku-product-card-extra">
                    ...
                    <Text className="sku-product-card-unit">
                      {productInfo.unitName ? `${productInfo.unitName}` : ''}
                    </Text>
                    {(productInfo.teamNum && (
                      <Label
            ...
            <View className="sku-stock">
              <View className="sku-stock-left">
                <Text className="sku-stock-title">
                  {intl.formatMessage({
                    id: 'commodityMerge.components.skuPopup.quantity',
                    defaultMessage: '购买数量',
                  })}
                </Text>
      ...
      {columnTypeList.length ? (
        <Popup key="xyPopup" visible={visible1} onClose={handleClosePopup1} closeable>
          <View style={{ display: 'flex', flexFlow: 'column' }}>
            <View style={{ overflow: 'auto', height: '60vh' }}>
              <PopupIndex id={columnTypeList[0].id} type="sign" columnType="7"></PopupIndex>
            </View>
            <View className="sku-action-inner">
              <Button
                type="primary"
                ...
                loading={confirmLoading}
                disabled={confirmDisabled || second > 0}
                full
              >
                {intl.formatMessage({
                  id: 'commodityMerge.components.skuPopup.confirm',
