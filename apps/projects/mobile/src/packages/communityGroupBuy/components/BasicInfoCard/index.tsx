...
const BasicInfoCard = (props: BasicInfoCardProps) => {
  const { data, customStyle, customRenderPrice, customRenderFoot } = props

  // const [slogans] = useState(props?.data?.slogan?.split('|') || []);
  ...
  return (
    <View className="basicInfo-card" style={customStyle}>
      <View className="basicInfo-card-body">
        ...
        <Text className="basicInfo-card-name">{data?.name}</Text>
        {data && data.slogan && <Text className="basicInfo-card-describe">{data?.slogan}</Text>}

        {/* {data && data.slogan && (
          slogans.map((item, index) => (
            <Text className='basicInfo-card-describe' key={index}>
              {item}
            </Text>
          ))
        )} */}
        ...
        {data?.sellingPoint && data?.sellingPoint.length > 0 ? (
          <View className="basicInfo-card-tags">
            {data?.sellingPoint.map((item, index) => (
              <View key={index} className="basicInfo-card-tags-item">
                <Text className="basicInfo-card-tags-item-text">{item}</Text>
              </View>
            ))}
          </View>
        ) : null}
      </View>
      {customRenderFoot ? <View className="basicInfo-card-foot">{customRenderFoot?.(data)}</View> : null}
    </View>
  )
}
