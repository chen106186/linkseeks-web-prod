    ...
    <>
      {deliveryType && deliveryType !== DELIVERY_TYPE_ENUM.SELF_PICKUP ? (
        <Bookshelf.Item
          ...
          labelWidth={64}
          content={
            isSelfPickup && pickupPoint ? (
              <View className={styles['pickup-point']}>
                <Icons name="Address" size={12} color="#252D37" />
                <Text className={styles['pickup-point-name']}>{pickupPoint?.pickupPointName}</Text>
              </View>
            ) : (
