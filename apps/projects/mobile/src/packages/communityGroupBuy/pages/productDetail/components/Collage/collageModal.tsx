...
const CollageModal = (props: Iprops) => {
  ...
  const { teamList, teamHasMore, teamLoading, handleLoadMore } = useGetTeamData({
    ...
    if (teamLoading) {
      return (
        <View className={styles.footerLoading}>
          {intl.formatMessage({
            id: 'commodityMerge.stocksSourcing.components.collage.loading',
            defaultMessage: '加载中',
          })}
        </View>
      )
  ...
  return (
    <Overlay position="center" visible={visible}>
      <View className={styles.container}>
        <View className={styles['modal']}>
          <View className={styles['modal-header']}>
            {intl.formatMessage({
              id: 'commodityMerge.stocksSourcing.components.collage.joining',
              defaultMessage: '正在拼团',
            })}
          </View>
          <View className={styles['modal-content']}>
            ...
            >
              {teamList.map((_item) => {
                return <CollageItem key={_item.id} {..._item} onJoin={handleJoin} />
              })}
            </ScrollView>
          </View>
        </View>
