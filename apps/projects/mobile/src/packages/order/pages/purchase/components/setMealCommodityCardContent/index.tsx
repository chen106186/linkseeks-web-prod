...
const SetMealCommodityCardContent: React.FC<Iprops> = (props: Iprops) => {
  const intl = useIntl()
      ...
      <View className={styles['contetnt-warp']}>
        <View className={styles['content-warp']}>
          <View>
            ...
            >
              <Checkbox value={newThisCommodity.skuId} checked={selectCommodity.indexOf(newThisCommodity.skuId) > -1} />
            </Checkbox.Group>
          </View>
          <View className={styles['commodity-image']}>
            ...
            <ImageBox source={newThisCommodity.commodityLogo} width={86} height={86} />
          </View>
          <View className={styles['commodity-brief']}>
            ...
            >
              <View>{newThisCommodity.name}</View>
            </View>
              ...
              <View>
                <Text className={styles['estimate']}>
                  {intl.formatMessage({
                    ...
                    data: fnKeepTwo(newThisCommodity.handPrice || newThisCommodity.estimatePrice),
                  })}
                </Text>
              </View>
            )}
            <View className={styles['card-config-warp']}>
              <View className={styles['product-number']}>
                ...
                <Text className={styles['min-text']}>{` / ${newThisCommodity.unitName}`}</Text>
              ...
              {isMain ? (
                ...
                <View className={styles['min-text']}>{`X${newThisCommodity.count}`}</View>
              )}
            </View>
          </View>
        </View>
    ...
    <>
      {thisCommodity.map((item: any) => {
        if (item.isMain) {
          return (
            <>
              <SwipeAction
                ...
                // onClick={(selectConfig: any) => fnConfig(selectConfig, item)}
          ...
          return fnGetContent(item, item.isMain)
        }
      })}
    </>
  )
...
export default observer(SetMealCommodityCardContent)
