...
const SearchHistory: React.FC<SearchHistoryPropsType> = (props: SearchHistoryPropsType) => {
  ...
  const { searchStore: { searchHistory, initSearchHistoryByStorage, clearSearchHistory } } = useStores()
  const intl = useIntl()
  useEffect(() => {
  ...
  return (
    <View className={styles['container-history']}>
      <View className={styles['header']}>
        <Text className={styles['title']}>{intl.formatMessage({id: 'search.sousuolishi', defaultMessage: '搜索历史'})}</Text>
        <View className={styles['clearHistoryBtn']} onClick={() => handleclearSearchHistory()}>{intl.formatMessage({id: 'search.qingchulishi', defaultMessage: '清除历史'})}</View>
      </View>
      <View className={styles['historyList']}>
        {
          historyList && historyList.map((historyItem) => (
            <View
