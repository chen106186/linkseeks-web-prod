  ...
  return (
    <Popup key="skuPopup" visible={visible} onClose={handleClosePopup} closeable>
      ...
      >
        ...
        <View className="sku-calculate">
          <Text className="sku-stock-quota">
            {intl.formatMessage({id: 'teamLeader.residue',defaultMessage: '剩余',})}
            {currentSku.stockNum || 0}
            {currentSku.unit}
          </Text>
          <Text className="sku-calculate-rebate">
            {intl.formatMessage({id: 'teamLeader.estimaterebate',defaultMessage: '预估返利',})}
            {intl.formatMessage({ id: 'currency', defaultMessage: '¥' })}
            {formatPriceParts(currentSku.reward ?? 0).join('.')}
          </Text>
        </View>
        <View
