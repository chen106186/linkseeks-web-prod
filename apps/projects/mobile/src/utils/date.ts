...
export const getDateDiff = (dateStr: string): string => {
  ...
  if (days <= 0 && hours > 0) {
    return getIntl().formatMessage({id: 'utils.data.hours.ago', defaultMessage: '{{hours}}小时前',hours});
  }
  if (hours <= 0 && minutes > 0) {
    return getIntl().formatMessage({id: 'utils.data.minutes.ago', defaultMessage: '{{minutes}}分钟前',minutes});
  }
  if (seconds < 60) {
    if (seconds <= 0) {
      return getIntl().formatMessage({id: 'utils.data.just',  defaultMessage: '刚刚'});
    }
    return  getIntl().formatMessage({id: 'utils.data.seconds.ago', defaultMessage: '{{seconds}}秒前', seconds});
  }

  if (days === 1) {
    return getIntl().formatMessage({id: 'utils.data.yestoday',  defaultMessage: '昨天'});
  }

  if (days === 2) {
    return getIntl().formatMessage({id: 'utils.data.before.yestoday',  defaultMessage: '前天'});
  }

  if (days > 2 && days < 7) {
    return  getIntl().formatMessage({id: 'utils.data.days.ago',defaultMessage:  '{{days}}天前',days});
  }

  if (days >= 7 && days < 14) {
    return  getIntl().formatMessage({id: 'utils.data.week.ago',defaultMessage: '{{week}}周前',week: 1 });
  }
  if (days >= 14 && days < 21) {
    return  getIntl().formatMessage({id: 'utils.data.week.ago',defaultMessage: '{{week}}周前',week: 2 });
  }
  if (days >= 21 && days < 30) {
    return  getIntl().formatMessage({id: 'utils.data.week.ago',defaultMessage: '{{week}}周前',week: 3 });
  }

  if (months > 0 && months < 12) {
    return getIntl().formatMessage({id: 'utils.data.months.ago', defaultMessage: '{{months}}个月前', months});
  }
...
export const interval = (completeTime: string | number | Date) => {
  ...
  const time = `${days.toString().padStart(2, '0')}${hours
    .toString()
