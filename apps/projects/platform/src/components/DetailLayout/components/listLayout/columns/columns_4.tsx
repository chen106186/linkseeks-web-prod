...
const columns_4 = ({ dataSource, setDataSource, handleDelete, form, handlCollocation, value }) => {
  const intl = getIntl()
  /** 输入  */
        ...
        <>
          ...
          {value === 6 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'transaction_components.shezhizengpin' })}
            </Button>
          )}
          {value === 13 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'transaction_components.shezhihuangoushangpin' })}
            </Button>
          )}
          {value === 15 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'transaction_components.shezhidapeishangpin' })}
            </Button>
          )}
        </>
      ),
