...
const FlowRecords: React.ForwardRefRenderFunction<FlowRecordsRefHandle, IProps> = (props: IProps, ref) => {
  ...
  const intl = useIntl()
  console.log('🚀 ~ file: index.tsx ~ line 96 ~ outerDataSource', outerDataSource)
  ...
  const [radioValue, setRadioValue] = useState<'inner' | 'outer'>(radioValue_)
  ...
  const getInnerList = (params: FetchListParams) => {
    ...
    if (fetchInnerList) {
      ...
      fetchInnerList(params)
        .then((res) => {
          if (res) {
            if (mounted.current) setInnerList(res)
          }
        })
        .finally(() => {
          if (mounted.current) setInnerLoading(false)
        })
    }
  }
  ...
  const options = [
    outerList && outerList.data
      ? {
          label: intl.formatMessage({ id: 'components.waibuzhuangtai' }),
          value: 'outer',
        }
      : null,
    innerList && innerList.data
      ? {
          label: intl.formatMessage({ id: 'components.neibuzhuangtai' }),
          value: 'inner',
        }
      : null,
  ].filter(Boolean) as []
  ...
  return (
    <MellowCard extra={<ButtonSwitch options={options} onChange={handleRadioChange} value={radioValue} />} {...rest}>
      {radioValue === 'outer' ? (
        <PolymericTable
          rowKey={outerRowkey}
