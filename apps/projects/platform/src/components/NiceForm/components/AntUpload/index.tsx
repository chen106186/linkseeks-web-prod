...
import { toArr, isArr, isEqual, mapStyledProps, connect } from '@apps/formily'
import {
  LoadingOutlined,
  PlusOutlined,
  UploadOutlined,
...
const UploadPlaceholder = styled((props) => (
  <div>
    ...
    <div className={'ant-upload-text'}>{intl.formatMessage({ id: 'components.shangchuan' })}</div>
  </div>
))``
    ...
    public render() {
      ...
      if (listType?.indexOf('dragger') > -1) {
        return (
          ...
          >
            ...
            <p className={'ant-upload-text'}>{intl.formatMessage({ id: 'components.tuozhuaishangchuan' })}</p>
          </UploadDragger>
        )
      }
      ...
      return (
        <div>
          {value
            ? value.map((item) => (
                <p key={item.uid} style={{ width: 424, display: 'flex', justifyContent: 'space-between' }}>
          ...
          >
            {!others.readOnly ? (
              <Button style={{ margin: '0 0 10px' }}>
                <UploadOutlined />
                {(locale && locale.uploadText) || intl.formatMessage({ id: 'components.shangchuanwenjian' })}
              </Button>
            ) : null}
          </AntdUpload>
        </div>
