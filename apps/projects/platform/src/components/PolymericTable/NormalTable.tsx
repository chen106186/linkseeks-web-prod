...
const NormalTable: React.ForwardRefRenderFunction<NormalTableRefHandleType, NormalTableProps<{}>> = <T extends {}>(
  ...
  // 表单值，请求用
  const extraParamsRef = useRef<SearchValuesType>(searchFormProps?.initialValues || {})
  ...
  const paginationProps: PaginationProps = pagination
    ? {
        ...
        size: 'small',
        showTotal: () =>
          `${intl.formatMessage({ id: 'components.gong' })} ${totalCount || 0} ${intl.formatMessage({
            id: 'components.tiao',
          })}`,
        ...pagination,
        total: totalCount,
...
const NormalTableForWard = React.forwardRef<NormalTableRefHandleType, NormalTableProps<{}>>(NormalTable)
