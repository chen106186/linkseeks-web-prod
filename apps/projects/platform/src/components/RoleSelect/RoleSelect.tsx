...
function RoleSelect(props: RoleSelectProps) {
  ...
  const handleSubmit = useCallback(() => {
    if (select === null) {
      message.error(translate('web.resource.member.qingxuanzehuiyuan'))
      return
    }
    closeDrawer()
  }, [visible, select])
  ...
  return (
    <>
      ...
      <Drawer
        visible={visible}
        title={title ? title : translate('web.resource.member.xuanzehuiyuan')}
        onClose={closeDrawer}
        width="50vw"
        footer={
          <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
            ...
            <Button onClick={closeDrawer}>{translate('web.common.cancel')}</Button>
          </Space>
        }
      >
        <Form form={form}>
          ...
          <Form.Item name="name">
            <Input.Search
              placeholder={translate('web.resource.member.memberSupperName')}
              onSearch={() => {
