...
import { Spin, Row, Col, Badge, Tooltip } from 'antd'
import { QuestionCircleOutlined } from '@ant-design/icons'
import { useIntl } from '@linkseeks/i18n'
import { history } from '@linkseeks/router-manager'
import {
  getAftersalesReplaceGoodsGetDetailByConsumer,
  GetAftersalesReplaceGoodsGetDetailByConsumerResponse,
  getAftersalesReplaceGoodsPageInnerWorkflowRecord,
  getAftersalesReplaceGoodsPageOuterWorkflowRecord,
  postAftersalesReplaceGoodsConfirmReplaceReceiveGoods,
  postAftersalesReplaceGoodsConfirmReturnDeliveryGoods,
  postAftersalesReplaceGoodsConfirmReturnGoodsReceipt,
} from '@apps/apis'
...
interface DetailInfo extends GetAftersalesReplaceGoodsGetDetailByConsumerResponse {
  fileList: FileData[]
}
...
const DetailInfo: React.FC<DetailInfoProps> = ({
  ...
  const handleReceivedConfirmReturnDeliver = (id): Promise<any> => {
    return postAftersalesReplaceGoodsConfirmReturnDeliveryGoods({
      id,
    }).then((res) => {
      if (res.code === 1000) {
        setTimeout(() => {
  ...
  const handleConfirmReturnBack = (id): Promise<any> => {
    return postAftersalesReplaceGoodsConfirmReturnGoodsReceipt({
      id,
    }).then((res) => {
      if (res.code === 1000) {
        getDetailInfo()
  ...
  // 确认换货收货
  const handleConfirmExchangeReceive = (id): Promise<any> => {
    return postAftersalesReplaceGoodsConfirmReplaceReceiveGoods({
      id,
    }).then((res) => {
      if (res.code === 1000) {
        history.goBack()
