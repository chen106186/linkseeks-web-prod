...
import { Button, Card, Spin, Badge, message, Upload, Empty } from 'antd'
...
import { getAftersalesReplaceGoodsGetDetailByConsumer, postAftersalesReplaceGoodsSave } from '@apps/apis'
import { normalizeFiledata, FileData } from '@/utils'
import { ORDER_TYPE2_POINTS, ORDER_TYPE2_CHANNEL_POINTS } from '@/constants/order'
...
const ExchangeForm: React.FC<BillsFormProps> = ({ id, isEdit = false, orderId, orderType: outerOrderType }) => {
  ...
  const childTableColumn: ColumnType<any>[] = [
    ...
    },
    {
      ...
      dataIndex: 'amount',
      render: (text) => `${!isPointsOrder ? translate('web.common.currencySymbol') : ''} ${text}`,
    },
    {
  ...
  const getDetailInfo = () => {
    ...
    setInfoLoading(true)
    getAftersalesReplaceGoodsGetDetailByConsumer({
      replaceId: id,
    })
      .then((res) => {
        if (res.code === 1000) {
          ...
          setDetailInfo({
            ...
            deliveryAddress: {
              ...
              name: replaceGoodsAddress.receiveUserName,
            },
            // 物流
            shippingAddress:
            ...
            supplierMember: supplierName
              ...
              : [],
            replaceGoodsList: goodsDetailList.map((item) => ({
              ...item,
              extraData: {
  ...
  const handleSubmit = (value) => {
    ...
    postAftersalesReplaceGoodsSave(payload)
      .then((res) => {
        if (res.code === 1000) {
          setUnsaved(false)
          setTimeout(() => {
  ...
  // ArrayTable自定义渲染
  const renderListTableRemove = (index) => {
    return (
      <Button style={{ color: '#ff4d4f' }} type="link" onClick={() => handleRemoveItem(index)}>
  ...
  // ArrayTable自定义渲染
  const renderListTableEmpty = () => {
    return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} style={{ margin: '12px 0' }} />
  }
  ...
  return (
    <Spin spinning={infoLoading}>
      <PageHeaderWrapper
        title={
          !id
            ? intl.formatMessage({ id: 'exchangeApplication.exchangePrSubmit.add', defaultMessage: '新建换货申请单' })
            : isEdit
            ? intl.formatMessage({ id: 'exchangeApplication.exchangePrSubmit.edit', defaultMessage: '编辑换货申请单' })
            : intl.formatMessage({ id: 'exchangeApplication.exchangePrSubmit.check', defaultMessage: '查看换货申请单' })
        }
        extra={
          isEdit || !id
            ? [
