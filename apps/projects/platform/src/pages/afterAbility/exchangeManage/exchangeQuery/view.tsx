 ...
 * @LastEditTime: 2021-11-17 19:51:40
 * @Description: 换货申请单查询
 */
...
import {
  ...
  getAftersalesReplaceGoodsPageItems,
  postAftersalesReplaceGoodsStopReturnGoods,
} from '@apps/apis'
  ...
  const defaultColumns: ColumnType<any>[] = [
    {
      ...
      render: (text, record) => (
        <DetailAuthButton>
          <EyeAuthButton url={`/afterAbility/exchangeManage/exchangeQuery/detail?id=${record.replaceId}`}>
            {text}
          </EyeAuthButton>
        </DetailAuthButton>
      ),
          ...
          {record.innerStatus >= EXCHANGE_INNER_STATUS_CONFIRM_SUCCESS &&
            ...
            record.innerStatus !== EXCHANGE_INNER_STATUS_CONFIRM_FAILED && (
              <AuthButton type="custom" code="stop">
                <Button type="link" onClick={() => handleStop(record.replaceId)}>
