...
import { getSettlementBusinessReconciliationTeamworkConfirmReconciliationList } from '@apps/apis'
...
const ReadyConfirm: React.FC = () => {
  ...
  const loadingTableData = async (params) => {
    ...
    const { data } = await getSettlementBusinessReconciliationTeamworkConfirmReconciliationList(_params)
    return data
  }
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) =>
        AuthUrl('detail') ? (
          <Link
            to={`/balance/businessReconciliationCollaboration/readyConfirm/detail?id=${record.reconciliationId}&no=${text}`}
          >
            {text}
          </Link>
        ) : (
    ...
    },
    {
      ...
      render: (text: any, record: any) =>
        AuthUrl('detail') && (
          <Link
            to={`/balance/businessReconciliationCollaboration/readyConfirm/confirm?id=${record.reconciliationId}&no=${record.reconciliationNo}`}
          >
            {intl.formatMessage({ id: 'balance.queren' })}
          </Link>
        ),
    },
  ]
