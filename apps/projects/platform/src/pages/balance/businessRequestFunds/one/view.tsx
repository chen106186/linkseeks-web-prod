...
import { getSettlementBusinessApplyAmountToAddApplyAmountExamine1List } from '@apps/apis'
...
const One = () => {
  const columns: any[] = [
    {
      ...
      render: (_text, record) => (
        <>
          {AuthUrl('detail') ? (
            <Link to={`/balance/businessRequestFunds/one/detail?id=${record.id}&no=${record.applyNo}`}>{_text}</Link>
          ) : (
            _text
          )}
        </>
    ...
    },
    {
      ...
      render: (_text, record) => (
        <AuthButton type="custom" code="audit">
          <Link to={`/balance/businessRequestFunds/one/detail?id=${record.id}&no=${record.applyNo}`}>
            {intl.formatMessage({ id: 'dealAbility.shenhe', defaultMessage: '审核' })}
          </Link>
        </AuthButton>
      ),
    },
  ...
  return (
    <Table
      columns={columns}
      effects="applyNo"
      fetch={getSettlementBusinessApplyAmountToAddApplyAmountExamine1List}
      schema={schema}
    />
  )
}
