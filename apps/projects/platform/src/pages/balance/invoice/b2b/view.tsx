...
import {
  ...
  getSettlementJobManualTriggeredInvoice,
  getSettlementJobManualTriggeredInvoiceResult,
} from '@apps/apis'
...
const SettlementList: React.FC = () => {
  ...
  /** 查询票据生成定时任务的结果 */
  const checkVoiceTaskResult = async () => {
    try {
      const res = await getSettlementJobManualTriggeredInvoiceResult()
      if (res.code === 1000 && res.data === true) {
        clearInterval(timerRef.current)
        ref.current.reloadCurrent()
        setRefreshLoading(false)
        message.success(translate('web.resource.balance.shujushuaxinchenggong'))
      } else {
        timerRef.current = setTimeout(checkVoiceTaskResult, 10000) // 10秒后再次查询
      }
    } catch (error) {
      console.error('查询失败:', error)
      timerRef.current = setTimeout(checkVoiceTaskResult, 10000) // 错误时继续轮询
    }
  }

  const _handleRefreshInvoice = () => {
              ...
              components={{
                ...
                exportBtn: () => (
                  <Space>
                    ...
                    <AuthButton type="custom" code="refresh">
                      <Button
                        disabled={refreshLoading}
                        loading={refreshLoading}
                        type="primary"
                        onClick={_handleRefreshInvoice}
                      >
                        {translate('web.resource.balance.shuaxinshuju')}
                      </Button>
                    </AuthButton>
