...
const UploadProductLog: React.FC<IProps> = ({ tableRef, visible }) => {
  ...
  const exportErrorLog = (exceptionContent: any[]) => {
    if (exceptionContent) {
      const fileName = `products-error-log-${new Date().getTime()}.txt`
      if (Array.isArray(exceptionContent) && exceptionContent.length > 0) {
        download_txt(
          fileName,
          exceptionContent
            .map((item) => {
              return `${translate('web.resource.commodity.hanghao')}：${item.location}  ${translate(
  ...
  const columns = StandardFormTable.createColumns<GetSupportDatasheetFileLogImportPageResponseDetail>([
    {
      title: translate('web.resource.commodity.daorupici'),
      key: 'batchNo',
    },
    {
  ...
  const fetchDate = async (params) => {
    if (visible) {
      const res = await getSupportDatasheetFileLogImportPage({ ...params, bizType: 2 })
      return res
    } else {
      return {
        data: [],
...
export default UploadProductLog
