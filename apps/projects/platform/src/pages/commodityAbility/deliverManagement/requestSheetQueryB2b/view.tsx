...
import {
  getProductSampleDeliverBuyerPageByB2bManage,
...
const getTableDataList = async (params) => {
  try {
    const res = await getProductSampleDeliverBuyerPageByB2bManage(params)
    if (res.code == 1000) {
      return res.data
    }
      ...
      <Space>
        <AddAuthButton>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => history.push(`/commodityAbility/deliverManagement/requestSheetQueryB2b/add`)}
          >
    ...
    (record: any) => {
      const optionButtonMap = [
        ...
        },
        {
          ...
          handler: () => {
            history.push(`/commodityAbility/deliverManagement/requestSheetQueryB2b/detail?edit=true&id=${record.id}`)
          },
        },
        {
