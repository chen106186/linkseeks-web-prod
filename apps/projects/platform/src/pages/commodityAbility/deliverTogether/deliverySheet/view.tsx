...
import { getProductSampleDeliverVendorPageToConfirm } from '@apps/apis'
...
const DeliverySheetList: React.FC = () => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <DetailAuthButton>
          <Button
            type="link"
            onClick={() => history.push(`/commodityAbility/deliverTogether/deliverySheet/detail?id=${record.id}`)}
          >
            {intl.formatMessage({ id: 'customerAbility.songyang.confirm', defaultMessage: '确认' })}
          </Button>
        </DetailAuthButton>
      ),
    },
  ...
  const fetchListData = async (params) => {
    const { data } = await getProductSampleDeliverVendorPageToConfirm(params)
    return data
  }
