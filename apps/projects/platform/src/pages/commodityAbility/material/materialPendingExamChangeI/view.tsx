...
import {
  getProductMaterielMaterielExamineChangeList1,
  postProductMaterielMaterielExamineChangeBatch1,
} from '@apps/apis'
...
const MaterialQuery = () => {
  ...
  const columns = getColumn({
    detailUrl: '/commodityAbility/material/materialPendingExamChangeI/detail',
    extraColumn: [
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <DetailAuthButton>
                <Link to={`/commodityAbility/material/materialPendingExamChangeI/detail?id=${record.id}`}>
  ...
  const handleBatchSuccess = async () => {
    ...
    setLoading(true)
    try {
      const { data, code } = await postProductMaterielMaterielExamineChangeBatch1({ idList: selectedRowKeys })
      if (code === 1000) {
  ...
  const fetchListData = async (params) => {
    try {
      const { data, code } = await getProductMaterielMaterielExamineChangeList1(params)
      if (code === 1000) {
        return data
      }
      return EMPTY
  ...
  return (
    <PageHeaderWrapper title={intl.formatMessage({ id: 'material.exam.changeI', defaultMessage: '待审核变更一级' })}>
      <Card>
        <StandardTable
          tableProps={{
            rowKey: 'id',
