...
import { getProductMaterielMaterielExamineList1, postProductMaterielMaterielExamineBatch1 } from '@apps/apis'
...
const MaterialQuery = () => {
  ...
  const columns = getColumn({
    detailUrl: '/commodityAbility/material/materialPendingExamI/detail',
    extraColumn: [
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <DetailAuthButton>
                <Link to={`/commodityAbility/material/materialPendingExamI/detail?id=${record.id}`}>
  ...
  const handleBatchSuccess = async () => {
    ...
    setLoading(true)
    try {
      const { data, code } = await postProductMaterielMaterielExamineBatch1({ idList: selectedRowKeys })
      if (code === 1000) {
  ...
  const fetchListData = async (params) => {
    try {
      const { data, code } = await getProductMaterielMaterielExamineList1(params)
      if (code === 1000) {
        return data
      }
      return EMPTY
  ...
  return (
    <PageHeaderWrapper title={intl.formatMessage({ id: 'material.examI.list.title', defaultMessage: '待审核一级' })}>
      <Card>
        <StandardTable
          tableProps={{
            rowKey: 'id',
