...
import { getProductMaterielMaterielExamineList2, postProductMaterielMaterielExamineBatch2 } from '@apps/apis'
...
const MaterialQuery = () => {
  ...
  const columns = getColumn({
    detailUrl: '/commodityAbility/material/materialPendingExamII/detail',
    extraColumn: [
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <DetailAuthButton>
                <Link to={`/commodityAbility/material/materialPendingExamII/detail?id=${record.id}`}>
  ...
  const handleBatchSuccess = async () => {
    ...
    setLoading(true)
    try {
      const { data, code } = await postProductMaterielMaterielExamineBatch2({ idList: selectedRowKeys })
      if (code === 1000) {
  ...
  const fetchListData = async (params) => {
    try {
      const { data, code } = await getProductMaterielMaterielExamineList2(params)
      if (code === 1000) {
        return data
      }
      return EMPTY
  ...
  return (
    <PageHeaderWrapper title={intl.formatMessage({ id: 'material.examII.list.title', defaultMessage: '待审核二级' })}>
      <Card>
        <StandardTable
          tableProps={{
            rowKey: 'id',
