...
import { Button, Table, Input, Select, DatePicker, InputNumber, Form } from 'antd'
...
const FormList = (props: any) => {
  const { fromData, currentRef, fetchdata, Price, form } = props
  ...
  const tabcolumns: any = [
    ...
    },
    {
      ...
      key: 'x',
      render: (_, item, index) => (
        <a style={{ width: 80 }} onClick={() => Delete(item, index)}>
  ...
  const addtable = () => {
    ...
    //   setkeys('');
    // } else {
    let random = String(data.length) + String(Math.random())
    data.push({
  ...
  const handleItemChange = (list) => {
    ...
    //触发验证
    list.map((i, k) => {
      i.payRatio && form.validateFields(['payRatio' + i.id])
      i.payAmount && form.validateFields(['payAmount' + i.id])
    })
  }
  ...
  return (
    <div style={{ width: '100%', overflow: 'auto' }} className={styles.formNoStyle}>
      <Form form={form}>
        <Table
          columns={tabcolumns}
          dataSource={PlanList}
