...
const Levelexamine = () => {
  ...
  const columns: ColumnType<any>[] = [
      ...
      render: (text, record) => (
        <div>
          <EyeAuthButton
            ...
            url={`${
              isCreatExamine ? '/contract/manage/creatExamine/detail' : '/contract/manage/secondaryCreatExamine/detail'
            }?contractId=${record.id}&type=${isCreatExamine ? 'CreatExamineStepOne' : 'CreatExamineStepTwo'}`}
          >
            {text}
          </EyeAuthButton>
    ...
    },
    {
      ...
      render: (text, record) => {
              ...
              <span
                ...
                onClick={() =>
                  history.push(
                    `${
                      isCreatExamine
                        ? '/contract/manage/creatExamine/examine'
                        : '/contract/manage/secondaryCreatExamine/detail'
                    }?contractId=${record.id}&type=${
                      isCreatExamine ? 'CreatExamineStepOne' : 'CreatExamineStepTwo'
                    }&status=examine`,
                  )
                }
              >
        ...
        //                 ? '/contract/manage/creatExamine/examine'
        //                 : '/contract/manage/secondaryCreatExamine/detail'
        //             }?contractId=${record.id}&type=${isCreatExamine ? 'CreatExamineStepOne' : 'CreatExamineStepTwo'}&status=examine`,
        //           )
        //         }
        //       >
        //         {intl.formatMessage({ id: 'contract.shenhe' })}
  ...
  const fetchData = (params?: any) => {
    console.log('待审核合同创建', isCreatExamine ? '(1级)' : '(2级)')
    let api = isCreatExamine
      ? getContractManageCreatePageToBeExamineStepOne
      : getContractManageCreatePageToBeExamineStepTwo
