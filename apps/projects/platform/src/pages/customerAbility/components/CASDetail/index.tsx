...
import {
  ...
  GetMemberCustomerComplaintSubGetRequest,
  GetMemberCustomerComplaintSubGetResponse,
  getMemberCustomerComplaintUpperGet,
  GetMemberCustomerComplaintUpperGetRequest,
  ...
  postMemberCustomerComplaintSubSend,
  postMemberCustomerComplaintUpperSend,
} from '@apps/apis'
...
const TobeEvaluateDetail = () => {
  const { id, lastTypeParams, isCustomer } = usePageStatus()
  const params = useMemo(() => {
    return id ? { id: id.toString() } : null
  }, [id])
  const isCustomerBoolean = useMemo(() => Boolean(Number(isCustomer)), [isCustomer])
  const { initialValue } = isCustomerBoolean
    ? useInitialValue<GetMemberCustomerComplaintSubGetResponse, GetMemberCustomerComplaintSubGetRequest>(
        getMemberCustomerComplaintSubGet,
        params,
      )
    : useInitialValue<GetMemberCustomerComplaintUpperGetResponse, GetMemberCustomerComplaintUpperGetRequest>(
        getMemberCustomerComplaintUpperGet,
        params,
      )
  ...
  const { basicInfo, resultInfo } = useGetDetailCommon({ initialValue }, isCustomerBoolean)
  const intl = useIntl()
  ...
  const handleSubmit = async (value: SubmitData) => {
    ...
    const requestFunction = isCustomerBoolean
      ? postMemberCustomerComplaintSubSend
      : postMemberCustomerComplaintUpperSend
