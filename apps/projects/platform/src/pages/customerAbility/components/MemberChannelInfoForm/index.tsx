...
// import { getMemberCustomerDepositVerifyProvince, getMemberCustomerDepositVerifyCity } from '@apps/apis'
...
const MemberChannelInfoForm: React.ForwardRefRenderFunction<ChannelRefHandle, IProps> = (props, ref) => {
  ...
  const useBusinessEffects = () => {
    ...
    onFieldValueChange$('upperRelationId').subscribe((fieldState) => {
      ...
      // getMemberCustomerDepositVerifyProvince({
    ...
    onFieldValueChange$('areaCodes.*.provinceCode').subscribe(async (fieldState) => {
      ...
      // getMemberCustomerDepositVerifyCity({
  ...
  return (
    <MellowCard
      title={intl.formatMessage({
        id: 'customerAbility.components.MemberChannelInfoForm.title',
        defaultMessage: '渠道信息',
      })}
      bodyStyle={{
        paddingBottom: 0,
