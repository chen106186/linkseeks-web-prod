...
const VerifyModal: React.FC<IProps> = (props: IProps) => {
  ...
  return (
    <Modal
      title={intl.formatMessage({ id: 'customerAbility.components.VerifyModal.title', defaultMessage: '单据审核' })}
      visible={visible}
          ...
          onFieldValueChange$('agree').subscribe((fieldState) => {
            setFieldState('reason', (state) => {
              state.title =
                fieldState.value === 0
                  ? intl.formatMessage({
                      id: 'customerAbility.components.VerifyModal.reason.noPass',
                      defaultMessage: '不通过原因',
                    })
                  : intl.formatMessage({
                      id: 'customerAbility.components.VerifyModal.reason.pass',
                      defaultMessage: '通过原因',
                    })
