...
import { postMemberCustomerDepositClassify, getMemberCustomerDepositClassifyDetail } from '@apps/apis'
...
const CustomerDepositClassifyVerify: React.FC<{}> = () => {
  ...
  const fetchDetail = (): Promise<IRequestSuccess<DetailType>> =>
    getMemberCustomerDepositClassifyDetail({
      validateId,
    })
  ...
  const handleVerifySubmit = (value: VerifyData) => {
    ...
    postMemberCustomerDepositClassify(payload, {
      timeout: 0,
    })
      .then((res) => {
        if (res.code !== 1000) {
...
export default CustomerDepositClassifyVerify
