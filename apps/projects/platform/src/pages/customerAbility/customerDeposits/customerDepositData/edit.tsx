...
import { postMemberCustomerDepositVerify, getMemberCustomerDepositVerifyDetail } from '@apps/apis'
...
const CustomerDepositDataVerify: React.FC<{}> = () => {
  ...
  const fetchDetail = (): Promise<IRequestSuccess<DetailType>> => {
    return new Promise((resolve, reject) => {
      getMemberCustomerDepositVerifyDetail({
        validateId,
      })
        .then((res) => {
  ...
  const handleVerifySubmit = (value: VerifyData) => {
    ...
    postMemberCustomerDepositVerify(payload, {
      timeout: 0,
    })
      .then((res) => {
        if (res.code !== 1000) {
...
export default CustomerDepositDataVerify
