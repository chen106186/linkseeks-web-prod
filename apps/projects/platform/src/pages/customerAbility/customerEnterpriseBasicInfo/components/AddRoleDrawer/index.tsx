...
import {
  getMemberCustomerAbilityInfoMembertypeList,
  getMemberCustomerAbilityInfoRoleList,
  getMemberCustomerAbilityInfoUpperMemberInfo,
} from '@apps/apis'
...
const AddRoleDrawer: React.FC<IProps> = (props: IProps) => {
  ...
  const getMemberCustomertype = (memberId?: number) => {
  ...
  return (
    <Drawer
      title={intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.component.title', defaultMessage: '增加会员角色' })}
      width={600}
      onClose={handleClose}
      visible={visible}
      footer={
      ...
      <NiceForm
        ...
        effects={($, { setFieldState, getFieldValue }) => {
          ...
          onFieldInputChange$('upperMemberId').subscribe((fieldState) => {
            ...
            getMemberCustomertype(+fieldState.value)
              .then((res) => {
          ...
          onFormInit$().subscribe(() => {
            const initMemberType = () => {
              ...
              linkage.loading('memberType')
              getMemberCustomertype().then((res) => {
                linkage.loaded('memberType')
                linkage.enum('memberType', res)
              })
            }
            ...
            if (!GlobalConfig.global.siteInfo.enableMultiTenancy) {
              ...
              getMemberCustomerAbilityInfoUpperMemberInfo().then((res) => {
                if (res.code === 1000) {
