...
const SupplierForm: React.FC<SupplierFormProps> = (props) => {
  const intl = useIntl()
  const { validateId, id, type, title, mode = 'any' } = props
  ...
  const getDetailedInfo = async () => {
    ...
    if (type === OperateType.myself && !id && !validateId) {
      ...
      formActions.setFieldState('basicInfo.NO_SUBMIT_LAYOUT.*(!email)', (state) => {
        state.editable = false
      })
      if (infoRes && infoRes.data?.memberType && infoRes.data?.roleId) {
        fetchLevelOptions(String(infoRes.data.memberType), String(infoRes.data.roleId))
      }
    }
    if (id && validateId) {
  ...
  const updateMemberCustomDetail = (roleId: string) => {
    try {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        const res = await getMemberCustomerAbilitySubPageitemsDetail({ roleId })
        if (res.code === 1000) {
          const { data = [] } = res
          setMemberItems(data)
        }
  ...
  const fetchLevelOptions = (memberType: string, roleId: string) => {
    ...
    onFieldValueChange$('roleId').subscribe((fieldState) => {
      ...
      setTimeout(async () => {
        ...
        if (fieldState.value && memberType) {
          ...
          fetchLevelOptions(memberType, fieldState.value)
        }
      }, 0)
    })
  ...
  return (
    <Spin spinning={infoLoading} className={styles['mian']}>
      <FormDetailContext.Provider value={providerValue}>
        <FormDetailHeader
          title={title}
          hideBack={type === OperateType.myself}
          schema={addSchema(memberItems, type)}
