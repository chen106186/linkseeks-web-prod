...
import {
  getMemberCustomerInvitationSendPage,
  postMemberCustomerInvitationAdd,
  postMemberCustomerInvitationUpdate,
  postMemberCustomerInvitationSend,
  postMemberCustomerInvitationDelete,
  getMemberCustomerAbilitySubPageitemsRole,
  getMemberCustomerInvitationStateList,
  GetMemberCustomerInvitationSendPageResponseDetail,
} from '@apps/apis'
...
const fetchData = async (params: any) => {
  ...
  const res = await getMemberCustomerInvitationSendPage(payload, { ctlType: 'none' })
...
const CustomerInvite: React.FC<[]> = () => {
  ...
  const REGISTER_URL = `${window.location.origin}/user/register`

  const translate = useWebIntl()
  const OPERATE_API_MAP = {
    add: postMemberCustomerInvitationAdd,
    edit: postMemberCustomerInvitationUpdate,
  }
  ...
  const sendInviteCode = async (info) => {
    if (sendState.current) {
      ...
      try {
        const res = await postMemberCustomerInvitationSend({ id: info.id, emailMsg })
  ...
  const handleDel = (id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        const res = await postMemberCustomerInvitationDelete({ id })
        if (res.code === 1000) {
          ref.current.reload()
        }
      },
  ...
  const defaultColumns: ColumnType<GetMemberCustomerInvitationSendPageResponseDetail>[] = [
    {
  ...
  const fetchSelectOptions = async () => {
    const res = await getMemberCustomerInvitationStateList()
    const roleRes = await getMemberCustomerAbilitySubPageitemsRole()
  ...
  const getSeleteDataRoles = async () => {
    const res = await getMemberCustomerAbilitySubPageitemsRole()
    ...
    <PageHeaderWrapper>
      <Card>
        ...
        <ModalForm
          modalTitle={
            operateType === 'edit'
              ? intl.formatMessage({ id: 'customerAbility.import.invite.modal.edit.title' })
              : intl.formatMessage({ id: 'customerAbility.import.invite.modal.add.title' })
          }
          previewPlaceholder=""
                ...
                properties: {
                  ...
                  roleId: {
                    ...
                    'x-rules': [
                      {
                        required: true,
                        message: intl.formatMessage({ id: 'customerAbility.import.invite.supplierRole.required' }),
                      },
                    ],
                    enum: [],
                  },
                  memberName: {
                    ...
                    'x-rules': [
                      {
                        required: true,
                        message: intl.formatMessage({ id: 'customerAbility.import.invite.supplierName.required' }),
                      },
                      {
                        max: 40,
                        message: intl.formatMessage({ id: 'supplier.import.invite.supplierName.max' }),
                      },
                    ],
                  },
                  email: {
                    ...
                    'x-rules': [
                      {
                        required: true,
                        message: intl.formatMessage({ id: 'supplier.import.invite.supplierEmail.required' }),
                      },
                      {
                  ...
                  invitationValidityTime: {
                    ...
                    'x-rules': [
                      {
                        required: true,
                        message: intl.formatMessage({ id: 'supplier.import.invite.inviteDate.required' }),
                      },
                    ],
                    enum: [
                      {
                        label: intl.formatMessage({ id: 'supplier.import.invite.inviteDate.days.3' }),
                        value: 3,
                      },
                      {
                        label: intl.formatMessage({ id: 'supplier.import.invite.inviteDate.days.7' }),
                        value: 7,
                      },
                      {
                        label: intl.formatMessage({ id: 'supplier.import.invite.inviteDate.days.30' }),
                        value: 30,
                      },
                    ],
                  },
