...
import {
  GetMemberCustomerLifecycleWaitSubmitPageResponseDetail,
  getMemberCustomerLifecycleWaitSubmitPage,
} from '@apps/apis'
...
const CustomerAggregateRatingsIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()

  const handleJumpVerify = (record: GetMemberCustomerLifecycleWaitSubmitPageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitSubmitPageResponseDetail>(pathname).concat({
    title: '操作',
    dataIndex: 'actions',
    align: 'center',
    fixed: 'right',
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitSubmitPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerAggregateRatingsIndex
