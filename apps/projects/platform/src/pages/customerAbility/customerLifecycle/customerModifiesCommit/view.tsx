...
import { GetMemberCustomerLifecycleWaitAddPageResponseDetail, getMemberCustomerLifecycleWaitAddPage } from '@apps/apis'
...
const CustomerModifiesCommit: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpFormPage = (record?: GetMemberCustomerLifecycleWaitAddPageResponseDetail) => {
    history.push(!record ? `${pathname}/add` : `${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitAddPageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitAddPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerModifiesCommit
