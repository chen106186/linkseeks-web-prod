...
import {
  GetMemberCustomerLifecycleWaitConfirmPageResponseDetail,
  getMemberCustomerLifecycleWaitConfirmPage,
} from '@apps/apis'
...
const CustomerModifiesConfirmIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberCustomerLifecycleWaitConfirmPageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitConfirmPageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitConfirmPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerModifiesConfirmIndex
