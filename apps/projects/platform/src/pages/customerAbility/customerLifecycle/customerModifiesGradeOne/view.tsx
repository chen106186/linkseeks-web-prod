...
import {
  GetMemberCustomerLifecycleWaitAuditOnePageResponseDetail,
  getMemberCustomerLifecycleWaitAuditOnePage,
} from '@apps/apis'
...
const CustomerModifiesGradeOneIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberCustomerLifecycleWaitAuditOnePageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitAuditOnePageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitAuditOnePage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerModifiesGradeOneIndex
