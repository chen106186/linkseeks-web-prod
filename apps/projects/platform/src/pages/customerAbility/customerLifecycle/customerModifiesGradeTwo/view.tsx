...
import {
  GetMemberCustomerLifecycleWaitAuditTwoPageResponseDetail,
  getMemberCustomerLifecycleWaitAuditTwoPage,
} from '@apps/apis'
...
const CustomerModifiesGradeTwoIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberCustomerLifecycleWaitAuditTwoPageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitAuditTwoPageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitAuditTwoPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerModifiesGradeTwoIndex
