...
import {
  getMemberCustomerLifecycleWaitGradePage,
  GetMemberCustomerLifecycleWaitGradePageResponseDetail,
} from '@apps/apis'
...
const CustomerModifiesRatingIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberCustomerLifecycleWaitGradePageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberCustomerLifecycleWaitGradePageResponseDetail>(pathname).concat({
      ...
      <>
        <EditAuthButton>
          <Button type="link" onClick={() => handleJumpVerify(record)}>
            {translate('web.resource.member.pinfenrenpinfen')}
          </Button>
        </EditAuthButton>
      </>
    ),
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberCustomerLifecycleWaitGradePage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default CustomerModifiesRatingIndex
