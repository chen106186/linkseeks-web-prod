...
import {
  getMemberCustomerAbilityAssignedPage,
  getMemberCustomerAbilityAssignedPageitems,
  postMemberCustomerAbilityAssignedBind,
  GetMemberCustomerAbilityAssignedPageResponseDetail,
} from '@apps/apis'
...
const MemberMaintain: React.FC<[]> = () => {
  ...
  const fetchData = async (params: any) => {
    ...
    const res = await getMemberCustomerAbilityAssignedPage(payload)
  ...
  const defaultColumns: ColumnType<GetMemberCustomerAbilityAssignedPageResponseDetail>[] = [
    {
      ...
      render: (text, record) => (
        <>
          <div>{text}</div>
          {/* <EyeAuthButton
            url={`/customerAbility/manage/maintain/detail?id=${record.memberId}&validateId=${record.validateId}`}
          >
            {record.name}
          </EyeAuthButton> */}
          {record.name}
    ...
    {
      title: intl.formatMessage({
        id: 'customerAbility.management.maintain.query.memberTypeName',
      }),
    ...
    {
      title: `${intl.formatMessage({
        id: 'customerAbility.management.assigned.query.sourceName',
      })}`,
    ...
    {
      title: `${intl.formatMessage({
        id: 'customerAbility.memberQuery.query.defaultColumns.registerTime',
      })}`,
    ...
    {
      title: intl.formatMessage({
        id: 'customerAbility.management.maintain.query.level',
      }),
      dataIndex: 'level',
    ...
    {
      title: intl.formatMessage({
        id: 'customerAbility.management.maintain.query.statusName',
      }),
      dataIndex: 'statusName',
    },
    {
      title: intl.formatMessage({
        id: 'customerAbility.management.maintain.query.outerStatusName',
      }),
    ...
    {
      title: intl.formatMessage({
        id: 'customerAbility.management.maintain.query.innerStatusName',
      }),
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button type="link" onClick={() => handleReceive(record.memberId, record.roleId)}>
          {intl.formatMessage({
            id: 'customerAbility.management.assigned.query.receive',
          })}
        </Button>
      ),
    },
  ...
  const fetchSelectOptions = async () => {
    const res = await getMemberCustomerAbilityAssignedPageitems()
  ...
  const handleReceive = (memberId, roleId) => {
    ...
    postMemberCustomerAbilityAssignedBind(selectDataParam, { ctlType: 'none' })
      .then((res) => {
        if (res.code === 1000) {
          message.success(
            intl.formatMessage({
              id: 'customerAbility.management.assigned.query.bind.suc',
            }),
          )
          setTimeout(() => {
            ref.current.reload()
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({
        id: 'customerAbility.management.memberPrVerifyComingData.query.verify-tip',
      }),
      onOk() {
  ...
  const ControllerBtns = () => (
    <Space>
      <Button onClick={handleBatch}>
        {intl.formatMessage({
          id: 'customerAbility.management.assigned.query.batchReceive',
        })}
      </Button>
    </Space>
  )
