...
import {
  getMemberCustomerAbilitySubGet,
  getMemberCustomerAbilitySubPageitemsBasic,
  getMemberCustomerAbilitySubPageitemsDetail,
  getMemberCustomerAbilitySubPageitemsLevel,
  getMemberCustomerAbilitySubPageitemsRole,
  getMemberMainpageDetailGet,
  postMemberCustomerAbilitySubAdd,
  postMemberCustomerAbilitySubUpdate,
...
const MemberForm: React.FC<MemberFormProps> = ({ id, validateId, mode, isEdit = false }) => {
  ...
  const getDetailedInfo = async () => {
    ...
    if (id && validateId) {
      infoRes = await getMemberCustomerAbilitySubGet({
        memberId: `${id}`,
    ...
    // 获取渠道信息
    // const channelRes = await getMemberCustomerAbilitySubPageitemsChannel({
  ...
  const handleSubmit = (values: any) => {
    ...
    if (mode === 'myself') {
      ...
      const msg = message.loading({
        content: intl.formatMessage({ id: 'customerAbility.management.import.query.form.saving' }),
        duration: 0,
      })
      postMemberMainpageDetailUpdate({
        email,
        ...
        .then(({ code }) => {
          ...
          authService.setAuth({
            ...
            validateMsg: null,
            validateStatus: 1,
            validateStatusDesc: intl.formatMessage({ id: 'customerAbility.status.notAaudit' }), // 待审核
          } as any)
          setUnsaved(false)
          setTimeout(() => {
    ...
    if (!id && isEdit) {
      setSubmitLoading(true)
      const msg = message.loading({
        content: intl.formatMessage({ id: 'customerAbility.management.import.query.form.creating' }),
        duration: 0,
      })
      postMemberCustomerAbilitySubAdd(payload, {
        timeout: 0,
      })
        .then((res) => {
          if (res.code !== 1000) {
  ...
  const getPageitemsBasic = async () => {
    const res = await getMemberCustomerAbilitySubPageitemsBasic()
  ...
  const useBusinessEffects = ($: IFormExtendsEffectSelector<any, ISchemaFormActions | ISchemaFormAsyncActions>) => {
    ...
    onFieldValueChange$('*(memberType,roleId)').subscribe((fieldState) => {
      ...
      setTimeout(async () => {
        const otherValue = await formActions.getFieldState(otherName, (state) => state.value)
    ...
    onFieldValueChange$('upperRelationId').subscribe((fieldState) => {
      ...
      // getMemberCustomerAbilitySubPageitemsProvince({
    ...
    onFieldValueChange$('areas.*.provinceCode').subscribe((fieldState) => {
      ...
      // getMemberCustomerAbilitySubPageitemsCity({
            ...
            },
            {
              validator: (value) => {
                return value?.length !== current.phoneLength
                  ? intl.formatMessage({
                      id: 'customerAbility.management.import.query.form.basic.phone.max',
  ...
  return (
    <Spin spinning={infoLoading}>
      <PageHeaderWrapper
        title={
          !id && mode === 'any'
            ? intl.formatMessage({ id: 'customerAbility.management.import.query.form.create' })
            : intl.formatMessage({ id: 'customerAbility.management.import.query.form.edit' })
        }
        extra={[
          <Button
            key="1"
