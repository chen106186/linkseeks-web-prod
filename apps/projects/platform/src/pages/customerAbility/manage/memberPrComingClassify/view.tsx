...
import { getMemberDepositClassifyPage, getMemberDepositPageConditions } from '@apps/apis'
...
const MemberPrComingClassify: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrComingClassify/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/customerAbility/manage/memberPrComingClassify/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberDepositClassifyPage(payload)
