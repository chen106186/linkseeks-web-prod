...
import { getMemberDepositInspectPage, getMemberDepositPageConditions } from '@apps/apis'
...
const MemberPrComingInvestigate: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrComingInvestigate/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/customerAbility/manage/memberPrComingInvestigate/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberDepositInspectPage(payload)
