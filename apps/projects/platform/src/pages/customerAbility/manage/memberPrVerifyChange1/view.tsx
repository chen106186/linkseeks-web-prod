...
import { getMemberModifyGradeOnePage, getMemberModifyPageConditions, postMemberModifyGradeOneBatch } from '@apps/apis'
...
const MemberPrVerifyChange1: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrVerifyChange1/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyChangeColumn('/customerAbility/manage/memberPrVerifyChange1/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberModifyGradeOnePage(payload)
