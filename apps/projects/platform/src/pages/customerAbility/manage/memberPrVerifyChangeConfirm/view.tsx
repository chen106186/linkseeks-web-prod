...
import { getMemberModifyConfirmPage, getMemberModifyPageConditions, postMemberModifyConfirmBatch } from '@apps/apis'
...
const MemberPrVerifyChangeConfrim: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrVerifyChangeConfirm/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyChangeColumn('/customerAbility/manage/memberPrVerifyChangeConfirm/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberModifyConfirmPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyChangeConfirm.query.verify-tip' }),
      onOk() {
