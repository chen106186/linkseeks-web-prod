...
import { getMemberDepositConfirmPage, getMemberDepositPageConditions, postMemberDepositConfirmBatch } from '@apps/apis'
...
const MemberPrVerifyComingConfirm: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrVerifyComingConfirm/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/customerAbility/manage/memberPrVerifyComingConfirm/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberDepositConfirmPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingConfirm.query.verify-tip' }),
      onOk() {
