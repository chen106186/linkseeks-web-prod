 ...
 * @LastEditTime: 2021-12-02 16:58:23
 * @Description: 审核入库资料
 */
...
import { postMemberDepositVerify, getMemberDepositVerifyDetail } from '@apps/apis'
...
const MemberPrVerifyComingDataVerify: React.FC<{}> = () => {
  ...
  const handleVerifySubmit = (value: VerifyData) => {
    ...
    const msg = message.loading({
      content: intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingData.verify.commiting' }),
      duration: 0,
    })
    postMemberDepositVerify(payload, {
      timeout: 0,
    })
      .then((res) => {
        if (res.code !== 1000) {
  ...
  const handleVerify = () => {
    const promises = [
      channelFormRef?.current?.validate().catch(() => {
        message.warning(
          intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingData.verify.channel.required' }),
        )
        return Promise.reject()
      }),
      depositFormRef?.current?.validate().catch(() => {
        message.warning(
          intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingData.verify.deposit.required' }),
        )
        return Promise.reject()
      }),
      qualitiesFormRef?.current?.validate().catch(() => {
        message.warning(
          intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingData.verify.qualities.required' }),
        )
        return Promise.reject()
      }),
    ]
...
export default MemberPrVerifyComingDataVerify
