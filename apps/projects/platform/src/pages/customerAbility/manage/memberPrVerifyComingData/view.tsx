 ...
 * @LastEditTime: 2021-12-02 15:54:43
 * @Description: 待审核入库资料
 */
...
import { getMemberDepositVerifyPage, postMemberDepositVerifyBatch, getMemberDepositPageConditions } from '@apps/apis'
...
const MemberPrVerifyComingData: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrVerifyComingData/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/customerAbility/manage/memberPrVerifyComingData/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberDepositVerifyPage(payload)
...
export default MemberPrVerifyComingData
