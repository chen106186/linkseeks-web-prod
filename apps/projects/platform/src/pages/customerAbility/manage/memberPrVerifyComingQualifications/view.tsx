...
import { getMemberDepositPageConditions, getMemberDepositQualifyPage, postMemberDepositQualifyBatch } from '@apps/apis'
...
const memberPrVerifyComingQualifications: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/customerAbility/manage/memberPrVerifyComingQualifications/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/customerAbility/manage/memberPrVerifyComingQualifications/detail').concat(
    [
      {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberDepositQualifyPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'customerAbility.management.memberPrVerifyComingData.query.verify-tip' }),
      onOk() {
  ...
  const fetchSearchItems = async () => {
    const res = await getMemberDepositPageConditions()
...
export default memberPrVerifyComingQualifications
