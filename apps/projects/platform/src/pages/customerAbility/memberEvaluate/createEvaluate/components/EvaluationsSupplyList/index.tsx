...
interface EvaluationsSupplyListProps {
  /**
   * 数据，待定
   */
  data: any,
...
const EvaluationsSupplyList: React.FC<EvaluationsSupplyListProps> = (props) => {
  const { data } = props;
  ...
  return (
    <PolymericTable
      rowKey="id"
      columns={columns}
      fetchDataSource={(params) => fetchList(params)}
      defaultPageSize={5}
    />
  );
};

export default EvaluationsSupplyList;
