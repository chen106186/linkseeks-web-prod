...
import {
  getMemberCustomerAppraisalWaitPublishPage,
  ...
  postMemberCustomerAppraisalDelete,
} from '@apps/apis'
...
const defaultColumns = setColumnsByLinks(
  {
    detail: '/customerAbility/memberEvaluate/createEvaluate/detail',
  },
  ['totalScore'],
)
interface Iprops {}
...
const List: React.FC<Iprops> = (props: Iprops) => {
    ...
    [
      {
        ...
        render: (_text, _record) => (
          <Space>
            {_record.publish && (
              <AuthButton type="custom" code="public">
                <Popconfirm
                  placement="top"
                  title={intl.formatMessage({
            ...
            {_record.updateOrDel && (
              <>
                <EditAuthButton>
                  <Link to={`/customerAbility/memberEvaluate/createEvaluate/edit?id=${_record.id}`}>
                    {intl.formatMessage({
  ...
  const controllerBtns = () => (
    <Space>
      <AddAuthButton>
        <Link to={'/customerAbility/memberEvaluate/createEvaluate/add'}>
          <Button type="primary" icon={<PlusOutlined />}>
            {intl.formatMessage({ id: 'member.memberInspection.index.new' })}
          </Button>
        </Link>
      </AddAuthButton>
      {/* <AuthButton btnCode="customerCreateEvaluate.batchdel">
        <Button
      ...
      <AuthButton btnCode="customerCreateEvaluate.batchPublic">
        <Button
          loading={submitLoading}
  ...
  const handleFetch = async (params: Partial<GetMemberAppraisalWaitPublishPageRequest>) => {
    const result = fetchListData<
      ...
      GetMemberAppraisalWaitPublishPageResponse
    >(getMemberCustomerAppraisalWaitPublishPage, params)
    return result
  }
  ...
  const handlePublic = useCallback(
    async (ids: number[]) => {
      ...
      //   const { code, data } = await postMemberCustomerAppraisalWaitPublishPublish({ ids: ids });
      //   if (code === 1000) {
  ...
  const handleBatchRemove = useCallback(
    async (ids: number[]) => {
      try {
        setSubmitLoading(() => true)
        const { code, data } = await postMemberCustomerAppraisalDelete({ idList: ids })
        if (code === 1000) {
          ref.current?.submit()
        }
      } catch (error) {
