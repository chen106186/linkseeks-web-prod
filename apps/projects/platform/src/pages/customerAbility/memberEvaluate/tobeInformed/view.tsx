...
import {
  getMemberCustomerAppraisalWaitNotificationPage,
  GetMemberAppraisalWaitNotificationPageRequest,
  GetMemberAppraisalWaitNotificationPageResponseDetail,
  postMemberCustomerAppraisalWaitNotificationNotification,
} from '@apps/apis'
...
const defaultColumns = setColumnsByLinks({
  detail: '/customerAbility/memberEvaluate/tobeInformed/detail',
})
...
const List: React.FC<Iprops> = (props: Iprops) => {
    ...
    [
      {
        ...
        render: (_text, _record) => (
          <DetailAuthButton>
            <Space>
              <Link to={`/customerAbility/memberEvaluate/tobeInformed/detail?id=${_record.id}`}>
                {intl.formatMessage({
    ...
    async (ids: number[]) => {
      if (ids.length) {
        try {
          ...
          const { code, data } = await postMemberCustomerAppraisalWaitNotificationNotification({ idList: ids })
          if (code === 1000) {
  ...
  const handleFetch = useCallback(async (params: GetMemberAppraisalWaitNotificationPageRequest) => {
    const result = fetchListData(getMemberCustomerAppraisalWaitNotificationPage, params)
    return result
  }, [])
