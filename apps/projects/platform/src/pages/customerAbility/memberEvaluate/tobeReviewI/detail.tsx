...
import {
  getMemberCustomerAppraisalWaitAuditOneGet,
  GetMemberAppraisalWaitAuditOneGetResponse,
  postMemberCustomerAppraisalWaitAuditOneAudit,
} from '@apps/apis'
...
const TobeEvaluateDetail = () => {
  ...
  const onSubmit = async (values: SubmitDataTypes) => {
    const { data, code } = await postMemberCustomerAppraisalWaitAuditOneAudit({
      id: +id,
      agree: values.status,
