...
import {
  getMemberCustomerAppraisalWaitAuditOnePage,
  ...
  postMemberCustomerAppraisalWaitAuditOneAuditBatch,
} from '@apps/apis'
...
const defaultColumns = setColumnsByLinks({
  detail: '/customerAbility/memberEvaluate/tobeReviewI/detail',
})
...
const List: React.FC<Iprops> = (props: Iprops) => {
  ...
  const intl = useIntl()
  const { columns, fetchStatusOptions } = useEvaluateColumn<GetMemberAppraisalWaitAuditOnePageResponseDetail>(
    defaultColumns,
    [
      {
        ...
        render: (_text, _record) => (
          <DetailAuthButton>
            <Space>
              <Link to={`/customerAbility/memberEvaluate/tobeReviewI/detail?id=${_record.id}`}>
                {intl.formatMessage({
    ...
    async (ids: number[]) => {
      if (ids.length) {
        try {
          ...
          const { code, data } = await postMemberCustomerAppraisalWaitAuditOneAuditBatch({ idList: ids })
          if (code === 1000) {
  ...
  const handleFetch = useCallback(async (params: GetMemberAppraisalWaitAuditOnePageRequest) => {
    const result = fetchListData(getMemberCustomerAppraisalWaitAuditOnePage, params)
    return result
  }, [])
