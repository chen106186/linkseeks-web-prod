...
import {
  getMemberCustomerAppraisalWaitAuditTwoGet,
  GetMemberAppraisalWaitAuditTwoGetResponse,
  postMemberCustomerAppraisalWaitAuditTwoAudit,
} from '@apps/apis'
import { history } from '@linkseeks/router-manager'
import { useIntl } from '@linkseeks/i18n'
import { authUrl } from '@apps/domains'
import { AuthButton, EditAuthButton, AddAuthButton, DetailAuthButton } from '@apps/components'
import createSchema from '../createEvaluate/components/MemberEvaluationCreationForm/schema'
import CustomerAssessmentProject from '../createEvaluate/components/CustomerAssessmentProject'
...
const TobeEvaluateDetail = () => {
  ...
  const { loading, initialValue } = useInitialValue<GetMemberAppraisalWaitAuditTwoGetResponse, { id: string }>(
    getMemberCustomerAppraisalWaitAuditTwoGet,
    params,
  )
  ...
  const projectSchema = createSchema(false)?.properties?.ASSESSMENT_PROJECT?.properties?.assessmentProject

  const onSubmit = async (values: SubmitDataTypes) => {
    const { data, code } = await postMemberCustomerAppraisalWaitAuditTwoAudit({
      id: +id,
      agree: values.status,
  ...
  return (
    <Spin spinning={loading}>
      ...
      >
        ...
        <CustomerAssessmentProject data={initialValue?.items} />
        <div style={{ margin: `${theme['@margin-md']} 0` }}>
          <CustomizeColumn
            id="result"
