...
import {
  getMemberCustomerAppraisalWaitAuditTwoPage,
  GetMemberAppraisalWaitAuditTwoPageRequest,
  GetMemberAppraisalWaitAuditTwoPageResponseDetail,
  postMemberCustomerAppraisalWaitAuditTwoAuditBatch,
} from '@apps/apis'
...
const defaultColumns = setColumnsByLinks({
  detail: '/customerAbility/memberEvaluate/tobeReviewII/detail',
})
...
const List: React.FC<Iprops> = (props: Iprops) => {
    ...
    [
      {
        ...
        render: (_text, _record) => (
          <DetailAuthButton>
            <Space>
              <Link to={`/customerAbility/memberEvaluate/tobeReviewII/detail?id=${_record.id}`}>
                {intl.formatMessage({
  ...
  const handleBatchUpdate = async (ids: number[]) => {
    if (ids.length) {
      try {
        ...
        const { code, data } = await postMemberCustomerAppraisalWaitAuditTwoAuditBatch({ idList: ids })
        if (code === 1000) {
  ...
  const handleFetch = async (params: GetMemberAppraisalWaitAuditTwoPageRequest) => {
    const result = fetchListData(getMemberCustomerAppraisalWaitAuditTwoPage, params)
    return result
  }
