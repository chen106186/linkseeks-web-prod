...
import {
  getMemberCustomerAppraisalWaitSubmitGet,
  GetMemberAppraisalWaitSubmitGetResponse,
  postMemberCustomerAppraisalWaitSubmitSubmit,
} from '@apps/apis'
...
} from '../createEvaluate/components/CustomerAssessmentProjectForm'
...
const TobeEvaluateDetail = () => {
  ...
  const { loading, initialValue, setInitialValue, refresh } = useInitialValue<
    GetMemberAppraisalWaitSubmitGetResponse,
    { id: string }
  >(getMemberCustomerAppraisalWaitSubmitGet, params)
  ...
  const assessmentProjectFormRef = useRef<CustomerAssessmentProjectFormRef>()
  ...
  const resultOnSubmit = async (value: SubmitData) => {
    try {
      ...
      const { code, data } = await postMemberCustomerAppraisalWaitSubmitSubmit(postData as any)
      if (code === 1000) {
        resultToggle(false)
        history.goBack()
      }
