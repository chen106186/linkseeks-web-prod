/*
 * @Description: 客户评分模板 > 客户评分模板配置 > 新增客户评分模板
 */
...
import {
  ...
  getMemberCustomerScoringTemplateTypeList,
  postMemberCustomerScoringTemplateAdd,
  postMemberCustomerScoringTemplateUpdate,
  PostMemberCustomerScoringTemplateAddRequest,
  PostMemberCustomerScoringTemplateUpdateRequest,
} from '@apps/apis'
...
const MemberEvaluationTemplateEdition: React.FC<{}> = (props) => {
  const { pathname } = useLocation()

  const backLink = pathname.split('/').slice(0, -1).join('/') /** 返回到 客户评分模板配置 页 */
  ...
  /** 新增 评分模板 */
  const postEvaluationTemplateCreation = async (params: PostMemberCustomerScoringTemplateAddRequest) => {
    const res = await postMemberCustomerScoringTemplateAdd(params, { ctlType: 'none' })
    if (res.code === 1000) {
      message.success(`${translate('web.resource.member.yichenggongxinzengpinfenmuban')}：${params.templateName}`)
      history.redirect(backLink)
    }
  }

  /** 编辑 评分模板 */
  const postEvaluationTemplateEdition = async (params: PostMemberCustomerScoringTemplateUpdateRequest) => {
    const res = await postMemberCustomerScoringTemplateUpdate(params, { ctlType: 'none' })
    if (res.code === 1000) {
  ...
  const getEvaluationTemplateDetail = async (templateId: string) => {
    try {
      setIsSpinning(true)
      const res = await getMemberCustomerScoringTemplateDetail({ templateId })
      if (res.code === 1000) {
