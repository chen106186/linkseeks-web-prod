...
import {
  ...
  postMemberCustomerInspectAdd,
  postMemberCustomerInspectUpdate,
...
const InspectionAdd = () => {
  ...
  useEffect(() => {
    ...
    async function getInitValue() {
      const { data, code } = await getMemberCustomerInspectGet({ id: id.toString() })
      if (code === 1000) {
  ...
  const handleFetchData = useCallback(async (params: GetMemberInspectMembersRequest) => {
    const { data, code } = await postMemberCustomerLifecycleArchivesManagementPage(
  ...
  const handleOnSubmit = async (values: SubmitDataType) => {
    try {
      ...
      const service = isEdit ? postMemberCustomerInspectUpdate : postMemberCustomerInspectAdd
  ...
  return (
    <PageHeaderWrapper
      title={
        isEdit
          ? `${intl.formatMessage({
              id: 'customerAbility.inspection.edit',
              defaultMessage: '修改客户考察',
            })}`
          : `${intl.formatMessage({
              id: 'customerAbility.inspection.add',
              defaultMessage: '新增客户考察',
            })}`
      }
      extra={
          ...
          expressionScope={{
            connectMember: (
              <div onClick={() => toggle(true)}>
                ...
                {intl.formatMessage({
                  id: 'customerAbility.inspection.subMember.select',
                  defaultMessage: '选择客户',
                })}
              </div>
            ),
