...
import {
  getMemberCustomerInspectPage,
  GetMemberInspectPageRequest,
  GetMemberInspectPageResponse,
  getMemberCustomerInspectTypes,
  postMemberCustomerInspectDelete,
} from '@apps/apis'
...
const MemberInspection: React.FC<Iprops> = (props: Iprops) => {
  ...
  const { columns } = useColumns(queryColumns, [
    {
      ...
      render: (text, record, index) => {
        ...
        return (
          <Space>
            <EditAuthButton>
              <Link to={`/customerAbility/memberInspection/edit?id=${record.id}`}>
                {intl.formatMessage({
  ...
  const handleDelete = async (params: { id: number }) => {
    const { data, code } = await postMemberCustomerInspectDelete(params)
    if (code === 1000) {
      ref.current?.submit()
    }
  }
  ...
  const controllerBtns = (
    <div>
      <AddAuthButton>
        <Link to={`/customerAbility/memberInspection/add`}>
  ...
  const fetchListData = async (params: GetMemberInspectPageRequest): Promise<GetMemberInspectPageResponse> => {
    const { data, code } = await getMemberCustomerInspectPage(params)
    if (code === 1000) {
      return data
    }
    return {
  ...
  const fetchInspectType = useCallback(async () => {
    const { code, data } = await getMemberCustomerInspectTypes()
    if (code === 1000) {
