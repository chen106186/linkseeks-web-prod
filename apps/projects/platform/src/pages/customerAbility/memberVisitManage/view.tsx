...
import {
  GetMemberCustomerVisitListResponseDetail,
  postMemberCustomerVisitDelete,
  getMemberCustomerVisitList,
  getMemberCustomerVisitVisitTypeItems,
} from '@apps/apis'
...
type GetMemberCustomerVisitListRequestParams = FetchParamsType & {
  memberName: string
  visitTheme: string
  visitType: number
  visitLevel: number
...
type GetMemberCustomerVisitListRequestResponse = GetMemberCustomerVisitListResponseDetail & {}
...
const MemberVisitManageIndex: React.FC<{}> = (props) => {
  ...
  const handleJumpFormPage = (record?: GetMemberCustomerVisitListRequestResponse) => {
    history.push(
      !record ? '/customerAbility/memberVisitManage/add' : `/customerAbility/memberVisitManage/edit?id=${record.id}`,
    )
  }

  const handleDeleteMemberVisitRecord = (record: GetMemberCustomerVisitListRequestResponse) => {
  ...
  const columns: ColumnType<GetMemberCustomerVisitListRequestResponse>[] = [
    {
    ...
    },
    {
      ...
      render: (text: any, record) => (
        <>
          <EyeAuthButton
            url={`/customerAbility/memberVisitManage/detail?id=${record.id}`}
  ...
  const fetchMemberVisitList = async (params: GetMemberCustomerVisitListRequestParams) => {
    const res = await getMemberCustomerVisitList({
      ...params,
    ...
    <PageHeaderWrapper>
      <Card>
        <PolymericTable
          rowKey="id"
          columns={columns}
          fetchDataSource={(params) => fetchMemberVisitList(params as GetMemberCustomerVisitListRequestParams)}
          defaultPageSize={10}
          searchFormProps={{
            ...
            effects: ($, actions) => {
              ...
              useAsyncInitSelect(['visitType', 'visitLevel'], async () => {
                const { data, code } = await getMemberCustomerVisitVisitTypeItems()
                if (code === 1000) {
                  return {
