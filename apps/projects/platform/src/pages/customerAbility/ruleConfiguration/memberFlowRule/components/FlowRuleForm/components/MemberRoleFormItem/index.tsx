...
import { getMemberCustomerProcessRuleRolePage } from '@apps/apis'
...
const fetchListData = async (params: any) => {
  ...
  const res = await getMemberCustomerProcessRuleRolePage(payload)
  if (res.code === 1000) {
    return res.data
  }
...
const MemberRoleFormItem = (props) => {
  ...
  const columns: ColumnType<ValueType>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'customerAbility.profile.roleName' }),
      dataIndex: 'roleName',
    },
    {
    ...
    <div className={styles.memberRole}>
      ...
      <div className={styles['memberRole-stamp']}>
        <Descriptions column={1}>
          <Descriptions.Item
            label={intl.formatMessage({ id: 'customerAbility.profile.roleName' })}
          ...
          <Descriptions.Item label={translate('web.resource.member.memberType')} labelStyle={{ width: 104 }}>
      ...
      <Drawer
        title={intl.formatMessage({
          id: 'customerAbility.flowRuleForm.memberRole.drawer.title',
          defaultMessage: '选择客户角色',
        })}
        width={1000}
