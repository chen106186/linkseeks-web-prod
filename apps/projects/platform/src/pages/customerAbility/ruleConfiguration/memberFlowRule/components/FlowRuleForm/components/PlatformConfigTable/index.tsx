...
import { getMemberCustomerProcessRuleRoleConfigPage } from '@apps/apis'
...
const PlatformConfigTable = (props: IProps) => {
  ...
  const fetchMemberCustomerProcessRuleRoleConfigPage = async (params: FetchListParams = {}) => {
    if (!roleId) {
      return
    }
    setLoading(true)
    const nextName = params.name !== undefined ? params.name : name
    const res = await getMemberCustomerProcessRuleRoleConfigPage({
      roleId: `${roleId}`,
  ...
  useEffect(() => {
    fetchMemberCustomerProcessRuleRoleConfigPage()
  }, [props.roleId])
  ...
  const handleSearch = (value: string) => {
    fetchMemberCustomerProcessRuleRoleConfigPage({ current: 1, name: value })
  }
