...
import {
  getMemberCustomerProcessRuleGet,
  GetMemberCustomerProcessRuleGetResponse,
  postMemberCustomerProcessRuleAdd,
  postMemberCustomerProcessRuleUpdate,
} from '@apps/apis'
...
type RuleInfoType = Omit<
  GetMemberCustomerProcessRuleGetResponse,
  'roleName' | 'memberTypeName' | 'roleId' | 'roleTypeName'
> & {
  memberRole: {
    roleName: string
...
const FlowRuleForm: React.FC<MemberFormProps> = ({ id, isEdit = false }) => {
  ...
  const getDetailedInfo = async () => {
    ...
    setInfoLoading(true)
    const res = await getMemberCustomerProcessRuleGet({
      id: `${id}`,
    })
  ...
  return (
    <Spin spinning={infoLoading}>
      <PageHeaderWrapper
        title={
          !id
            ? intl.formatMessage({ id: 'customerAbility.ruleConfiguration.flowRule.add.title' })
            : isEdit
            ? intl.formatMessage({ id: 'customerAbility.ruleConfiguration.flowRule.modify.title' })
            : intl.formatMessage({ id: 'customerAbility.ruleConfiguration.flowRule.details.title' })
        }
        extra={[
          isEdit ? (
            <Button
