...
import { getMemberCustomerAbilityLevelRolePage, getMemberCustomerAbilityLevelTypes } from '@apps/apis'
...
import MemberApplicableRoleFormField, {
  MemberApplicableRoleValue,
  MemberApplicableRoleProps,
} from './components/MemberApplicableRoleFormField'
...
const MemberLevelForm: React.FC<MemberLevelFormProps> = (props) => {
  ...
  const fetchMemberApplicableRole = (): MemberApplicableRoleProps['fetchDataSource'] => async (params) => {
    const res = await getMemberCustomerAbilityLevelRolePage({
      ...params,
        ...
        <NiceForm
          ...
          effects={($, actions) => {
            ...
            useAsyncSelect('levelType', async () => {
              const { data, code } = await getMemberCustomerAbilityLevelTypes()
              if (code === 1000) {
