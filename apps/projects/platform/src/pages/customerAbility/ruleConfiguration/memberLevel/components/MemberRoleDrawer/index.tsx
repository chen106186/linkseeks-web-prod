...
const MemberRoleDrawer = (props: MemberRoleDrawerProps) => {
  ...
  const columns: ColumnType<MemberRoleType>[] = [
    {
      title: intl.formatMessage({ id: 'member.memberLevel.roleName.placeholder', defaultMessage: '会员角色名称' }),
      dataIndex: 'roleName',
    },
    {
  ...
  const handleConfirm = () => {
    if (!rowCtl.selectRow.length) {
      message.warning(intl.formatMessage({ id: 'member.memberLevel.role.required', defaultMessage: '请选择会员角色' }))
      return
    }
    if (onSubmit) {
  ...
  return (
    <Drawer
      title={intl.formatMessage({ id: 'member.memberLevel.role.choose', defaultMessage: '选择会员' })}
      width={1000}
      onClose={handleClose}
      visible={visible}
      footer={
