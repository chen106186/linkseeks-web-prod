...
import {
  getTradeNotarizeEnquiryQuotedPriceAuditListTwo,
  getTradeProductInquiryExternalStateEnum,
  getTradeProductInquiryInteriorStateEnum,
  postTradeNotarizeEnquiryQuotedPriceAuditAllTwo,
} from '@apps/apis'
...
const WaitAuditInquiryOne = () => {
  ...
  const fetchSubmitBatch = async () => {
    const res = await postTradeNotarizeEnquiryQuotedPriceAuditAllTwo({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/confirmOffer/waitAuditInquiryTwo/offer?id=${record.id}`}
        >
          {text}
        </EyeAuthButton>
      ),
    },
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/confirmOffer/waitAuditInquiryTwo/inquiry?id=${record.inquiryListId}`}
        >
          {text}
        </EyeAuthButton>
      ),
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <AuthButton type="custom" code="offer/detail">
          <Button
            type="link"
            onClick={() => history.push(`/dealAbility/confirmOffer/waitAuditInquiryTwo/offer/detail?id=${record.id}`)}
          >
  ...
  return (
    <Table
      ...
      columns={columns}
      effects="quotationNo"
      fetch={getTradeNotarizeEnquiryQuotedPriceAuditListTwo}
