...
import {
  getTradeNotarizeEnquiryQuotedPriceAffirmList,
  getTradeProductInquiryExternalStateEnum,
  getTradeProductInquiryInteriorStateEnum,
  postTradeNotarizeEnquiryQuotedPriceAffirmAll,
} from '@apps/apis'
...
const WaitSubmitOffer = () => {
  ...
  const fetchSubmitBatch = async () => {
    const res = await postTradeNotarizeEnquiryQuotedPriceAffirmAll({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/confirmOffer/waitSubmitOffer/offer?id=${record.id}`}
        >
          {text}
        </EyeAuthButton>
      ),
    },
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/confirmOffer/waitSubmitOffer/inquiry?id=${record.inquiryListId}`}
        >
          {text}
        </EyeAuthButton>
      ),
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <AuthButton type="custom" code="submit">
          <Button
            type="link"
            onClick={() => history.push(`/dealAbility/confirmOffer/waitSubmitOffer/offer?id=${record.id}&submit=1`)}
          >
            {intl.formatMessage({ id: 'dealAbility.querenbaojiadan' })}
          </Button>
        </AuthButton>
      ),
    },
  ...
  return (
    <Table
      ...
      columns={columns}
      effects="quotationNo"
      fetch={getTradeNotarizeEnquiryQuotedPriceAffirmList}
      reload={reload}
      selectedRow
          ...
          <Col span={6}>
            <AuthButton type="custom" code="batch">
              <Button disabled={rowkeys.length === 0} onClick={() => fetchSubmitBatch()}>
                {/* 待提交报价单-确认报价 */}
                {intl.formatMessage({ id: 'dealAbility.querenbaojia' })}
              </Button>
            </AuthButton>
          </Col>
        </Row>
