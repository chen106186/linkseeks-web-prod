...
const WaitAuditOfferTwo = () => {
  ...
  const fetchSubmitBatch = async () => {
    const res = await postTradeProductQuotationtAuditAllTwo({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/inquiryOffer/waitAuditOfferTwo/offer/detail?id=${record.id}`}
        >
          {text}
        </EyeAuthButton>
      ),
    },
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/inquiryOffer/waitAuditOfferTwo/inquiry?id=${record.inquiryListId}`}
        >
          {text}
        </EyeAuthButton>
      ),
      ...
      render: (text: any, record: any) => (
        <AuthButton type="custom" code="examine">
          <Button
            ...
            onClick={() => history.push(`/dealAbility/inquiryOffer/waitAuditOfferTwo/offer?id=${record.id}`)}
          >
  ...
  return (
    <Table
      ...
      columns={columns}
      effects="quotationNo"
      fetch={getTradeAuditProductQuotationListTwo}
