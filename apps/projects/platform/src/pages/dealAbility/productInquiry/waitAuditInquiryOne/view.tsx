...
import { getTradeInquiryToAuditList, postTradeInquiryDocumentsReviewAll } from '@apps/apis'
...
const WaitAuditInquiryOne = () => {
  ...
  const fetchSubmitBatch = async () => {
    const res = await postTradeInquiryDocumentsReviewAll({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <EyeAuthButton
          ...
          url={`/dealAbility/productInquiry/waitAuditInquiryOne/detail?id=${record.id}`}
        >
          {text}
        </EyeAuthButton>
      ),
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <AuthButton type="custom" code="examine">
          <Button
            type="link"
            onClick={() => history.push(`/dealAbility/productInquiry/waitAuditInquiryOne/examine?id=${record.id}`)}
          >
  ...
  return (
    <Table
      ...
      fetch={getTradeInquiryToAuditList}
