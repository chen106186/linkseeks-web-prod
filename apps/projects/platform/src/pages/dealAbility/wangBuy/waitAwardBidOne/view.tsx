...
import {
  ...
  postTradeAskPurchaseAwardBidAuditBatch,
} from '@apps/apis'
...
const WaitAwardBidTwo = () => {
  ...
  const handleAuditAgree = () => {
    ...
    postTradeAskPurchaseAwardBidAuditBatch({ idList: selectItems.map((item) => item.id), agree: 1 })
      .then((res) => {
        if (res.code === 1000) {
          tableRef.current.reload()
        }
  ...
  return (
    <Fragment>
      <StandardFormTable
        ...
        searchButtons={[
          {
            children: translate('web.resource.deal.piliangshenhetongguo'),
            onClick() {
              handleAuditAgree()
            },
          },
...
export default WaitAwardBidTwo
