  ...
  const handleSubmit = () => {
    ...
    ref.current
      ...
      .then((values) => {
        ...
        postTradeAskPurchaseQuoteAuditLevel2(obj).then((res) => {
          if (res.code === 1000) {
            setVisible(false)
            setLoading(false)
          ...
          <>
            <AuthButton code="shenhe" type="custom">
              <Button
                ...
                onClick={() => {
                  history.push(
                    `/dealAbility/wangBuyOffer/inquiryOffer/waitAddOffer/detail?id=${record.quoteId}&verify=2`,
                  )
                }}
              >
                {intl.formatMessage({
