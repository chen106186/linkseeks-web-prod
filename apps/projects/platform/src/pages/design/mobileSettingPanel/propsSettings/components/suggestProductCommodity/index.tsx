...
const SuggestProductCommodity: React.FC<SuggestProductCommodityProps> = (props: SuggestProductCommodityProps) => {
  const { id, tags, layoutType } = props
  const { shopId }: any = usePageStatus()
    ...
    if (id && id != record.id) {
      ...
      switch (layoutType) {
        case LAYOUT_TYPE.shop:
          getFn = getMarketingAdornGoodsListAdorn
          break
  ...
  const _handleToDetailPage = (id, belongType) => {
    if (belongType === 1) {
      history.open(`/marketingAbility/selfManagement/search/detail?id=${id}`)
    } else {
      history.open(`/marketingManage/merchantMarketing/merchantMarketingSearch/detail?id=${id}`)
    }
  }
