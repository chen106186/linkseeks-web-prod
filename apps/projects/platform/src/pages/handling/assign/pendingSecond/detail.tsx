...
import { getEnhanceSupplierToBeAddDetails, postEnhanceSupplierToBeSecondExamExam } from '@apps/apis'
...
const Info = () => {
  ...
  const onExamVerifySubmit = async (value: SubmitDataTypes) => {
    const { code, data } = await postEnhanceSupplierToBeSecondExamExam({
      id: +id,
      status: value.status,
      cause: value.reason,
    })
