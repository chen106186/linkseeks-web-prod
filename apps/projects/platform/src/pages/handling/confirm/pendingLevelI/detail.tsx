...
import { getEnhanceProcessToBeFirstExamDetails, postEnhanceProcessToBeFirstExamExam } from '@apps/apis'
...
const Info = () => {
  ...
  const { columns } = useColumnWithFilter(defaultColumns, [
    {
      ...
      render: (text, record) => {
        return (
          <a onClick={() => handleViewDetail(record)}>{intl.formatMessage({ id: 'handling.zhakanjiagongmingxi' })}</a>
        )
      },
    },
  ])
  ...
  const handleSubmit = async (value: SubmitDataTypes) => {
    const { data, code } = await postEnhanceProcessToBeFirstExamExam({
      id: +id,
      status: value.status,
      cause: value.reason,
    })
      ...
      <PageHeaderWrapper
        ...
        extra={
          <Button onClick={() => examToggle(true)} type="primary">
            {intl.formatMessage({ id: 'handling.detail.otherRequire' })}
          </Button>
        }
      >
