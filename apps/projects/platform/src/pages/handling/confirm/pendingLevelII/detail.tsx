...
import { getEnhanceProcessToBeSecondExamDetails, postEnhanceProcessToBeSecondExamExam } from '@apps/apis'
...
const Info = () => {
  ...
  const handleSubmit = async (value: SubmitDataTypes) => {
    const { data, code } = await postEnhanceProcessToBeSecondExamExam({
      id: +id,
      status: value.status,
      cause: value.reason,
    })
      ...
      <PageHeaderWrapper
        ...
        extra={
          <Button onClick={() => examToggle(true)} type="primary">
            {intl.formatMessage({ id: 'handling.shenhe' })}
          </Button>
        }
      >
