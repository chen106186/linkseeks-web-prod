...
import { getLogisticsOrderWaitConfirmGet } from '@apps/apis'
...
const LogisticsBillManageDetail = () => {
    ...
    setBasicEffect([
      ...
      },
      {
        col: [
          ...
          { label: intl.formatMessage({ id: 'logistics.fahuofang' }), extra: data.shipperMemberName },
    ...
    setFreightEffect([
      {
        col: [
          ...
          },
          {
            ...
            extra: `${translate('web.common.currencySymbol')}${
              data.freightPrice ? Number(data.freightPrice).toFixed(2) : '-'
            }`,
          },
          {
  ...
  const fetchData = useCallback(async () => {
    await getLogisticsOrderWaitConfirmGet({ id, code })
      .then((res) => {
        if (res.code !== 1000) {
          return
        }
  ...
  const handleConfirm = (value) => {
    ...
    console.log(params.freightPrice)
  ...
  const handleSubmit = () => {
    ...
    if (!taxRate && !freightPrice) {
      message.warning(intl.formatMessage({ id: 'logistics.qingxianbianjiyunfei' }))
      return
    }
    setVisible(true)
  }
  ...
  return (
    <Context.Provider value={dataSource}>
      <PageHeaderWrapper
        subTitle={dataSource.logisticsOrderNo}
          ...
          <>
            {path === 'detail' && (
              <Button type="primary" onClick={handleSubmit}>
                <CheckCircleOutlined />
                {intl.formatMessage({ id: 'logistics.danjuqueren' })}
              </Button>
            )}
          </>
        }
        ...
        <Fragment>
          ...
          <FreightLayout
            ...
            extra={
              <>
                {path === 'detail' && (
                  <Button type="link" onClick={() => setFreightVisible(true)}>
      ...
      />
      <BillSubmit dataSource={dataSource} visible={visible} onClose={() => setVisible(false)} />
    </Context.Provider>
  )
}
