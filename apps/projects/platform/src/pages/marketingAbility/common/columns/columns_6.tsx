...
const columns_6 = ({ dataSource, setDataSource, handleDelete, form }) => {
  return [
    ...
    {
      title: (
        <Tooltip placement="top" title={intl.formatMessage({ id: 'marketingAbility.negotiateStarting' })}>
          {intl.formatMessage({ id: 'marketingAbility.theStartingPrice' })}
    ...
    },
    {
      title: (
        <Tooltip
          placement="top"
          title={intl.formatMessage({ id: 'marketingAbility.processBargainingBargainingCannotExceedBargain' })}
        >
          {intl.formatMessage({ id: 'marketingAbility.bargainPrice' })}
          <QuestionCircleOutlined />
        </Tooltip>
      ),
      key: 'activityPrice',
