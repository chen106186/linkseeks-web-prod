    ...
    },
    {
      ...
      render: (text: any, record: any, index: number) => (
        <Form.Item
          style={{ margin: 0 }}
          initialValue={text}
          key={`startPrice_${record.id}`}
          name={`startPrice_${record.id}`}
          rules={[{ required: true, message: `${intl.formatMessage({ id: 'marketingAbility.qingshurujine' })}` }]}
        >
          <Input
            value={record.startPrice}
            onChange={(e) => {
              _changePrice(record, e.target.value, 'startPrice')
            }}
    ...
    },
    {
      ...
      render: (text: any, record: any, index: number) => (
        <Form.Item
          style={{ margin: 0 }}
          initialValue={text}
          key={`minPrice_${record.id}`}
          name={`minPrice_${record.id}`}
          rules={[{ required: true, message: `${intl.formatMessage({ id: 'marketingAbility.qingshurujine' })}` }]}
        >
          <Input
            value={record.minPrice}
            onChange={(e) => {
              _changePrice(record, e.target.value, 'minPrice')
            }}
            placeholder={intl.formatMessage({ id: 'marketingAbility.qingshurujine' })}
          />
        </Form.Item>
