...
const columns = (props: any) => {
  ...
  return [
    ...
    {
      title: (
        <Tooltip placement="top" title={intl.formatMessage({ id: 'marketingAbility.zhijiangjiageweishangpinjia' })}>
      ...
      render: (text: any, record: any, index: number) => (
        <Form.Item
          style={{ margin: 0 }}
          initialValue={text}
          // label={intl.formatMessage({ id: 'marketingAbility.huodongjiage' })}
          key={`priceReduction_${record.id}`}
          name={`priceReduction_${record.id}`}
          rules={[
            { required: true, message: `${intl.formatMessage({ id: 'marketingAbility.qingshuruhuodongjiage' })}` },
          ]}
        >
          <Input
            value={record.priceReduction}
            onChange={(e) => {
    ...
    },
    {
      ...
      render: (text: any, record: any, index: number) => (
        ...
        >
          {intl.formatMessage({ id: 'marketingAbility.shanchu' })}
        </Button>
      ),
    },
  ]
