...
const columns = (props: any) => {
  ...
  const _changeActivityPrice = (record: any, value: any) => {
    ...
    _item.activityPrice = _val * (Number(_val) / 10)
  ...
  return [
    ...
    },
    {
      ...
      key: 'price',
      dataIndex: 'price',
      render: (text: any, record: any, index: number) => `${translate('web.common.currencySymbol')}${text}`,
    },
    {
      title: (
        <Tooltip
          ...
          title={intl.formatMessage({
            id: 'marketingAbility.zhekouweishangpinjiagedezhekou，shurushuzi，ru85zhe，shuru8.5，9zheshuru9',
          })}
        >
          {intl.formatMessage({ id: 'marketingAbility.zhekou' })}
      ...
      render: (text: any, record: any, index: number) => (
        <Form.Item
          style={{ margin: 0 }}
          initialValue={text}
          key={`discount_${record.id}`}
          name={`discount_${record.id}`}
          rules={[{ required: true, message: `${intl.formatMessage({ id: 'marketingAbility.qingshuruzhekou' })}` }]}
        >
          <Input
            value={record.discount}
            addonAfter={intl.formatMessage({ id: 'marketingAbility.zhe' })}
            onChange={(e) => {
              _changeActivityPrice(record, e.target.value)
            }}
            placeholder={intl.formatMessage({ id: 'marketingAbility.qingshuruzhekou' })}
          />
        </Form.Item>
      ),
    },
    {
      title: `${intl.formatMessage({ id: 'marketingAbility.huodongjiage' })}`,
      key: 'activityPrice',
