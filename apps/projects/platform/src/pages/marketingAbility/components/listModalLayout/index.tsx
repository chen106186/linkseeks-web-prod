...
const ListModalLayout: React.FC<ListModalLayoutProps> = (props: any) => {
  const intl = useIntl()
  const { idNotInList, remind, value, shopIdList, title, visible, onClose, onConfirm, isPreview, isGift } = props
  ...
  const handleClick = () => {
    form.validateFields().then((_res) => {
      ...
      if (new Set(limitValue).size !== limitValue.length) {
        message.error(
          intl.formatMessage({
            id: 'selfManagement.jietijiagejiaoyan',
            defaultMessage: '{{data}}阶梯必须大于或小于其他{{data}}阶梯',
            data: remind.label[1],
          }),
        )
      } else {
