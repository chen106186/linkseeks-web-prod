...
const columns_2 = ({ dataSource, setDataSource, handleDelete, form, current }) => {
  const intl = getIntl()
  ...
  return [
    ...
    {
      title: (
        <Tooltip placement="top" title={intl.formatMessage({ id: 'marketingAbility.zhijiangjiageweishangpin' })}>
          {intl.formatMessage({ id: 'marketingAbility.zhijiangjiage' })}
                ...
                validator: (_rule, value) => {
                  ...
                  if (!value) {
                    return Promise.reject(
                      new Error(`${intl.formatMessage({ id: 'marketingAbility.qingshuruhuodongjiage' })}`),
                    )
                  }
