...
import { Image, Form, Input, Popconfirm, Button, Typography } from 'antd'
import { normalizeUnitPrice } from '@/pages/marketingAbility/merchantCoupon/utils'
import { getWebIntl } from '@apps/locales'

const translate = getWebIntl()
const columns_4 = ({ dataSource, setDataSource, handleDelete, form, handlCollocation, value, current }) => {
  /** 输入  */
  const intl = getIntl()
        ...
        <>
          ...
          {value === 6 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'marketingAbility.shezhizengpin' })}
            </Button>
          )}
          {value === 13 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'marketingAbility.shezhihuangoushangpin' })}
            </Button>
          )}
          {value === 15 && (
            <Button type="link" onClick={() => handlCollocation(_record)}>
              {intl.formatMessage({ id: 'marketingAbility.shezhidapeishangpin' })}
            </Button>
          )}
        </>
      ),
