...
const columns_5 = ({ dataSource, setDataSource, handleDelete, form, current }) => {
  /** 输入  */
  const intl = getIntl()
                ...
                validator: (_rule, value) => {
                  ...
                  if (!value) {
                    return Promise.reject(
                      new Error(`${intl.formatMessage({ id: 'marketingAbility.qingshurutuangoujiage' })}`),
                    )
                  }
                  if (!pattern.test(value) || Number(value) >= Number(_record.price)) {
                    return Promise.reject(
                      new Error(
                        `${intl.formatMessage({
                          id: 'marketingAbility.bixudayu0qiexiaoyushangpinjiage',
