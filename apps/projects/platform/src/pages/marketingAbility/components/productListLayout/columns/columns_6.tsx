...
const columns_6 = ({ dataSource, setDataSource, handleDelete, form, current }) => {
  /** 输入  */
  const intl = getIntl()
  ...
  return [
    ...
    },
    {
      title: (
        <Tooltip
          placement="top"
          title={intl.formatMessage({ id: 'marketingAbility.diyigeyonghubangkanjiashideqishijiage' })}
        >
          {intl.formatMessage({ id: 'marketingAbility.qishijiage' })}
        ...
        return (
                ...
                validator: (_rule, value) => {
                  ...
                  if (!value) {
                    return Promise.reject(
                      new Error(`${intl.formatMessage({ id: 'marketingAbility.qingshuruqishijiage' })}`),
                    )
                  }
          ...
          >
            <Input
              ...
              onPressEnter={(e) => handleInputChange(e, 'plummetPrice', _index)}
              onBlur={(e) => handleInputChange(e, 'plummetPrice', _index)}
            />
          </Form.Item>
        )
      },
      ...
      title: (
        <Tooltip
          ...
          title={intl.formatMessage({
            id: 'marketingAbility.kanjiaguochengzhongzuihouyicikanjiabunengchaoguokanjiadijia',
          })}
        >
          {intl.formatMessage({ id: 'marketingAbility.kanjiadijia' })}
          <QuestionCircleOutlined />
        </Tooltip>
      ),
      key: 'activityPrice',
        ...
        return (
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: `${intl.formatMessage({ id: 'marketingAbility.qingshurukanjiadijia' })}`,
              },
              ({ getFieldValue }) => ({
                validator: (_rule, value) => {
                  const pattern = /(^[1-9](\d+)?(\.\d{1,4})?$)|(^\d\.\d{1,4}$)/
                  ...
                  if (!(Number(value) < Number(plummetPrice))) {
                    return Promise.reject(
                      new Error(`${intl.formatMessage({ id: 'marketingAbility.bixudayu0qiexiaoyuqishijiage' })}`),
                    )
                  }
                  return Promise.resolve()
                },
