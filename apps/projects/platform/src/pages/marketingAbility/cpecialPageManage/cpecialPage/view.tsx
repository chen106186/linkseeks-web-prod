    ...
    <Fragment>
      ...
      >
        <Form form={form} labelCol={{ span: 5 }} labelAlign="left">
          ...
          <Form.Item style={{ marginBottom: 0 }} label={translate('web.resource.marketing.shiyongshangcheng')} required>
            <Row gutter={8}>
              {/* <Col span={8}>
                <Form.Item
                  name="type"
          ...
          <Form.Item noStyle shouldUpdate={(prevValues, currentValues) => prevValues.shopId !== currentValues.shopId}>
            {({ getFieldValue }) => {
              const shopId = getFieldValue('shopId')
              const shopInfo = shopList.find((item) => item.value === shopId)
              if (shopInfo && !shopInfo.isSelf) {
                return (
                  <Form.Item
                    label={translate('web.resource.marketing.shiyongdianpu')}
                    name="storeId"
                    rules={[
                      {
                        required: true,
