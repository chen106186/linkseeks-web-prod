...
import { ACTIVITY_TYPE_4, ACTIVITY_TYPE_5 } from '@/constants/marketing'
const intl = getIntl()

const actions = createFormActions()

type GoodListType = GetMarketingAdornMerchantActivityListAdornResponseDetail['goodsList']
...
const ActivityProductDrawer: React.FC<Iprops> = (props: Iprops) => {
  ...
  /** 当前选择的活动 */
  const [selectedActivity, setSelectedActivity] = useState<GetMarketingAdornMerchantActivityListAdornResponseDetail>()
  /** 当前选中的活动的商品 */
  const [selectedActivityProductList, setSelectedActivityProductList] = useState<
        ...
        return (
          <div className={styles.priceInfo}>
            <span>
              {intl.formatMessage({ id: 'common.money' })}
              {selectedActivity &&
              (selectedActivity.activityType === ACTIVITY_TYPE_4 || selectedActivity.activityType === ACTIVITY_TYPE_5)
                ? _record.price
                : _record.activityPrice || _record?.plummetPrice || _record.deductionPrice || _record.price}
            </span>
            <span className={styles.unit}>({_record.unit})</span>
          </div>
        )
  ...
  const handleSelectAll = (
    selected: boolean,
    selectedRows: any[],
    changeRows: GetMarketingAdornMerchantActivityListAdornResponseDetail['goodsList'],
  ) => {
    if (selected) {
