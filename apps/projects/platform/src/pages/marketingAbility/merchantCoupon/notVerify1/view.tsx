...
const MerchantCouponNotVerify1: React.FC = () => {
  ...
  const fetchData = async (params: any) => {
    ...
    let res = await getMarketingCouponWaitAuditOnePage(newParams as any)
    return res.data
  }

  const defaultColumns = commonColumn('/marketingAbility/merchantCoupon/notVerify1/detail').concat([
    {
      ...
      render: (_, record) => (
        <>
          <AuthButton type="custom" code="verify">
            <Button
              type="link"
              onClick={() => history.push(`/marketingAbility/merchantCoupon/notVerify1/verify?id=${record.id}`)}
            >
