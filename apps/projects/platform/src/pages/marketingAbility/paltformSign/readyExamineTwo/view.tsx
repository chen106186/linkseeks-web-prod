...
import {
  ...
  postMarketingPlatformActivitySignupExamineStep2Batch,
} from '@apps/apis'
...
const ReadyExamineTwo = () => {
      ...
      render: (text, record) => (
        <DetailAuthButton>
          <EyeAuthButton
            ...
            url={`/marketingAbility/paltformSign/readyExamineTwo/detail?activityId=${record.activityId}&signUpId=${record.id}&preview=true`}
          >
            {text}
          </EyeAuthButton>
        </DetailAuthButton>
      ...
      render: (_, record) => (
        <AuthButton type="custom" code="examine">
          <Button
            ...
            onClick={() =>
              history.push(
                `/marketingAbility/paltformSign/readyExamineTwo/detail?activityId=${record.activityId}&signUpId=${record.id}`,
              )
            }
          >
  ...
  const fetchSubmitBatch = async () => {
    const res = await postMarketingPlatformActivitySignupExamineStep2Batch({ ids: rowkeys })
  ...
  return (
    <TableLayout
      ...
      // rowKey="activityId"
      fetch={getMarketingPlatformActivitySignupPageTobeExamineStep2}
