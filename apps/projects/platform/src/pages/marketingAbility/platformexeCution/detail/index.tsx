...
const DetialLayout = () => {
    ...
    setBasicEffect([
      {
        col: [
          ...
          {
            label: `${intl.formatMessage({ id: 'paltformSign.externalState' })}`,
            extra: <Tag color={OuterStatusColor(data.outerStatus)}>{data.outerStatusName}</Tag>,
          },
        ],
      },
      {
  ...
  const fetchDataSource = useCallback(async () => {
    await getMarketingAbilityActivityExecutePlatformDetail({ id } as any)
      .then((res: any) => {
        if (res.code !== 1000) {
          return
        }
    ...
    (params?: any) => {
      return new Promise((resolve, reject) => {
        if (!Array.isArray(fetch)) {
          getMarketingAbilityActivityExecutePlatformDetailGoodsExecuteDetailPage({ ...params, ...param })
            .then((res) => {
              resolve(res.data)
      ...
      >
        <Space direction="vertical" style={{ display: 'flex' }} size={16}>
          ...
          <ListLayout
            ...
            fetch={getMarketingAbilityActivityExecutePlatformDetailGoodsPage}
            columns={coulumsList}
          />
        </Space>
      </PageHeaderWrapper>
