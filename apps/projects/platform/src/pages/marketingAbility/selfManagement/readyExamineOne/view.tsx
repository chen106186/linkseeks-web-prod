...
const ReadyExamineOne = () => {
  ...
  const fetchSubmitBatch = async () => {
    ...
    await postMarketingMerchantActivityExamineStep1Batch({ idList: rowkeys })
      .then((res) => {
        if (res.code !== 1000) {
      ...
      render: (text, record) => (
        <DetailAuthButton>
          <EyeAuthButton
            ...
            url={`/marketingAbility/selfManagement/readyExamineOne/detail?id=${record.id}&preview=1`}
          >
            {text}
          </EyeAuthButton>
        </DetailAuthButton>
    ...
    },
    {
      ...
      render: (_, record) => (
        <AuthButton type="custom" code="examine">
          <Button
            type="link"
            onClick={() => history.push(`/marketingAbility/selfManagement/readyExamineOne/detail?id=${record.id}`)}
          >
  ...
  return (
    <TableLayout
      ...
      fetch={getMarketingMerchantActivityPageExamineStep1}
