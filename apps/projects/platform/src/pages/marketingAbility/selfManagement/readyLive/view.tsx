...
const ReadyLive = () => {
  ...
  const fetchSubmitBatch = async (id?: number) => {
    let res: any = null
    if (id) {
      res = await postMarketingMerchantActivityOnline({ id: Number(id) })
    } else {
      res = await postMarketingMerchantActivityOnlineBatch({ idList: rowkeys })
    }
  ...
  const columns: ColumnType<any>[] = [
      ...
      render: (text, record) => (
        <DetailAuthButton>
          <EyeAuthButton
            ...
            url={`/marketingAbility/selfManagement/readyLive/detail?id=${record.id}&preview=1`}
          >
            {text}
          </EyeAuthButton>
        </DetailAuthButton>
    ...
    },
    {
      ...
      render: (_text, _record) => (
        <Fragment>
          {_record.online && (
            <AuthButton type="custom" code="online">
              <Popconfirm
                okButtonProps={{ loading: submitLoading }}
                title={intl.formatMessage({ id: 'selfManagement.launchActivities?' })}
  ...
  return (
    <TableLayout
      ...
      fetch={getMarketingMerchantActivityPageTobeOnline}
          ...
          <Col span={6}>
            <AuthButton type="custom" code="batch">
              <Button loading={submitLoading} onClick={() => fetchSubmitBatch()} disabled={rowkeys.length === 0}>
                {intl.formatMessage({ id: 'selfManagement.batchOnlineActivities' })}
              </Button>
            </AuthButton>
          </Col>
        </Row>
