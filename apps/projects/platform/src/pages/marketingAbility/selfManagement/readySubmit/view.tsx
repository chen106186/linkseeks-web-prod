...
const ReadySubmit = () => {
  ...
  const fetchSubmitBatch = async (id?: number) => {
    let res: any = null
    if (id) {
      res = await postMarketingMerchantActivitySubmit({ id: Number(id) })
    } else {
      res = await postMarketingMerchantActivitySubmitBatch({ idList: rowkeys })
    }
      ...
      render: (text, record) => (
        <DetailAuthButton>
          <EyeAuthButton
            ...
            url={`/marketingAbility/selfManagement/readySubmit/detail?id=${record.id}&preview=1`}
          >
            {text}
          </EyeAuthButton>
        </DetailAuthButton>
  ...
  return (
    <TableLayout
      ...
      fetch={getMarketingMerchantActivityPageTobeSubmit}
