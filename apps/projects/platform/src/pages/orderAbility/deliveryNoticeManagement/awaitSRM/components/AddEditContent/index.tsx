...
import { Button, DatePicker, Form, Input, message, Table, TimePicker } from 'antd'
...
const DeliveryNoticeManagementAwaitSRMDetails: React.FC<PropsType> = ({ type, id, planData, btnCode }) => {
  ...
  const getConst = (len = 0) => {
    return [
      base_info,
      ShippingInfo,
      { ...Material, label: `${translate('web.resource.commodity.songhuowuliao')}${!!len ? `(${len})` : ''}` },
      Remarks,
    ]
  }
  ...
  return (
    <PageHeaderWrapper
      ...
      items={getConst(tableDataSource?.length)}
      extra={
    ...
    >
      ...
      >
        ...
        <BaseInfo className="mt-16" title={Material.label} id={Material.key} cols={1}>
