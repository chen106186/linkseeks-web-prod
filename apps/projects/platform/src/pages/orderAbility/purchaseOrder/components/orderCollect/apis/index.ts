...
export const fetchOrderApi = {
  ...
  async getProductListByDemandOrderId(params) {
    // const { data } = await getOrderQuotationParticularsProductList(params)
    return []
  },
  ...
  async getPayInfoList(params) {
    // const { data } = await getOrderInitializationPaymentInformation(params)
    return []
  },
  ...
  //   const { data } = await getContractContractTemplateGet(params)
  //   return data
  // },

  /** 生成电子合同 */
  async createContract(params) {
    const { data } = await postContractContractSignProcurementSignContractCreate(params, { ctlType: 'none' })
    return data
  },

  /** 获取合并订单列表 */
  async getMergeOrderList(params) {
    // const { data } = await getOrderMergeOrderList(params)
    return []
  },

  /** 合同下单 获取合同采购物料 */
  async getContractPurchaseMaterielList(params) {
    const { data } = await getContractManagePagePurchaseMaterielList(params)
    return data
  },
}
