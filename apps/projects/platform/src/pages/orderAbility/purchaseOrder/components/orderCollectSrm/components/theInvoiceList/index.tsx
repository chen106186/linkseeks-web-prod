...
const TheInvoiceList = (props: ISchemaFieldComponentProps) => {
  ...
  if (pageStatus === PageStatus.ADD) {
  ...
  } else if (pageStatus === PageStatus.EDIT || pageStatus === PageStatus.VARIATION) {
    ...
    if (typeof value === 'object') {
      ...
      const target = dataSource.length ? dataSource.find((v) => v.id === value) : value
      setFieldState({
        dataSource,
        showMore,
        useValue: target,
  ...
  const reload = () => {
    fetchOrderApi.getInvoicesList().then((data) => {
      ...
      // 订单新增 重载全部置为第一个
      const _data = data.sort((a, b) => b.id - a.id)
      if (pageStatus === PageStatus.ADD) {
        setFieldState({
          dataSource: _data,
          showMore,
  ...
  const handleDelete = async (_id, e) => {
    ...
    e.stopPropagation()
    try {
      const result = await postSettlementInvoiceMessageDelete({ id: _id })
      if (result.code === 1000) {
        reload()
        setMode('delete')
      }
        ...
        <div className={styles.invoice_list}>
          <Row gutter={[16, 16]}>
            {showDataSource.map((item, index) => (
              <Col span={12} key={`invoice_list_item_${index + 1}`}>
                <Radio className={styles.list_radio} value={item} key={`address_list_radio_${item?.id}`}>
                  <div className={styles.invoice_list_item}>
                    <div className={styles.invoice_list_item_content}>
                      <div className={cx(styles.invoice_list_item_content_tag, item.kind !== 1 ? styles.special : '')}>
                        {item.kind === 1
                          ? intl.formatMessage({
                              id: 'purchaseOrder.orderCollect.theInvoiceList.kind1',
                            })
                          : intl.formatMessage({
                              id: 'purchaseOrder.orderCollect.theInvoiceList.kind2',
                            })}
                      </div>
                      <div className={styles.invoice_list_item_content_name}>
                        ...
                        <span>
                          ...
                          {item.type === 1
                            ? intl.formatMessage({
                                id: 'purchaseOrder.orderCollect.theInvoiceList.type1',
                              })
                            : intl.formatMessage({
                                id: 'purchaseOrder.orderCollect.theInvoiceList.type2',
                              })}
                          )
                        </span>
                        {item.isDefault === 1 ? (
                          ...
                          <div className={styles.set_default} onClick={(e) => handleSetDefault(item, e)}>
                            {intl.formatMessage({
                              id: 'purchaseOrder.orderCollect.theInvoiceList.default2',
                            })}
                          </div>
                        )}
                      </div>
