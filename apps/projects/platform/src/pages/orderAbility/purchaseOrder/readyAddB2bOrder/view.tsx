...
import '../index.less'
import { OrderModalType, ORDER_TYPE_INQUIRYT_PURCHASE } from '@/constants/order'
import { getOrderBuyerCreatePage, postOrderBuyerCreateDeleteBatch, postOrderBuyerCreateSubmitBatch } from '@apps/apis'
...
export interface ReadyAddB2bOrderProps {}

const fetchTableData = async (params) => {
  params.orderType = ORDER_TYPE_INQUIRYT_PURCHASE
  const { data } = await getOrderBuyerCreatePage(params)
  return data
}

const ReadyAddB2bOrder: React.FC<ReadyAddB2bOrderProps> = () => {
  const intl = useIntl()
  ...
  const clickAdd = () => {
    // b2b 询价报价下单
    history.push(`/orderAbility/purchaseOrder/readyAddB2bOrder/add?modelType=${OrderModalType.INQUIRY_QUOTATION_ORDER}`)
  }
...
ReadyAddB2bOrder.defaultProps = {}

export default ReadyAddB2bOrder
