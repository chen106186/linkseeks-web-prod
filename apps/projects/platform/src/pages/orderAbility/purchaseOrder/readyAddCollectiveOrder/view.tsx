...
import {
  getOrderCollectiveCreatePage,
  postOrderCollectiveCreateDelete,
  postOrderCollectiveCreateDeleteBatch,
  postOrderCollectiveCreateSubmit,
  postOrderCollectiveCreateSubmitBatch,
} from '@apps/apis'
...
const fetchTableData = async (params) => {
  const { data } = await getOrderCollectiveCreatePage(params)
  return data
}
 ...
 * @returns
 */
const ReadyAddSpotPurchaseOrder: React.FC = () => {
  const { run: deleteRun } = useHttpRequest(postOrderCollectiveCreateDeleteBatch as any)
  const tableRef = StandardFormTable.useTableRef()
  const { run: submitRun } = useHttpRequest(postOrderCollectiveCreateSubmitBatch as any)
  const { pathname } = useLocation()
  const translate = useWebIntl()

  const handleSubmit = async (id) => {
    await postOrderCollectiveCreateSubmit({ orderId: id })
    tableRef.current.clearSelection()
    tableRef.current.reload()
  }

  const handleDelete = async (id) => {
    await postOrderCollectiveCreateDelete({ orderId: id })
    tableRef.current.reload()
  }
        ...
        <Space>
          ...
          <EditAuthButton>
            <Button
              type="link"
              onClick={() =>
                history.push(`/orderAbility/purchaseOrder/readyAddCollectiveOrder/edit?id=${record.orderId}`)
              }
            >
        ...
        searchButtons={[
          {
            ...
            type: 'primary',
            onClick: () => {
              history.push('/orderAbility/purchaseOrder/readyAddCollectiveOrder/add')
            },
          },
          {
            key: 'batchSubmit',
...
export default ReadyAddSpotPurchaseOrder
