...
import { getOrderParamFindDeliveryDate } from '@apps/apis'
import { getProductCustomerGetMemberCustomerCategoryTree, getProductSelectGetMemberBrand } from '@apps/apis'
...
const asyncPadDataForProduct = async (ctx: ISchemaFormActions | ISchemaFormAsyncActions, productValue: any) => {
  ...
  const memberRoleId = ctx.getFieldValue('buyerRoleId')
  // 折扣请求
  const asyncPriceRequests: any[] = await Promise.all(
    productData
      .filter((v) => v.memberPrice === undefined)
      .map(async (v) => {
        const { code, data } = await getMemberManageLowerCreditParamGet(
          {
...
export const useOrderDeliverTimeEffect = async (ctx: ISchemaFormActions | ISchemaFormAsyncActions, shopId) => {
  if (shopId) {
    const { code, data } = await getOrderParamFindDeliveryDate({ shopId })
    if (code !== 1000) {
      return false
    }
...
export const useOrderFormInitEffect = (ctx: ISchemaFormActions | ISchemaFormAsyncActions) => {
  ...
  FormEffectHooks.onFieldValueChange$('buyerMemberMajorId').subscribe((state) => {
    if (state.value) {
      ctx.setFieldValue('hasInvoice', false)
