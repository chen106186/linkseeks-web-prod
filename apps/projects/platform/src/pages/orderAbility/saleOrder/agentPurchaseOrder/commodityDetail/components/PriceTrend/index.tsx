...
const PriceTrend: React.FC<InterestedPropsType> = (props) => {
  ...
  const SCREEN_OPTIONS = [
    { label: intl.formatMessage({ id: 'mall.price.trend.week.near' }), value: 7 },
    { label: intl.formatMessage({ id: 'mall.price.trend.month.near' }), value: 30 },
    { label: intl.formatMessage({ id: 'mall.price.trend.three.month' }), value: 90 },
  ]
  ...
  // 处理出最高价格
  // 处理出纵轴价格区间最大值
  // const dealScalePrice = (data: PriceRecordDataType) => {
  //   if (data?.commodityPriceRecordList) {
  //     let maxSortPrice: PriceRecordType[] = data?.commodityPriceRecordList.sort((a: PriceRecordType, b: PriceRecordType) => b.price - a.price)
  //     const pow = maxSortPrice[0].price.toString().split('.')[0].length || 0
  //     // setMaxPrice(maxSortPrice[0].price)
  //     setScalePrice({
  //       min: 0,
  //       max: Math.pow(10, pow)
  //     })
    ...
    <div className={cx(styles.price_trend)}>
      <div className={styles.price_trend_btn}>
        <div className={styles.price_trend_btn_con}>
          ...
          {/* <span className={styles.price_trend_btn_con_text}>{intl.formatMessage('commodityDetail.priceTrend.priceTrend')}</span> */}
        </div>
        <div className={styles.price_trend_chart}>
          <div className={styles.price_trend_chart_header}>
            <div className={styles.price_trend_chart_header_extreme}>
              <span>
                {intl.formatMessage({ id: 'commodityDetail.priceTrend.currentPrice' })}：
                {intl.formatMessage({ id: 'common.money' })}
                {commodityPriceInfo[0]?.price || '-'}
              </span>
              <span>
                {intl.formatMessage({ id: 'commodityDetail.priceTrend.minimumPrice' })}：
                {intl.formatMessage({ id: 'common.money' })}
