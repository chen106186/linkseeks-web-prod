...
const SkuInfo: React.FC<SkuInfoProps> = (props) => {
  const { type, skuId, productInfo, currentSku, onSelect } = props
  ...
  return skuList && skuList.length > 0 ? (
    <>
      {skuList.map(
        (skuItem, index) =>
                ...
                <div className={styles.product_info_line_list}>
                  {skuItem.customerAttributeValueList.map((childItem: any, index: number) => (
                    <div
                      ...
                      onClick={() => handleSelectAttrVal(skuItem.customerAttribute.id, childItem.id)}
                    >
