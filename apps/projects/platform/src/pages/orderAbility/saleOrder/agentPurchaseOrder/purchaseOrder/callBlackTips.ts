 ...
 */
const fnGetDiscount = (ladders: any) => {
  let callBlackTips = ''
  ladders.forEach((item: any, index: number) => {
    // const keys = Object.keys(item)[0];
    // callBlackTips = `${callBlackTips}满${keys}${unit}，总价${reduction === '折' ? '打' : '减免'}${reduction === '折' ? item[keys] / 10 : item[keys]}${reduction}${index < (ladders.length-1) ? ' ; ' : ''}`;
 ...
 * @returns 满量（额）折扣（减免元）梯度提示语
 */
const fnGetGift = (ladders: any, unit: string) => {
  let callBlackTips = ''
  ladders.forEach((item: any, index: number) => {
    const keys = Object.keys(item)[0]
    callBlackTips = `${callBlackTips}满${keys}${unit}，赠送${item[keys]}${index < ladders.length - 1 ? ' ; ' : ''}`
  })
  return callBlackTips
}
/**
...
const fnGetActivityTips = (newActivity: any) => {
  ...
  if (newActivity.activityType === 4 && newActivity.concreteType === 1) {
    // 4 + 1 满量促销+满量减免
    callBlackTips = fnGetDiscount(newActivity.preferentialTagDescs)
