...
const fnInitHandPricePar = (selectCommodity: any, mallId: number) => {
  ...
  if (!selectCommodity.parentSkuId) {
    return obj
  }
}
/**
...
const handleCountChange = (count: number, id: number, mallId: number) => {
  ...
  postProductShopPurchaseSaveOrUpdatePurchase(param, { headers: { shopId: String(mallId) } }).then((res) => {
    if (res.code === 1000) {
      //   message.destroy()
    }
  })
 ...
 * @param mallId 当前商城id
 */
const fnInitHandPrice = async (shopList: any[], mallId: number, agentMemberId: number, agentRoleId: number) => {
    ...
    res.data.forEach((item: any) => {
      shopList.find((thisShop: any) => {
        thisShop.orderList.find((thisCommodyty: any) => {
          if (thisCommodyty.purchaseSkuResp.id === item.skuId && (thisCommodyty.isMain || !thisCommodyty.parentSkuId)) {
            // thisCommodyty.price = item.
            // groupHandPrice 套餐到手价 handPrice 活动到手价 basePrice 会员比例后的价格 commodityPrice 策略价格
            if (item.enableQuantity) {
              // thisCommodyty.stockCount = item.enableQuantity;
              thisCommodyty.count = item.enableQuantity
              message.error(
                `${thisCommodyty.purchaseSkuResp.commodity.name}活动可购买数量不足,已帮你重置为${item.enableQuantity}${thisCommodyty.purchaseSkuResp.commodity.unitName}`,
              )
              shouResetInit = true
