...
import { getMemberOrderCommentVendorPage } from '@apps/apis'
...
const Unevaluated: React.FC<any> = (props) => {
  ...
  const defaultColumns: ColumnType<any>[] = [
    ...
    },
    {
      ...
      align: 'center',
      render: (text, record) => (
        <Badge color="#41CC9E" text={intl.formatMessage({ id: 'supplierEvaluation.yiwancheng' })} />
      ),
    },
    {
          ...
          <EditAuthButton>
            <Link to={`${pathname}/edit?id=${record.id}`}>
              {!record.completeCommentStatus && (
                <Button type="link">{intl.formatMessage({ id: 'supplierEvaluation.pingjia' })}</Button>
              )}
            </Link>
          </EditAuthButton>
        </>
