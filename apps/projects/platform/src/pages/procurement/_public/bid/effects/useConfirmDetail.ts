...
import { getPurchaseInviteTenderGetCheckFinishTender, getPurchaseInviteTenderGetInviteTenderProcess, getPurchaseInviteTenderInCheckRecordGetInviteTenderInCheckRecord, getPurchaseTenderOutCheckRecordGetInviteTenderOutCheckRecord } from '@apps/apis'
...
export const useConfirmDetail = (options: {}) => {
  ...
  const reloadFormData = useCallback(async () => {
    if (id) {
      const { code, data, message: msg } = await getPurchaseInviteTenderGetCheckFinishTender({inviteTenderId: id}, { ctlType: "none" })
      if (code === 1000) {
        setFormData(data)
      } else {
        message.error(msg)
