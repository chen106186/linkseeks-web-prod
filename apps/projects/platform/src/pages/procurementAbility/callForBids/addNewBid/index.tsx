...
const AddNewBid: React.FC<AddNewBidProps> = (props) => {
  ...
  const { materialAddButton, materialColumns, materialComponents } = useMaterialTable(
    addSchemaAction,
    goodRef,
    drawerSchemaAction,
  )
  ...
  const handleSubmit = async (value) => {
    setBtnLoading(true)
    const params = paramsConversionFn(value, pageStatus)
  ...
  return (
    <PageHeaderWrapper
      ...
      title={
        pageStatus === PageStatus.ADD
          ? intl.formatMessage({ id: 'table.purchase.xjzb' })
          : intl.formatMessage({ id: 'table.purchase.bjzb' })
      }
      extra={[
        <Button
          key="1"
    ...
    >
      <ReadyAddBidDetailContext.Provider value={providerValue}>
        ...
        {/* 公开方式招标 触发弹窗 */}
        <PublicBidModal currentRef={publicRef} pageAction={addSchemaAction} />
