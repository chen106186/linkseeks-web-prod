      ...
      setTableDataSource(() =>
        tempData.map((item, index) => {
          ...
          _memberList.forEach((_item) => {
            console.log(_item, '__')
            let temp = {}
            temp['memberList'] = { id: _item.id }
            temp['submitTender'] = { id: _item.submitTender.id }
            temp['totalAmount'] = _item.totalAmount
            // 默认不授标 授标率100
            temp['isAwardTender'] = _item.submitTender.submitTenderMateriel[index]['isAwardTender']
            temp['awardTenderRatio'] = _item.submitTender.submitTenderMateriel[index]['awardTenderRatio']
