...
import {
  ...
  getPurchaseInviteTenderGetWinTenderNotice,
...
export const useNoticeDetail = (options: DetailHookProps) => {
  // 详情数据
  ...
  const reloadFormData = useCallback(async () => {
    if (id) {
      const {
        ...
        data,
        message: msg,
      } = await getPurchaseInviteTenderGetWinTenderNotice({ inviteTenderId: id }, { ctlType: 'none' })
      if (code === 1000) {
        setFormData(data)
      } else {
        message.error(msg)
