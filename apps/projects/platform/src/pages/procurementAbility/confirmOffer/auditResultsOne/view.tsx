...
import {
  getPurchaseConfirmQuotedPriceStayExamineAward1List,
  postPurchaseConfirmQuotedPriceStayExamineAwards1,
} from '@apps/apis'
...
const AuditResultsOne = () => {
        ...
        <>
          <AuthButton type="custom" code="audit">
            <Button
              ...
              onClick={() =>
                history.push(
                  `/procurementAbility/confirmOffer/auditResultsOne/detail?id=${record.id}&turn=${record.turn}`,
                )
              }
            >
          ...
          <AuthButton type="custom" code="see">
            <Button
              ...
              onClick={() =>
                history.push(
                  `/procurementAbility/confirmOffer/auditResultsOne/detail?id=${record.id}&turn=${record.turn}&preview=true`,
                )
              }
            >
  ...
  const fetchSubmitBatch = async () => {
    setLoading(true)
    const res = await postPurchaseConfirmQuotedPriceStayExamineAwards1({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  return (
    <Table
      ...
      fetch={getPurchaseConfirmQuotedPriceStayExamineAward1List}
      controllerBtns={
        <Row>
          <Col span={6}>
