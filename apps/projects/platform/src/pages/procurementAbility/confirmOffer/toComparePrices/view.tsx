...
import { getPurchaseConfirmQuotedPriceStayContrastPrice } from '@apps/apis'
...
const ToComparePrices = () => {
  const columns: ColumnType<any>[] = [
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <>
          {record.button === BUTTONAUTHORITY.SIX && authFullUrl('/procurementAbility/confirmOffer/contrast') && (
            <Button
              type="link"
              onClick={() =>
                history.push(`/procurementAbility/confirmOffer/contrast?id=${record.id}&turn=${record.turn}`)
              }
            >
              {intl.formatMessage({ id: 'detail.purchase.method' })}
            </Button>
          )}
          {record.button === BUTTONAUTHORITY.SEVEN && (
            <AuthButton type="custom" code="editResult">
              <Button
                type="link"
                onClick={() =>
                  history.push(`/procurementAbility/confirmOffer/contrast?id=${record.id}&turn=${record.turn}`)
              ...
              >
                {intl.formatMessage({ id: 'detail.purchase.editResult' })}
              </Button>
            </AuthButton>
          )}
          <AuthButton type="custom" code="see">
            <Button
              ...
              onClick={() =>
                history.push(
                  `/procurementAbility/confirmOffer/toComparePrices/detail?id=${record.id}&turn=${record.turn}&preview=true`,
                )
              }
            >
  ...
  return (
    <Table
      ...
      fetch={getPurchaseConfirmQuotedPriceStayContrastPrice}
    />
  )
}
