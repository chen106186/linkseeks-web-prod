...
import { getPurchaseNeedPlanToBeFirstExamList } from '@apps/apis'
...
const DemandPlanOne = () => {
        ...
        <Space direction="vertical">
          <DetailAuthButton>
            <EyeAuthButton
              ...
              url={`/procurementAbility/demandPlan/demandPlanOne/detail?id=${record.id}`}
            >
              {text}
            </EyeAuthButton>
          </DetailAuthButton>
      ...
      render: (text: any, record: any) => (
        <>
          <AuthButton type="custom" code="submit">
            <Button
              type="link"
              onClick={() => history.push(`/procurementAbility/demandPlan/demandPlanOne/detail?id=${record.id}`)}
            >
  ...
  return (
    <Table
      ...
      columns={columns}
      effects="needPlanNo"
      fetch={getPurchaseNeedPlanToBeFirstExamList}
    />
  )
}
