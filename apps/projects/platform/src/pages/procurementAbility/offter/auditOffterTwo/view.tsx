...
import { getPurchaseQuotedPriceStayExamineList2, postPurchaseQuotedPriceExamine2Batch } from '@apps/apis'
...
const AuditOffterTwo = () => {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <Space direction="vertical">
          <EyeAuthButton
            url={`/procurementAbility/offter/auditOffterTwo/detail?id=${record.id}&number=${record.quotedPriceNo}&turn=${record.turn}`}
          >
            {text}
          </EyeAuthButton>
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space direction="vertical">
          <EyeAuthButton
            url={`/procurementAbility/offter/auditOffterTwo/preview?id=${record.purchaseInquiryId}&number=${record.purchaseInquiryNo}`}
          >
            {text}
          </EyeAuthButton>
        ...
        <>
          <AuthButton type="custom" code="audit">
            <Button
              ...
              onClick={() =>
                history.push(
                  `/procurementAbility/offter/auditOffterTwo/detail?id=${record.id}&number=${record.quotedPriceNo}&turn=${record.turn}`,
                )
              }
            >
  ...
  const fetchSubmitBatch = async () => {
    setLoading(true)
    const res = await postPurchaseQuotedPriceExamine2Batch({ ids: rowkeys })
    if (res.code === 1000) {
  ...
  return (
    <Table
      ...
      fetch={getPurchaseQuotedPriceStayExamineList2}
      controllerBtns={
        <Row>
          <Col span={6}>
