...
const ReadyExamineResultOne = () => {
        ...
        <Space direction="vertical" style={{ width: 300 }}>
          <DetailAuthButton>
            <EyeAuthButton
              ...
              url={`/procurementAbility/purchaseBid/readyExamineResultOne/detail?id=${record.id}&number=${text}`}
            >
              {text}
            </EyeAuthButton>
          </DetailAuthButton>
  ...
  const fetchSubmitBatch = async (id?: number) => {
    let res = null
    if (id) {
      res = await postPurchaseBiddingSubmitBidding1({ id, state: 1 })
    } else {
      res = await postPurchaseBiddingSubmitBidding1Batch({ ids: rowkeys })
    }
    if (res.code === 1000) {
  ...
  const handleExamine = (record: any) => {
    history.push(
      `/procurementAbility/purchaseBid/readyExamineResultOne/detail?id=${record.id}&number=${record.biddingNo}&action=1`,
    )
    // setId(id);
    // setVisible(!visible);
  }
  ...
  return (
    <>
      <Table
        ...
        columns={columns}
        effects="biddingNo"
        fetch={getPurchaseBiddingStaySubmitBiddingList1}
        controllerBtns={
          <Row>
            <Col span={24}>
              <Space size={16}>
