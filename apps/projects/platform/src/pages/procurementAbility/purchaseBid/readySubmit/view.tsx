...
import { getPurchaseBiddingStaySubmitList, postPurchaseBiddingSubmit, postPurchaseBiddingSubmitBatch } from '@apps/apis'
...
const ReadySubmit = () => {
  ...
  const columns: ColumnType<any>[] = [
        ...
        <Space direction="vertical" style={{ width: 300 }}>
          <DetailAuthButton>
            <EyeAuthButton
              ...
              url={`/procurementAbility/purchaseBid/readySubmit/detail?id=${record.id}&number=${text}`}
            >
              {text}
            </EyeAuthButton>
          </DetailAuthButton>
    ...
    },
    {
      ...
      align: 'center',
      render: (text: any, record: any) =>
        record?.buttons?.indexOf(PurchaseBidButtons.SUBMIT_A_PURCHASE_BID) >= 0 && (
  ...
  const fetchSubmitBatch = async (id?: number) => {
    let res = null
    if (id) {
      res = await postPurchaseBiddingSubmit({ id, state: 1 })
    } else {
      res = await postPurchaseBiddingSubmitBatch({ ids: rowkeys })
    }
    if (res.code === 1000) {
  ...
  const handleExamine = (record: any) => {
    history.push(
      `/procurementAbility/purchaseBid/readySubmit/detail?id=${record.id}&number=${record.biddingNo}&action=1`,
    )
    // setId(id);
    // setVisible(!visible);
  }
  ...
  return (
    <>
      <Table
        ...
        columns={columns}
        effects="biddingNo"
        fetch={getPurchaseBiddingStaySubmitList}
        controllerBtns={
          <Row>
            <Col span={24}>
              <Space size={16}>
