...
import { getCommodityShopListEnterpriseShopBySite, getCommodityShopListShopByReq } from '@apps/apis'
...
const Demand: React.FC<Iprops> = (props: any) => {
  ...
  const { currentRef, fetchdata, onBadge, badgeIndex, needOperate = true, isShop = false } = props
      ...
      return columns.concat([
        {
          ...
          render: (_text: any, _record: any) => (
            <>
              {(value === 3 || (value === 2 && _record.isSubMember === 1)) && (
  ...
  useEffect(() => {
    if (isShop) {
      ...
      getCommodityShopListShopByReq({ type: 2 } as any).then((res) => {
        if (res.code === 1000) {
          setStoreList(res.data)
        }
      })
      ...
      <Form {...layout} form={form}>
        ...
        >
          <Radio.Group onChange={changeRadio}>
            ...
            {/* <Radio value={2}>
              <Tooltip placement="topLeft" title={<>系统通过需求单品类、商品属性、适用地市与平台会员发布的商品品类、商品属性、归属地区进行匹配，推荐满足条件的平台会员</>}>
                系统匹配
                <QuestionCircleOutlined style={{ marginLeft: '5px' }} />
              </Tooltip>
            </Radio> */}
            ...
            <Row gutter={[16, 16]}>
              {store.map((item) => (
                <Col span={6} key={item.id} style={ColStyle}>
                  <Image width={32} height={32} src={item.logoUrl || defaultLogo} />
