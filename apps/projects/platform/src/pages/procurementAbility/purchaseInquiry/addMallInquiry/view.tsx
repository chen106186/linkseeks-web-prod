...
import {
  getPurchasePurchaseInquiryShopAddList,
...
const AddInquiry = () => {
  ...
  const columns: ColumnType<any>[] = [
    {
      ...
      render: (text: any, record: any) => (
        <Space direction="vertical">
          <EyeAuthButton
            url={`/procurementAbility/purchaseInquiry/addMallInquiry/detail?id=${record.id}&number=${record.purchaseInquiryNo}`}
          >
            {text}
          </EyeAuthButton>
          ...
          {(record.button === BUTTONAUTHORITY.ONE || record.button === BUTTONAUTHORITY.TWO) && (
            <AuthButton type="edit" code="edit">
              <Button
                ...
                onClick={() =>
                  history.push(
                    `/procurementAbility/purchaseInquiry/addMallInquiry/edit?id=${record.id}&number=${record.purchaseInquiryNo}`,
                  )
                }
              >
  ...
  const addFn = async () => {
    await getCommodityWebMemberPurchaseWebFindCurrMemberPurchase().then((res) => {
      ...
      history.push('/procurementAbility/purchaseInquiry/addMallInquiry/add')
    })
  }
