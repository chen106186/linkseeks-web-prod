...
import { getPurchasePurchaseInquiryStayExamineList2, postPurchasePurchaseInquiryExamine2Batch } from '@apps/apis'
...
const AuditInquiryTwo = () => {
  ...
  const fetchSubmitBatch = async () => {
    const res = await postPurchasePurchaseInquiryExamine2Batch({ ids: rowkeys })
    if (res.code === 1000) {
        ...
        <Space direction="vertical">
          <DetailAuthButton>
            <EyeAuthButton
              ...
              url={`/procurementAbility/purchaseInquiry/auditInquiryTwo/detail?id=${record.id}&number=${record.purchaseInquiryNo}`}
            >
              {text}
            </EyeAuthButton>
          </DetailAuthButton>
        ...
        <>
          <AuthButton type="custom" code="audit">
            <Button
              ...
              onClick={() =>
                history.push(
                  `/procurementAbility/purchaseInquiry/auditInquiryTwo/detail?id=${record.id}&number=${record.purchaseInquiryNo}`,
                )
              }
            >
  ...
  return (
    <Table
      ...
      fetch={getPurchasePurchaseInquiryStayExamineList2}
