...
export const useSelfTable = () => {
  ...
  const handleSubmit = async (id) => {
    history.push(`/procurementAbility/tender/readyBidRegister/add?id=${id}`)
  }
  ...
  const secondColumns: any[] = baseBidListColumns.concat([
    {
      ...
      render: (text, record) => {
        return (
          <>
            {record.isRegister && (
              <AuthButton type="custom" code="isRegister">
                <Button type="link" onClick={() => handleSubmit(record.inviteTender.id)}>
                  {intl.formatMessage({ id: 'detail.purchase.label14' })}
                </Button>
              </AuthButton>
            )}
            {record.isReRegister && (
              <AuthButton type="custom" code="isReRegister">
                <Button type="link" onClick={() => handleSubmit(record.inviteTender.id)}>
