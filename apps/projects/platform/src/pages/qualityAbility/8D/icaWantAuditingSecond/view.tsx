  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/icaWantAuditingSecond/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
        ...
        return (
          <AuthButton type="custom" code="audit">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/icaWantAuditingSecond/detail?id=${record.id}`)
              }}
            >
              <span>
                {intl.formatMessage({
                  id: 'eightD.shenheICAfankui',
                  defaultMessage: '审核ICA反馈',
                })}
              </span>
            </Button>
          </AuthButton>
