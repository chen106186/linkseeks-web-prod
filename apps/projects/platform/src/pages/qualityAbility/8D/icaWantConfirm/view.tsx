  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/icaWantConfirm/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
    ...
    },
    {
      ...
      width: '160px',
      render: (_text, record) => {
        // 内部状态为待确认ICA反馈、ICA不通过一级、ICA不通过二级，才显示确认按钮
        return record.internalStatus === QUATITY_EIGHTD_IN_STATUS_WANT_COMFIRM_ICA ||
          record.internalStatus === QUATITY_EIGHTD_IN_STATUS_NO_PASS_ICA_1 ||
          record.internalStatus === QUATITY_EIGHTD_IN_STATUS_NO_PASS_ICA_2 ? (
          <AuthButton type="custom" code="comfire">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/icaWantConfirm/detail?id=${record.id}`)
              }}
            >
              <span>
                {intl.formatMessage({
                  id: 'eightD.querenICAfankui',
                  defaultMessage: '确认ICA反馈',
                })}
              </span>
            </Button>
          </AuthButton>
