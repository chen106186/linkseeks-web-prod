  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/icaWantSubmit/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
        ...
        return (
          <AuthButton type="custom" code="submit">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/icaWantSubmit/detail?id=${record.id}`)
              }}
            >
              {intl.formatMessage({
                id: 'eightD.tijiaoshenheICAfankui',
                defaultMessage: '提交审核ICA反馈',
              })}
            </Button>
          </AuthButton>
        )
