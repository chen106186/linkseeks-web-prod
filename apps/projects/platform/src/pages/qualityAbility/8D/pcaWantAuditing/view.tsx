  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/pcaWantAuditing/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
        ...
        return (
          <AuthButton type="custom" code="submit">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/pcaWantAuditing/detail?id=${record.id}`)
              }}
            >
              {intl.formatMessage({
                id: 'eightD.tijiaoshenhePCAfankui',
                defaultMessage: '提交审核PCA反馈',
              })}
            </Button>
          </AuthButton>
        )
