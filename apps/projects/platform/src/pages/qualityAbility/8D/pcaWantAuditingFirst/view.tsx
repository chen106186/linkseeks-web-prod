  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/pcaWantAuditingFirst/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
        ...
        return (
          <AuthButton type="custom" code="audit">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/pcaWantAuditingFirst/detail?id=${record.id}`)
              }}
            >
              <span>
                {intl.formatMessage({
