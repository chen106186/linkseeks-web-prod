...
const index: React.FC = () => {
  ...
  const fnResetSummary = (messageDesc) => {
    const summaryDesc = [
      {
        label: intl.formatMessage({ id: 'eightD.shenhePCAfankui', defaultMessage: '审核PCA反馈' }),
        extra: messageDesc.correctionInformation?.effectConfirmed,
      },
    ]
    ...
    ref.current
      ...
      .then((values) => {
        ...
        const obj = {
          ...
          approvalStatus: values.state === 1 ? 2 : 3, // 因为这里少了下一步 所以没有1 通过就是2 不通过是3
  ...
  const fnGetCanAdopt = () => {
    const unAdopt = [13, 15] // 外部状态为13 15 的时候 一级或者二级不通过
  ...
  return (
    <div>
      <PageHeaderWrapper
        title={intl.formatMessage({ id: 'eightD.shenhePCAfankui', defaultMessage: '审核PCA反馈' })}
        items={TABLINK}
        extra={
          <Button
            onClick={() => {
