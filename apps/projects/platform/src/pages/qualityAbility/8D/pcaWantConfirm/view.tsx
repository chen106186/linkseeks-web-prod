  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/pcaWantConfirm/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
    ...
    },
    {
      ...
      width: '160px',
      render: (_text, record) => {
        // 内部状态为待确认PCA反馈、PCA不通过一级、PCA不通过二级，才显示确认按钮
        return record.internalStatus === QUATITY_EIGHTD_IN_STATUS_WANT_COMFIRM_PCA ||
          record.internalStatus === QUATITY_EIGHTD_IN_STATUS_NO_PASS_PCA_1 ||
          record.internalStatus === QUATITY_EIGHTD_IN_STATUS_NO_PASS_PCA_2 ? (
          <AuthButton type="custom" code="comfire">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8D/pcaWantConfirm/detail?id=${record.id}`)
              }}
            >
              <span>
                {intl.formatMessage({
                  id: 'eightD.querenPCAfankui',
                  defaultMessage: '确认PCA反馈',
                })}
              </span>
            </Button>
          </AuthButton>
