...
const index: React.FC = () => {
  ...
  const fnResetSummary = (messageDesc) => {
    const summaryDesc = [
      {
        label: intl.formatMessage({ id: 'eightD.xiaoguoqueren', defaultMessage: '效果确认' }),
  ...
  return (
    <div>
      <PageHeaderWrapper
        tittle={intl.formatMessage({ id: 'eightD.8Dxiangqing', defaultMessage: '8D详情' })}
        items={TABLINK}
        // extra={<Button onClick={()=>{setVisible(true)}}>{intl.formatMessage({id: 'eightD.shenhe',defaultMessage: '审核'})}</Button>}
      >
