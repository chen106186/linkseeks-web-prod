  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8D/wantaSubmittedSrm/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
    ...
    },
    {
      ...
      render: (_text, record) => {
        return (
          <>
            <AuthButton type="custom" code="sumbit">
              <Button
                type="link"
                onClick={() => {
              ...
              overlay={
                <Menu>
                  {AuthUrl('edit') && (
                    <Menu.Item
                      onClick={() => {
                        history.push(`/qualityAbility/8D/wantaSubmittedSrm/edit?id=${record.id}&edit=true`)
                      }}
                    >
                      {intl.formatMessage({
                        id: 'eightD.bianji',
                  ...
                  {AuthUrl('detail') && (
                    <Menu.Item
                      onClick={() => {
                        history.push(`/qualityAbility/8D/wantaSubmittedSrm/detail?id=${record.id}`)
                      }}
                    >
                      {intl.formatMessage({
                        id: 'eightD.zhakan',
    ...
    <Space>
      <AuthButton type="add" code="add">
        <Button
          type="primary"
          onClick={() => {
            history.push('/qualityAbility/8D/wantaSubmittedSrm/add')
          }}
          icon={<PlusOutlined />}
        >
