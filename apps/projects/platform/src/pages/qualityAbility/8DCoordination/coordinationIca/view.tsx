  ...
  const columns: ColumnType<PostOrderEightDRectificationListResponseDetail>[] = [
    {
      ...
      render: (text, record) => {
        return AuthUrl('detail') ? (
          <Link to={`/qualityAbility/8DCoordination/coordinationIca/detail?id=${record.id}`}>{text}</Link>
        ) : (
          text
        )
      },
    ...
    },
    {
      ...
      render: (_text, record) => {
        return (
          <AuthButton type="custom" code="feekback">
            <Button
              type="link"
              onClick={() => {
                history.push(`/qualityAbility/8DCoordination/coordinationIca/detail?id=${record.id}`)
              }}
            >
              <span>
                {intl.formatMessage({
                  id: 'eightD.ICAfankui',
                  defaultMessage: 'ICA反馈',
                })}
              </span>
            </Button>
          </AuthButton>
