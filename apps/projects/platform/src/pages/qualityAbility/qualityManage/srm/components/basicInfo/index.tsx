...
const BasicInfoLaout: React.FC<BasicInfoLaoutProps> = () => {
  ...
  const handleFetchData = useCallback((params: any) => {
    return new Promise((resolve) => {
      postMemberManageLowerProviderPage({ ...params }, { ctlType: 'none' })
        .then((res) => {
          if (res.code !== 1000) {
            return
          }
      ...
      <Row gutter={[16, 36]}>
        <Col span={12}>
          ...
          <Form.Item
            ...
            name="vendorRoleId"
            label={intl.formatMessage({
              id: 'quality.gongyingshanghuiyuanjueseId',
              defaultMessage: '供应商会员角色Id',
            })}
            rules={[{ required: true }]}
          >
            <Input
        ...
        <Col span={12}>
          ...
          >
            {dataSource?.orderResource !== ORDERRESOURCE.SEND ? (
              ...
              <Button target="_blank" type="link" href={``}>
                {dataSource?.receiveNo}
              </Button>
            )}
          </Form.Item>
