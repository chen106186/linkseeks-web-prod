...
import { getProductMaterielGetDoesNotFreezeMaterielList, getProductSelectGetSelectBrand } from '@apps/apis'
...
import { ODR_TWO, ORDERRESOURCE } from '../../add'
...
const QualityMaterialLayout: React.FC<QualityMaterialLayoutProps> = () => {
    ...
    (params: any) => {
      return new Promise((resolve) => {
        getProductMaterielGetDoesNotFreezeMaterielList(
          {
            ...params,
            materielIdList: (_dataSource || []).map((_item) => {
              return _item.orderProductId
            }),
          },
          { ctlType: 'none' },
        )
  ...
  const handleSubmit = (_selectRow: number[] | string[], selectedRows: Record<string, any>[]) => {
    const fieldsValue = form.getFieldValue('qualityOrderProductVOS') || []
    const nextList = fieldsValue.concat(
      ...selectedRows.map((_item) => {
        return {
          orderProductId: _item?.id,
          skuId: _item?.code,
          productName: _item?.name,
          type: _item?.type,
          category: _item?.customerCategory?.name,
          categoryId: _item?.customerCategory?.id,
          brand: _item?.brand?.name,
          unit: _item?.unitName,
        }
      }),
    )
  ...
  return (
    <CardLayout
      id="qualityMaterial"
      title={intl.formatMessage({ id: 'quality.zhijianwuliao', defaultMessage: '质检物料' })}
    ...
    >
            ...
            validator: async (_, qualityOrderProductVOS) => {
              if (!qualityOrderProductVOS || isEmpty(qualityOrderProductVOS)) {
                return Promise.reject(
                  new Error(
                    intl.formatMessage({
                      id: 'quality.qingtianjiazhijianwuliao',
                      defaultMessage: '请添加质检物料',
                    }),
                  ),
                )
              }
            ...
            {PATH !== 'formed' &&
              ...
              dataSource?.orderResource !== ODR_TWO && (
                <Button block type="dashed" icon={<PlusOutlined />} onClick={handleSelect}>
                  {intl.formatMessage({
                    id: 'quality.xuanzezhijianwuliao',
                    defaultMessage: '选择质检物料',
                  })}
                </Button>
              )}
            <Table
      ...
      <TableModal
        ...
        mode="checkbox"
        customKey="id"
        title={intl.formatMessage({ id: 'quality.xuanzewuliao', defaultMessage: '选择物料' })}
        schema={{
          type: 'object',
          properties: {
            megalayout: {
        ...
        columns={[
          ...
          {
            title: intl.formatMessage({ id: 'quality.wuliaozu', defaultMessage: '物料组' }),
            key: 'materialGroup',
