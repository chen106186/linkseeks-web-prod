...
import { getOrderQualityGetSrmQualityOrderPage, postOrderQualityDeleteById, postOrderQualitySubmit } from '@apps/apis'
            ...
            overlay={() => (
              <Menu>
                {AuthUrl('edit') && (
                  <Menu.Item key="1">
                    {' '}
                    <Button type="link" href={`srm/edit?id=${record?.id}`}>
        ...
        <Space direction="horizontal" size={16}>
          <AuthButton type="add" code="add">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              onClick={() => history.push('/qualityAbility/qualityManage/srm/add')}
            >
...
export default QualityManageSrmTable
