...
import { complaintAddSchemaForSupplier } from './schema'
...
import { memberColumnsForSupplier, userColumns } from '../../complaintsAndSuggests/common/columns'
...
import {
  ...
  GetMemberSupplierAbilityInfoPageRequest,
  GetMemberSupplierAbilityInfoPageResponse,
...
const SuggestAdd = () => {
  ...
  useEffect(() => {
    ...
    const { name, memberId, roleId, byUserName, byUserPhone, byUserId, ...rest } =
      initialValue as unknown as GetMemberSupplierComplaintSubGetResponse
    setMemberModalValue([
      {
        name,
        memberId,
  ...
  const formatedValue = useMemo(() => {
    ...
    const { byUserName, byUserPhone, ...rest } = initialValue as unknown as GetMemberSupplierComplaintSubGetResponse
    return {
      ...rest,
  ...
  const handleFetchData = useCallback(async (params: GetMemberSupplierAbilityInfoPageRequest) => {
    const { data, code } = await getMemberSupplierAbilityInfoPage(params)
    if (code === 1000) {
      return data
    }
  ...
  const handleOnOk = (
    ...
    selectRowRecord: GetMemberSupplierAbilityInfoPageResponse['data'],
  ) => {
    ...
    >
      <Card>
        <NiceForm
          ...
          editable={true}
          value={formatedValue}
          schema={complaintAddSchemaForSupplier}
          actions={formActions}
          components={{
      ...
      <TableModal
        ...
        title={translate('web.resource.member.xuanzecaigoushang')}
        columns={memberColumnsForSupplier}
        schema={memberSchema}
        onOk={handleOnOk}
