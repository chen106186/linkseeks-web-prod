...
import { getMemberSupplierComplaintSubPage, GetMemberSupplierComplaintSubPageRequest } from '@apps/apis'
import moment from 'moment'

/** 投诉建议 > 我收到的投诉建议 */
const MyReceivedComplaints: React.FC<any> = (props) => {
  ...
  const { columns } = useColumns(queryColumns, [
    {
      ...
      render: (text, record) => {
        if (record.handle) {
          return (
            <DetailAuthButton>
              <Link to={`${pathname}/detail?id=${record.id}&isSupplier=1`}>
                {intl.formatMessage({
