...
import {
  ...
  PostMemberSupplierLifecycleArchivesManagementPageRequest,
  PostMemberSupplierLifecycleArchivesManagementPageResponse,
  getMemberSupplierInspectUsers,
  GetMemberSupplierInspectUsersRequest,
  GetMemberSupplierInspectUsersResponse,
  postMemberSupplierComplaintUpperAdd,
  postMemberSupplierComplaintUpperUpdate,
} from '@apps/apis'
...
const SuggestAdd = () => {
  ...
  const { initialValue } = useInitialValue(getMemberSupplierComplaintUpperGet, params)
  ...
  useEffect(() => {
    ...
    const { name, subMemberId, subRoleId, byUserName, byUserPhone, byUserId, ...rest } =
      initialValue as unknown as GetMemberSupplierComplaintUpperGetResponse
    setMemberModalValue([
      {
        name: name,
  ...
  const handleSubmit = async (value: SubmitType) => {
    ...
    const service = !isEdit ? postMemberSupplierComplaintUpperAdd : postMemberSupplierComplaintUpperUpdate
    const tempPostData = {
  ...
  const handleFetchData = useCallback(async (params: PostMemberSupplierLifecycleArchivesManagementPageRequest) => {
    const { data, code } = await postMemberSupplierLifecycleArchivesManagementPage(params, { ctlType: 'none' })
    if (code === 1000) {
      return data
    }
  ...
  const handleOnOk = (
    ...
    selectRowRecord: PostMemberSupplierLifecycleArchivesManagementPageResponse['data'],
  ) => {
