...
const BookmarkItem: React.FC<IProps> = (props: IProps) => {
  const { value, icon, title, visible = true } = props
  ...
  return visible ? (
    ...
    >
      <div
        ...
        onClick={handleClick}
      >
        {icon ? <div className={styles['bookmark-item-left']}>{icon}</div> : null}
        <div className={styles['bookmark-item-center']}>{title}</div>
      </div>
    </li>
  ) : null
}
