...
import // getMemberSupplierAbilityMaintenanceDetailBasicChannel,
// GetMemberSupplierAbilityMaintenanceDetailBasicChannelResponse,
// postMemberSupplierAbilityMaintenanceDetailBasicChannelUpdate,
'@apps/apis'
...
const MemberChannelInfo: React.FC<ChannelInfoProps> = (props: ChannelInfoProps) => {
  ...
  const [channelInfo, setChannelInfo] = useState<GetMemberSupplierAbilityMaintenanceDetailBasicChannelResponse>()
  ...
  const handleModifyChannelInfo = () => {
    ...
    // setInfoLoading(true)
    // getMemberSupplierAbilityMaintenanceDetailBasicChannel({
    //   validateId,
    // })
    //   .then((res) => {
    //     if (res.code === 1000) {
  ...
  const handleSubmit = (value: ValueType) => {
    ...
    //   content: intl.formatMessage({ id: 'member.components.MemberChannelInfo.edit.message' }),
    //   duration: 0,
    // })
    // postMemberSupplierAbilityMaintenanceDetailBasicChannelUpdate(payload, {
    //   timeout: 0,
    // })
    //   .then((res) => {
    //     if (res.code !== 1000) {
  ...
  return (
    <MellowCard
      title={intl.formatMessage({ id: 'member.components.MemberChannelInfo.title' })}
      {...rest}
        ...
        <>
          {validateId && (
            <Button type="link" loading={infoLoading} onClick={handleModifyChannelInfo}>
              {intl.formatMessage({ id: 'member.components.MemberChannelInfo.edit' })}
            </Button>
          )}
        </>
      }
      ...
      <Row gutter={16}>
        <Col span={8}>
          <Descriptions column={1}>
            <Descriptions.Item
              label={intl.formatMessage({ id: 'member.components.MemberChannelInfo.level' })}
              labelStyle={{ width: 106 }}
            >
              {dataSource.level || ''}
            </Descriptions.Item>
            <Descriptions.Item
              label={intl.formatMessage({ id: 'member.components.MemberChannelInfo.type' })}
              labelStyle={{ width: 106 }}
            >
              {dataSource.type || ''}
            </Descriptions.Item>
            <Descriptions.Item
              label={intl.formatMessage({ id: 'member.components.MemberChannelInfo.desc' })}
              labelStyle={{ width: 106 }}
            >
              {dataSource.desc || ''}
            </Descriptions.Item>
        ...
        <Col span={16}>
          <Descriptions column={1}>
            <Descriptions.Item
              label={intl.formatMessage({ id: 'member.components.MemberChannelInfo.areas' })}
              labelStyle={{ width: 106 }}
            >
