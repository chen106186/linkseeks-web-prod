...
interface TreeSelectField_ extends TreeSelectProps {
  /**
   * 禁用项keys
   */
  disabledKeys: DisbledKeysType
...
const TreeSelectField: React.FC<any> | { isFieldComponent: boolean } = (props) => {
  ...
  const index = Number(path.match(/\d/gi)[0])

  const xComponentProps: TreeSelectField_ = props.props['x-component-props'] || {}
