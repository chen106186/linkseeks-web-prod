...
const MemberOrderEvaluation: React.FC<IProps> = (props: IProps) => {
  ...
  const evaluateColumns: EditableColumns[] = [
    ...
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateColumns.last7days' }),
      dataIndex: 'last7days',
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateColumns.last30days' }),
      dataIndex: 'last30days',
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateColumns.last180days' }),
      dataIndex: 'last180days',
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateColumns.before180days' }),
      ...
      case 2: {
        node = (
          <>
            ...
            <span>{intl.formatMessage({ id: 'member.components.MemberEvaluation.sad' })}</span>
          </>
        )
        break
      }
      case 3: {
        node = (
          <>
            ...
            <span>{intl.formatMessage({ id: 'member.components.MemberEvaluation.notBad' })}</span>
          </>
        )
        break
      }
      ...
      case 5: {
        node = (
          <>
            ...
            <span>{intl.formatMessage({ id: 'member.components.MemberEvaluation.smile' })}</span>
          </>
        )
        break
      }
  ...
  const evaluateRecordColumns: EditableColumns[] = [
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.star' }),
      dataIndex: 'star',
      filters: [
        {
          text: intl.formatMessage({ id: 'member.components.MemberEvaluation.sad' }),
          value: 1,
        },
        {
          text: intl.formatMessage({ id: 'member.components.MemberEvaluation.notBad' }),
          value: 2,
        },
        {
          text: intl.formatMessage({ id: 'member.components.MemberEvaluation.smile' }),
          value: 3,
        },
      ],
    ...
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.comment' }),
      dataIndex: 'comment',
      ellipsis: true,
    },
    {
      title: `${intl.formatMessage({
        id: 'member.components.MemberEvaluation.evaluateRecordColumns.product',
      })}/${intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.purchaseCount' })}`,
      dataIndex: 'product',
      ellipsis: true,
      render: (text, record) => `${text}/${record.purchaseCount}`,
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.byMemberName' }),
      dataIndex: 'byMemberName',
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.dealTime' }),
      dataIndex: 'dealTime',
    },
    {
      title: intl.formatMessage({ id: 'member.components.MemberEvaluation.evaluateRecordColumns.orderNo' }),
      dataIndex: 'orderNo',
    },
  ]
  ...
  const getSummaryEvaluatePie = (data: EstimateSumItems[]) => {
    ...
    const ret = [
      {
        x: `${intl.formatMessage({ id: 'member.components.MemberEvaluation.smile' })}  ${
          count > 0 ? ((good / count) * 100).toFixed(2) : '0'
        }%`,
        y: good,
      },
      {
        x: `${intl.formatMessage({ id: 'member.components.MemberEvaluation.notBad' })}  ${
          count > 0 ? ((notBad / count) * 100).toFixed(2) : 0
        }%`,
        y: notBad,
      },
      {
        x: `${intl.formatMessage({ id: 'member.components.MemberEvaluation.sad' })}  ${
  ...
  const options = [
    {
      label: intl.formatMessage({ id: 'member.components.MemberEvaluation.options.analysis' }),
      value: 'analysis',
    },
    {
      label: intl.formatMessage({ id: 'member.components.MemberEvaluation.options.records' }),
      value: 'records',
    },
  ]
        ...
        <Spin spinning={!!loading}>
          <Row gutter={24}>
            <Col flex="386px">
              <Pie
                hasLegend
                subTitle={intl.formatMessage({ id: 'member.components.MemberEvaluation.pie.subTitle' })}
