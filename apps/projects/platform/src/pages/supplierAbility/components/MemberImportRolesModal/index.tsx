...
import { getMemberSupplierAbilitySubExcelRole } from '@apps/apis'
...
interface MemberImportRolesModalProps {
  /**
   * 是否可见
   */
  visible: boolean
...
const MemberImportRolesModal: React.FC<MemberImportRolesModalProps> = (props) => {
  ...
  const fetchRolesList = () => {
    setListLoading(true)
    getMemberSupplierAbilitySubExcelRole()
      .then((res) => {
        if (res.code === 1000) {
          setList(res.data)
        }
  ...
  return (
    <Modal
      title={intl.formatMessage({ id: 'supplier.insteadRegister.import.title', defaultMessage: '选择导入角色' })}
      cancelText={intl.formatMessage({ id: 'common.button.cancel', defaultMessage: '取消' })}
      okText={intl.formatMessage({ id: 'supplier.insteadRegister.import.next', defaultMessage: '下一步' })}
      visible={visible}
    ...
    >
      <Spin spinning={listLoading}>
        <Radio.Group value={internalValue} className={styles['member-roles']}>
          {options.map((item) => (
            <div key={item.value} className={styles['member-roles-item']} onClick={() => handleSelectItem(item.value)}>
...
export default MemberImportRolesModal
