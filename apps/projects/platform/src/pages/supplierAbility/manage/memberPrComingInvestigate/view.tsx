...
import { getMemberSupplierDepositInspectPage, getMemberSupplierDepositPageConditions } from '@apps/apis'
...
const MemberPrComingInvestigate: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrComingInvestigate/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/supplierAbility/manage/memberPrComingInvestigate/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierDepositInspectPage(payload)
