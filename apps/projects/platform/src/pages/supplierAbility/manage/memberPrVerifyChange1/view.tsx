...
import {
  getMemberSupplierModifyGradeOnePage,
  getMemberSupplierModifyPageConditions,
  postMemberSupplierModifyGradeOneBatch,
} from '@apps/apis'
...
const MemberPrVerifyChange1: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyChange1/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyChangeColumn('/supplierAbility/manage/memberPrVerifyChange1/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierModifyGradeOnePage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'supplier.management.supplierPrVerifyChange1.query.verify-tip' }),
      onOk() {
...
export default MemberPrVerifyChange1
