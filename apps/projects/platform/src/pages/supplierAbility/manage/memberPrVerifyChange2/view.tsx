...
import {
  getMemberSupplierModifyGradeTwoPage,
  postMemberSupplierModifyGradeTwoBatch,
...
const MemberPrVerifyChange2: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyChange2/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyChangeColumn('/supplierAbility/manage/memberPrVerifyChange2/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierModifyGradeTwoPage(payload)
