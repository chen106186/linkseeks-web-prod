...
import {
  getMemberSupplierModifyConfirmPage,
  getMemberSupplierModifyPageConditions,
  postMemberSupplierModifyConfirmBatch,
} from '@apps/apis'
...
const MemberPrVerifyChangeConfrim: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyChangeConfirm/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyChangeColumn('/supplierAbility/manage/memberPrVerifyChangeConfirm/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierModifyConfirmPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'supplier.management.supplierPrVerifyChangeConfirm.query.verify-tip' }),
      onOk() {
  ...
  const fetchSearchItems = async () => {
    const res = await getMemberSupplierModifyPageConditions()
...
export default MemberPrVerifyChangeConfrim
