 ...
 * @LastEditTime: 2021-11-17 18:09:59
...
import {
  getMemberSupplierDepositGradeTwoPage,
  postMemberSupplierDepositGradeTwoBatch,
...
const MemberPrVerifyComing2: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyComing2/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/supplierAbility/manage/memberPrVerifyComing2/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierDepositGradeTwoPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'supplier.management.supplierPrVerifyComing1.query.verify-tip' }),
      onOk() {
...
export default MemberPrVerifyComing2
