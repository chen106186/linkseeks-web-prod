...
import {
  getMemberSupplierDepositConfirmPage,
  getMemberSupplierDepositPageConditions,
  postMemberSupplierDepositConfirmBatch,
} from '@apps/apis'
...
const MemberPrVerifyComingConfirm: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyComingConfirm/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/supplierAbility/manage/memberPrVerifyComingConfirm/detail').concat([
    {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierDepositConfirmPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'supplier.management.supplierPrVerifyComingConfirm.query.verify-tip' }),
      onOk() {
