...
import {
  getMemberSupplierDepositPageConditions,
  getMemberSupplierDepositQualifyPage,
  postMemberSupplierDepositQualifyBatch,
} from '@apps/apis'
...
const memberPrVerifyComingQualifications: React.FC<{}> = (props) => {
  ...
  const handleJumpAudit = (record) => {
    history.push(`/supplierAbility/manage/memberPrVerifyComingQualifications/edit?validateId=${record.validateId}`)
  }

  const defaultColumns = verifyComingColumn('/supplierAbility/manage/memberPrVerifyComingQualifications/detail').concat(
    [
      {
  ...
  const fetchListData = async (params: any) => {
    ...
    const res = await getMemberSupplierDepositQualifyPage(payload)
  ...
  const handleBatch = () => {
    ...
    confirm({
      ...
      content: intl.formatMessage({ id: 'supplier.management.supplierPrVerifyComingData.query.verify-tip' }),
      onOk() {
