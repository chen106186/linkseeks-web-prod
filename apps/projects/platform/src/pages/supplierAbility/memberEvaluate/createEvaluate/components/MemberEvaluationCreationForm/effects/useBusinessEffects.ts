...
import { AssessmentProjectCtlValueType } from '../../EvaluationsAssessmentProjectCtlField'
...
export const useBusinessEffects = (context, actions: ISchemaFormActions | ISchemaFormAsyncActions) => {
  ...
  onFieldValueChange$('subMember').subscribe((state) => {
    ...
    // 联动 变更申请单摘要
    const changeRequestSummaryValue = getFieldValue('changeRequestSummary')
    const targetLifecycleStageName = getFieldValue('targetLifecycleStageName')
    if (member && !changeRequestSummaryValue && targetLifecycleStageName) {
      setFieldValue(
  ...
  onFieldInputChange$('targetLifecycleStageId').subscribe((state) => {
    ...
    if (!changeRequestSummaryValue && member) {
      setFieldValue(
        'changeRequestSummary',
