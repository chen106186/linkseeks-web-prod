...
import {
  getMemberSupplierAppraisalWaitPublishGet,
  postMemberSupplierAppraisalWaitPublishAdd,
  postMemberSupplierAppraisalWaitPublishUpdate,
} from '@apps/apis'
...
const MemberEvaluationCreation: React.FC<{}> = () => {
  ...
  const getMemberEvaluationDetail = async () => {
    const res = await getMemberSupplierAppraisalWaitPublishGet({ id })
    if (res.code === 1000) {
  ...
  const onMemberEvaluationCreationFormSubmit = (value: SubmitCallValueType): Promise<void> =>
    new Promise((resolve, reject) => {
      ...
      const requestFunction = id
        ? () => postMemberSupplierAppraisalWaitPublishUpdate({ id: +id, ...value })
        : () => postMemberSupplierAppraisalWaitPublishAdd(value)
      requestFunction()
        .then((res) => {
          if (res.code === 1000) {
            resolve()
