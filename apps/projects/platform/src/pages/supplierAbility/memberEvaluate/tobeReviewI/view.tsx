...
import {
  getMemberSupplierAppraisalWaitAuditOnePage,
  ...
  postMemberSupplierAppraisalWaitAuditOneAuditBatch,
} from '@apps/apis'
...
const defaultColumns = setColumnsByLinks({
  detail: '/supplierAbility/memberEvaluate/tobeReviewI/detail',
})
...
const List: React.FC<Iprops> = (props: Iprops) => {
    ...
    [
      {
        ...
        render: (_text, _record) => (
          <DetailAuthButton>
            <Space>
              <Link to={`/supplierAbility/memberEvaluate/tobeReviewI/detail?id=${_record.id}`}>
                {intl.formatMessage({
    ...
    async (ids: number[]) => {
      if (ids.length) {
        try {
          ...
          const { code, data } = await postMemberSupplierAppraisalWaitAuditOneAuditBatch({ idList: ids })
          if (code === 1000) {
  ...
  const handleFetch = useCallback(async (params: GetMemberAppraisalWaitAuditOnePageRequest) => {
    const result = fetchListData(getMemberSupplierAppraisalWaitAuditOnePage, params)
    return result
  }, [])
