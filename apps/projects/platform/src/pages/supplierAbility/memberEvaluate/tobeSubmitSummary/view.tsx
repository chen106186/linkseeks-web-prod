...
import {
  getMemberSupplierAppraisalWaitSubmitPage,
...
const defaultColumns = setColumnsByLinks(
  {
    detail: '/supplierAbility/memberEvaluate/tobeSubmitSummary/detail',
  },
  ['totalScore'],
)
...
const List: React.FC<Iprops> = (props: Iprops) => {
    ...
    [
      {
        ...
        render: (_text, _record) =>
          _record.submitOrUpdate && (
            <DetailAuthButton>
              <Space>
                <Link to={`/supplierAbility/memberEvaluate/tobeSubmitSummary/detail?id=${_record.id}`}>
                  {intl.formatMessage({
                    id: 'member.memberEvaluate.components.FormilySelectMember.index.submit',
                  })}
                </Link>
                {/* <Link to={`/supplierAbility/memberEvaluate/tobeSubmitSummary/detail?id=${_record.id}`}>{ intl.formatMessage({ id: 'member.memberEvaluate.createEvaluate.index.edit'}) }</Link> */}
              </Space>
            </DetailAuthButton>
          ),
      },
  ...
  const handleFetch = async (params: GetMemberAppraisalWaitSubmitPageRequest) => {
    const result = fetchListData(getMemberSupplierAppraisalWaitSubmitPage, params)
    return result
  }
