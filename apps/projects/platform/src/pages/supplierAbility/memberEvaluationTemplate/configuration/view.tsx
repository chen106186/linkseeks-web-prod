...
import {
  ...
  postMemberSupplierScoringTemplateDelete,
  postMemberSupplierScoringTemplateStartOrStop,
  GetMemberSupplierScoringTemplatePageRequest,
} from '@apps/apis'
...
const MemberEvaluationTemplateConfiguration: React.FC<any> = () => {
  ...
  const defaultColumns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (state, template) => (
        <Popconfirm
          title={`是否${template.state ^ 1 ? '启用' : '停用'}评分模板：${template.templateName}？`}
          icon={
            state ? (
  ...
  /** 查询 评分模板配置列表 */
  const getEvaluationTemplates = async (params: GetMemberSupplierScoringTemplatePageRequest) => {
    try {
      const res = await getMemberSupplierScoringTemplatePage({ ...params })
      if (res.code === 1000) {
        return {
          data: res.data,
          totalCount: 0, // 接口没有分页
  ...
  const postEvaluationTemplateStateSwitching = async (template: any) => {
    setIsLoading(true)
    const res = await postMemberSupplierScoringTemplateStartOrStop(
      {
        id: template.id,
    ...
    if (res.code === 1000) {
      message.success(`已${template.state ^ 1 ? '启用' : '停用'}评分模板：${template.templateName}`)
    } else {
  ...
  const postEvaluationTemplateDeletion = async (template: any) => {
    setIsLoading(true)
    const res = await postMemberSupplierScoringTemplateDelete({ id: template.id }, { ctlType: 'none' })
    if (res.code === 1000) {
      message.success(`已成功删除评分模板：${template.templateName}`)
    }
    ref.current.reload()
  }
