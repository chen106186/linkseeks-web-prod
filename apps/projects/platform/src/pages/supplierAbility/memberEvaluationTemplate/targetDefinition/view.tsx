...
import { EditAuthButton } from '@apps/components'
...
import { getMemberSupplierScoringIndicatorPage, postMemberSupplierScoringIndicatorSubmit } from '@apps/apis'
...
const MemberEvaluationTargetDefinition: React.FC<any> = () => {
  ...
  const getScoringIndicatorSubmitList = async () => {
    setIsSpinning(true)
    const res = await getMemberSupplierScoringIndicatorPage()
  ...
  const onFormSubmit = async (formData: I_FormData) => {
    try {
      ...
      const res = await postMemberSupplierScoringIndicatorSubmit(
        {
