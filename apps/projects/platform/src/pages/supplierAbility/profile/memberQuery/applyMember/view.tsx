 ...
 * @LastEditTime: 2021-12-04 18:08:24
 * @Description: 申请会员
 */
...
import {
  getMemberSupplierAbilityInfoApplyDepositDetail,
  getMemberSupplierAbilityInfoDepositDetail,
  postMemberSupplierAbilityInfoApply,
  postMemberSupplierAbilityInfoDepositDetailUpdate,
} from '@apps/apis'
...
const { onFormInputChange$, onFieldInputChange$ } = FormEffectHooks
const onStepNext$ = createEffectHook('onStepNext')
const onStepPrevious$ = createEffectHook('onStepPrevious')

const MemberQueryApplyMember: React.FC<any> = (props: any) => {
  ...
  const getDepositInfo = async () => {
    ...
    setLoading(true)
    const res = await getMemberSupplierAbilityInfoApplyDepositDetail({
  ...
  const getDepositInfoByValidateId = async () => {
    ...
    setLoading(true)
    const res = await getMemberSupplierAbilityInfoDepositDetail({
  ...
  const handleSubmit = (values: ValueType) => {
    ...
    const msg = message.loading({
      content: intl.formatMessage({ id: 'member.memberQuery.applyMember.save.message' }),
      duration: 0,
    })
  ...
  return (
    <Spin spinning={loading || agreementLoading}>
      <PageHeaderWrapper
        ...
        title={`${
          !validateId
            ? intl.formatMessage({ id: 'supplier.supplierQuery.applysupplier.apply' })
            : pathname.includes('changeMember')
            ? intl.formatMessage({ id: 'supplier.supplierQuery.applysupplier.change' })
            : intl.formatMessage({ id: 'supplier.supplierQuery.applysupplier.edit' })
        }`}
        extra={
          <Space>
            {currenStep === 0 && !validateId ? (
              ...
              >
                {`${intl.formatMessage({ id: 'member.memberQuery.applyMember.agress' })} ${
            ...
            {currenStep === 1 ? (
              <>
                {!validateId && (
                  ...
                  >
                    {intl.formatMessage({ id: 'supplier.supplierQuery.applysupplier.step1.prev' })}
                  </Button>
                )}
                <Button
                  type="primary"
                ...
                >
                  {intl.formatMessage({ id: 'member.memberQuery.applyMember.step1.next' })}
                </Button>
              </>
            ) : null}
            {currenStep === 2 ? (
              <>
                ...
                {!validateId && (
                  ...
                  >
                    {intl.formatMessage({ id: 'member.memberQuery.applyMember.step2.prev' })}
                  </Button>
                )}
                <Button
                  type="primary"
                ...
                >
                  {intl.formatMessage({ id: 'member.memberQuery.applyMember.step2.next' })}
                </Button>
              </>
            ) : null}
            {currenStep === 3 ? (
              <>
                ...
                >
                  {intl.formatMessage({ id: 'member.memberQuery.applyMember.step3.prev' })}
                </Button>
                <Button
                  type="primary"
                ...
                >
                  {intl.formatMessage({ id: 'member.memberQuery.applyMember.step3.next' })}
                </Button>
              </>
            ) : null}
          </Space>
        ...
        <MellowCard bodyStyle={{ padding: 0 }}>
          <NiceForm
            ...
            effects={($, actions) => {
              ...
              onFieldInputChange$('step4.qualities').subscribe((state) => {
                const { value } = state
                if (isArray(value)) {
                  actions.setFieldInitialValue('step4.qualities', value)
                }
              })
