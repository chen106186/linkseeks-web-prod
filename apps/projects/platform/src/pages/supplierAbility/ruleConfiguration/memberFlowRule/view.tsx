...
import {
  getMemberSupplierProcessRulePage,
  postMemberSupplierProcessRuleDelete,
  postMemberSupplierProcessRuleUpdateStatus,
} from '@apps/apis'
...
const fetchListData = async (params: any) => {
  ...
  const res = await getMemberSupplierProcessRulePage(payload)
...
const MemberFlowRule: React.FC<any> = (props) => {
  ...
  const handleDelete = (id: number) => {
    const mesInstance = message.loading({
      content: intl.formatMessage({
        id: 'member.memberFlowRule.delete.message',
      }),
      duration: 0,
    })
    postMemberSupplierProcessRuleDelete({
  ...
  const handleModify = (id: number, status: number) => {
    const mesInstance = message.loading({
      content: intl.formatMessage({
        id: 'member.memberFlowRule.modify.message',
      }),
      duration: 0,
    })
    postMemberSupplierProcessRuleUpdateStatus({
  ...
  const defaultColumns: ColumnType<any>[] = [
    {
      title: intl.formatMessage({
        id: 'member.memberFlowRule.defaultColumns.id',
      }),
      dataIndex: 'id',
    },
    {
      title: intl.formatMessage({
        id: 'member.memberFlowRule.defaultColumns.ruleName',
      }),
      dataIndex: 'ruleName',
      render: (text, record) => (
        <>
    ...
    {
      title: intl.formatMessage({
        id: 'supplier.supplierFlowRule.defaultColumns.roleName',
      }),
      dataIndex: 'roleName',
    },
    {
      title: intl.formatMessage({
        id: 'member.memberFlowRule.defaultColumns.roleTypeName',
      }),
      dataIndex: 'roleTypeName',
    },
    {
      title: intl.formatMessage({
        id: 'supplier.supplierFlowRule.defaultColumns.supplierTypeName',
      }),
      dataIndex: 'memberTypeName',
    },
    {
      title: intl.formatMessage({
        id: 'member.memberFlowRule.defaultColumns.createTime',
      }),
      dataIndex: 'createTime',
    },
    {
      title: intl.formatMessage({
        id: 'member.memberFlowRule.defaultColumns.status',
      }),
      dataIndex: 'status',
      render: (_, record) => (
      ...
      render: (_, record: any) => (
        <>
          ...
          <AuthButton type="custom" code="delete">
            <Popconfirm
              title={intl.formatMessage({
                id: 'member.memberFlowRule.delete.tip',
              })}
              okText={intl.formatMessage({
                id: 'member.memberFlowRule.delete.yes',
              })}
              cancelText={intl.formatMessage({
                id: 'member.memberFlowRule.delete.no',
              })}
