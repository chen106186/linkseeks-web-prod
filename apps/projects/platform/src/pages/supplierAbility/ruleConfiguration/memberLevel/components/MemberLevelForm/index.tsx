...
import { getMemberSupplierAbilityLevelRolePage, getMemberSupplierAbilityLevelTypes } from '@apps/apis'
...
const MemberLevelForm: React.FC<MemberLevelFormProps> = (props) => {
  ...
  const fetchMemberApplicableRole = (): MemberApplicableRoleProps['fetchDataSource'] => async (params) => {
    const res = await getMemberSupplierAbilityLevelRolePage({
      ...params,
        ...
        <NiceForm
          ...
          effects={($, actions) => {
            ...
            useAsyncSelect('levelType', async () => {
              const { data, code } = await getMemberSupplierAbilityLevelTypes()
              if (code === 1000) {
