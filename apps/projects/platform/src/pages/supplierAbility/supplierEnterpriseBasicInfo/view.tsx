...
const MemberQuery: React.FC<{}> = () => {
  ...
  const handleJumpChangeDeposit = (record) => {
    history.push(`/supplierAbility/supplierEnterpriseBasicInfo/inventoryData/change?validateId=${record.validateId}`)
  }

  // 修改注册信息页面
  const handleJumpUpdate = (record) => {
    history.push(`/supplierAbility/supplierEnterpriseBasicInfo/updateRegistrationInfo?validateId=${record.validateId}`)
  }

  const defaultColumns: ColumnType<GetMemberSupplierAbilityInfoPageResponseDetail>[] = [
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.ID', defaultMessage: '供应商ID' }),
      dataIndex: 'memberId',
    },
    {
      ...
      render: (text, record) => (
        <EyeAuthButton
          ...
          url={`/supplierAbility/supplierEnterpriseBasicInfo/detail?validateId=${record.validateId}`}
        >
          {text}
        </EyeAuthButton>
      ),
    ...
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.memberRole', defaultMessage: '会员角色' }),
      dataIndex: 'roleName',
    },
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.applicationTime', defaultMessage: '申请时间' }),
      dataIndex: 'registerTime',
    },
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.warehousingTime', defaultMessage: '入库时间' }),
      dataIndex: 'depositTime',
    },
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.supplierCode', defaultMessage: '供应商编码' }),
      dataIndex: 'code',
    },
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.lifeCycleStage', defaultMessage: '生命周期阶段' }),
      dataIndex: 'lifecycleStagesName',
    },
    {
      title: intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.approvalStatus', defaultMessage: '审核状态' }),
          ...
          {record.levelTypeEnum === MEMBER_LEVEL_TYPE_PLATFORM && record.showUpdate && (
            <AuthButton type="custom" code="updateRegistrationInfo">
              <Button type="link" onClick={() => handleJumpUpdate(record)}>
                {intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.edit' })}
              </Button>
            </AuthButton>
          )}
          {/* 上级是非平台会员，且外部审核状态为不通过才可以 修改入库资料 */}
          {record.levelTypeEnum !== MEMBER_LEVEL_TYPE_PLATFORM && record.showModify && (
            <AuthButton type="custom" code="inventoryData/change">
              <Button type="link" onClick={() => handleJumpChangeDeposit(record)}>
                {intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.change' })}
              </Button>
            </AuthButton>
          )}
        </>
  ...
  const handleSubmit = (values) => {
    history.push(`/supplierAbility/supplierEnterpriseBasicInfo/add`, {
      query: {
        ...values,
      },
    })
    ...
    <Space>
      <AddAuthButton>
        <Button type="primary" onClick={() => setModalVisible(true)}>
          <PlusOutlined />
          {intl.formatMessage({ id: 'supplier.enterpriseBasicInfo.addMemberRole' })}
        </Button>
      </AddAuthButton>
    </Space>
  )
