...
import {
  GetMemberSupplierLifecycleWaitSubmitPageResponseDetail,
  getMemberSupplierLifecycleWaitSubmitPage,
} from '@apps/apis'
...
const SupplierAggregateRatingsIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberSupplierLifecycleWaitSubmitPageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberSupplierLifecycleWaitSubmitPageResponseDetail>(pathname).concat({
    ...
    render: (text: any, record) => (
      <>
        <AuthButton type="custom" code="rating">
          <Button type="link" onClick={() => handleJumpVerify(record)}>
            {translate('web.resource.member.huizongpinfenjieguo')}
          </Button>
        </AuthButton>
      </>
    ),
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberSupplierLifecycleWaitSubmitPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default SupplierAggregateRatingsIndex
