...
import {
  ...
  getMemberSupplierAbilityMaintenanceStatisticLifecycleCount,
  postMemberSupplierLifecycleArchivesManagementPage,
  getMemberSupplierLifecyclePageItems,
} from '@apps/apis'
...
const SupplierArchivesManagementIndex: React.FC<any> = (props) => {
  ...
  const defaultColumns: ColumnType<PostMemberSupplierAbilityMaintenancePageResponseDetail>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'supplier.profile.roleName', defaultMessage: '供应商角色' }),
      dataIndex: 'roleName',
    },
    {
      title: intl.formatMessage({ id: 'supplier.profile.registerTime', defaultMessage: '申请注册时间' }),
      dataIndex: 'registerTime',
    },
    {
      title: intl.formatMessage({ id: 'supplier.profile.depositTime', defaultMessage: '入库时间' }),
      dataIndex: 'depositTime',
    },
    {
      title: intl.formatMessage({ id: 'supplier.classify.code', defaultMessage: '供应商编码' }),
      dataIndex: 'memberCode',
    },
    {
      title: intl.formatMessage({ id: 'supplier.profile.lifecycle', defaultMessage: '生命周期阶段' }),
      ...
      render: (_, record) => (
        <>
          <AuthButton type="custom" code="modifies">
            <Link
              to={`/supplierAbility/supplierLifecycle/supplierModifiesCommit/add?subMemberId=${record.memberId}&subRoleId=${record.roleId}&subMemberName=${record.name}&lifeCycleStageName=${record.lifeCycleStageName}&lifeCycleStageId=${record.lifeCycleStageId}`}
            >
          ...
          <AuthButton type="custom" code="modifiesQuery">
            <Link to={`/supplierAbility/supplierLifecycle/supplierModifiesQuery?name=${record.name}`}>
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await postMemberSupplierLifecycleArchivesManagementPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
  ...
  const fetchSelectOptions = async () => {
    const res = await getMemberSupplierLifecyclePageItems()
  ...
  const fetchLifeCycleTabs = async () => {
    try {
      const { data, code } = await getMemberSupplierAbilityMaintenanceStatisticLifecycleCount({}, { ctlType: 'none' })
      if (code === 1000) {
...
export default SupplierArchivesManagementIndex
