...
import {
  GetMemberSupplierLifecycleWaitAuditOnePageResponseDetail,
  getMemberSupplierLifecycleWaitAuditOnePage,
} from '@apps/apis'
...
const SupplierModifiesGradeOneIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberSupplierLifecycleWaitAuditOnePageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberSupplierLifecycleWaitAuditOnePageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberSupplierLifecycleWaitAuditOnePage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default SupplierModifiesGradeOneIndex
