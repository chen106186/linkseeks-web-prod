...
import {
  GetMemberSupplierLifecycleWaitAuditTwoPageResponseDetail,
  getMemberSupplierLifecycleWaitAuditTwoPage,
} from '@apps/apis'
...
const SupplierModifiesGradeTwoIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberSupplierLifecycleWaitAuditTwoPageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberSupplierLifecycleWaitAuditTwoPageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberSupplierLifecycleWaitAuditTwoPage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default SupplierModifiesGradeTwoIndex
