...
import {
  GetMemberSupplierLifecycleSummaryPageResponseDetail,
  getMemberSupplierLifecycleSummaryPage,
  getMemberSupplierLifecycleStatusList,
} from '@apps/apis'
...
const SupplierModifiesQueryIndex: React.FC<any> = (props) => {
  ...
  const defaultColumns = modifiesColumn<GetMemberSupplierLifecycleSummaryPageResponseDetail>(pathname)
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberSupplierLifecycleSummaryPage(mergedPayload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default SupplierModifiesQueryIndex
