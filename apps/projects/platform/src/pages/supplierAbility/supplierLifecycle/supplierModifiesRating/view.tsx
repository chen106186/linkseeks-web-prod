...
import {
  getMemberSupplierLifecycleWaitGradePage,
  GetMemberSupplierLifecycleWaitGradePageResponseDetail,
} from '@apps/apis'
...
const SupplierModifiesRatingIndex: React.FC<any> = (props) => {
  const { pathname } = useLocation()

  const ref = useRef<any>({})
  const intl = useIntl()
  const translate = useWebIntl()
  const handleJumpVerify = (record: GetMemberSupplierLifecycleWaitGradePageResponseDetail) => {
    history.push(`${pathname}/edit?id=${record.id}`)
  }

  const defaultColumns = modifiesColumn<GetMemberSupplierLifecycleWaitGradePageResponseDetail>(pathname).concat({
  ...
  const fetchList = async (params: SearchFormValuesType) => {
    ...
    try {
      const res = await getMemberSupplierLifecycleWaitGradePage(payload, { ctlType: 'none' })
      if (res.code === 1000) {
        return res.data
      }
...
export default SupplierModifiesRatingIndex
