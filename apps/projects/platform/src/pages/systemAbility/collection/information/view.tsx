...
import {
  GetManageContentInformationCollectListResponseDetail,
...
const Information: React.FC = () => {
  const intl = useIntl()
  const [list, setList] = useState<GetManageContentInformationCollectListResponseDetail[]>([])
  const [current, setCurrent] = useState<number>(1)
  const [pageSize, setPageSize] = useState<number>(10)
  const [totalCount, setTotalCount] = useState<number>(0)
  const { shopLink } = useShopUrl({ shopType: 5 })
   ...
   * 获取收藏的资讯列表
   */
  const fetchCollectInformationList = async () => {
    const param: any = {
      current,
      pageSize,
    }

    getManageContentInformationCollectList(param).then((res) => {
      if (res.code === 1000) {
  ...
  const linkToDetail = (detail) => {
    if (detail.status === 2 && shopLink) {
      const el = document.createElement('a')
      el.href = `${shopLink}/info/infoDetail/${detail.id}`
      el.target = '_blank'
      el.id = detail.id
    ...
    Modal.confirm({
      ...
      onOk: () => {
        return new Promise((resolve, reject) => {
          ...
          postManageContentInformationCollect(param)
            .then((res) => {
              if (res.code === 1000) {
                // fetchPurchaseList()
                fetchCollectInformationList()
