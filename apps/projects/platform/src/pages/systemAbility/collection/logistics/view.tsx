...
import {
  GetCommodityWebMemberLogisticsWebCollectListResponseDetail,
  getCommodityWebMemberLogisticsWebCollectList,
...
const Logistics: React.FC = () => {
  const intl = useIntl()
  const [list, setList] = useState<GetCommodityWebMemberLogisticsWebCollectListResponseDetail[]>([])
  const [current, setCurrent] = useState<number>(1)
  const [pageSize, setPageSize] = useState<number>(10)
  const [totalCount, setTotalCount] = useState<number>(0)
  const { shopLink } = useShopUrl({ shopType: 3 })
  ...
  const fetchCollectList = async () => {
    ...
    getCommodityWebMemberLogisticsWebCollectList(param).then((res) => {
      if (res.code === 1000) {
  ...
  const linkToDetail = (detail) => {
    if (detail.status !== 0 && shopLink) {
      ...
      message.destroy()
      message.info(intl.formatMessage({ id: 'systemSetting.collection.logisticsProvidersFrozen' }))
    }
  }
    ...
    Modal.confirm({
      ...
      onOk: () => {
        return new Promise((resolve, reject) => {
          ...
          postCommodityWebMemberLogisticsWebCollect(param)
            .then((res) => {
              if (res.code === 1000) {
                // fetchPurchaseList()
                fetchCollectList()
