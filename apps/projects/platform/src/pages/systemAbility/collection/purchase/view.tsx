...
import {
  GetCommodityWebMemberPurchaseWebCollectListResponseDetail,
  getCommodityWebMemberPurchaseWebCollectList,
  postCommodityWebMemberPurchaseWebCollect,
} from '@apps/apis'
...
const Purchase: React.FC = () => {
  const intl = useIntl()
  const [list, setList] = useState<GetCommodityWebMemberPurchaseWebCollectListResponseDetail[]>([])
  const [current, setCurrent] = useState<number>(1)
  const [pageSize, setPageSize] = useState<number>(10)
  const [totalCount, setTotalCount] = useState<number>(0)
  const { shopLink } = useShopUrl({ shopType: 2 })
   ...
   * 获取收藏的资讯列表
   */
  const fetchCollectPurchaseList = () => {
    const param: any = {
      current,
      pageSize,
    }

    getCommodityWebMemberPurchaseWebCollectList(param).then((res) => {
      if (res.code === 1000) {
  ...
  const linkToDetail = (detail) => {
    if (detail.status !== 0 && shopLink) {
      ...
      el.href = `${shopLink}/shopIndex/${detail.id}`
      el.target = '_blank'
      el.id = detail.id
      ...
      message.destroy()
      message.info(intl.formatMessage({ id: 'systemSetting.collection.doorFrozen' }))
    }
  }
    ...
    Modal.confirm({
      ...
      onOk: () => {
        return new Promise((resolve, reject) => {
          ...
          postCommodityWebMemberPurchaseWebCollect(param)
            .then((res) => {
              if (res.code === 1000) {
                // fetchPurchaseList()
                fetchCollectPurchaseList()
