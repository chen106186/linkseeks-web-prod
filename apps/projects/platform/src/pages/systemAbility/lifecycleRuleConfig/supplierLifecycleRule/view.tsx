...
import { postMemberSupplierLifecycleRuleAdd, postMemberSupplierLifecycleRuleGet } from '@apps/apis'
...
const LifecycleRuleConfig = () => {
  ...
  const getLifecycleRuleConfig = async () => {
    setSpinning(true)
    const { code, data, message: msg } = await postMemberSupplierLifecycleRuleGet({}, { ctlType: 'none' })
    if (code === 1000 && data) {
      // 整理出前端自用的 lifecycleKey
      // 查询出来的数据直接使用 lifecycleStagesId 来生成 lifecycleKey
      // 前端新增时的生命周期通过当前时间戳来生成 lifecycleKey
      const lifecycle =
  ...
  const handleSubmit = () => {
    form.validateFields().then((values) => {
      ...
      setLoading(true)
      postMemberSupplierLifecycleRuleAdd(params)
        .then(({ code }) => {
          if (code === 1000) {
            // 刷新配置数据
    ...
    <>
      <PageHeaderWrapper
        ...
        title={intl.formatMessage({
          id: 'lifecycle.supplierLifecycleRuleConfig',
          defaultMessage: '供应商生命周期规则配置',
        })}
        extra={
        ...
        <Spin spinning={spinning}>
          <Form
            ...
            onValuesChange={(changedValues: any, values: any) => {
              ...
              switch (changeKey) {
                ...
                case 'lifecycle':
                  // 操作生命周期配置时
                  // 当已选择的 初始阶段设置 的 lifecycleKey 不在生命周期配置列表里（即可能被删除）
                  // 则清除 初始阶段配置 的相关信息
          ...
          >
            <Form.Item name={'lifecycle'} initialValue={[]} style={{ marginBottom: 0 }}>
              <LifecycleSortList ref={lifecycleRef} lifecycleType="SUPPLIER" />
            </Form.Item>
            <BaseInfo
              className="mt-16"
              cols={2}
            ...
            >
              <Row gutter={[0, 16]}>
                <Col span={24} style={{ color: '#91959B' }}>
                  {intl.formatMessage({
                    id: 'lifecycle.supplierInitialStageSettingTips',
                    defaultMessage: '供应商入库申请审核通过后，供应商的生命周期阶段',
                  })}
                </Col>
                <Col span={24}>
                  <Form.Item
