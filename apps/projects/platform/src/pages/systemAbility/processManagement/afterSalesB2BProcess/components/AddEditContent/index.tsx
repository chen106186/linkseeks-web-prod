/**
 * 系统能力 - 流程引擎 - 售后(B2B)流程规则配置 新增/编辑/详情公共组件
 * @author: Crayon
 */
...
import {
  getOrderTradeProcessAfterSaleB2bBaseList,
  getOrderTradeProcessAfterSaleGet,
  postOrderTradeProcessAfterSaleB2bSave,
  postOrderTradeProcessAfterSaleB2bUpdate,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, btnCode, title, pageType }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      // 基本的流程规则修改或新增
      let requestApi =
        pageType === PAGE_TYPE.EDIT ? postOrderTradeProcessAfterSaleB2bUpdate : postOrderTradeProcessAfterSaleB2bSave
  ...
  const getProcessBaseList = async (params: any = {}) => {
    const { code, data } = await getOrderTradeProcessAfterSaleB2bBaseList(params)
    if (code === 1000) {
  ...
  const getDetail = async () => {
    if (processId) {
      const { code, data } = await getOrderTradeProcessAfterSaleGet({ processId })
      if (code === 1000) {
