/**
 * 系统能力 - 流程引擎 - 售后(SRM)流程规则配置 新增/编辑/详情公共组件
 * @author: Crayon
 */
...
import {
  getOrderPurchaseProcessAfterSaleGet,
  getOrderPurchaseProcessAfterSaleSrmBaseList,
  postOrderPurchaseProcessAfterSaleSrmSave,
  postOrderPurchaseProcessAfterSaleSrmUpdate,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, btnCode, title, pageType }) => {
  ...
  const getProcessBaseList = async (params: any = {}) => {
    const { code, data } = await getOrderPurchaseProcessAfterSaleSrmBaseList(params)
    if (code === 1000) {
  ...
  const getDetail = async () => {
    if (processId) {
      const { code, data } = await getOrderPurchaseProcessAfterSaleGet({ processId })
      if (code === 1000) {
