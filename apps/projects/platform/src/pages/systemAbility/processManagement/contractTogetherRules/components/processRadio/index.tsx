...
const ProcessRadio: React.FC<Iprops> & { isFieldComponent: boolean } = (props: Iprops) => {
  ...
  const onChange = (_item: EnumType) => {
    ...
    mutators.change(_item.processId)
  }
    ...
    <div className={styles.panel}>
      <div className={styles.container}>
        {dataSource.map((_item) => {
          return (
            <div className={styles.item} key={_item.processId}>
              <div
                className={className(styles.itemContainer, { [styles.active]: _item.processId === value })}
                key={_item.processId}
                onClick={() => onChange(_item)}
              >
                <Radio disabled={!editable} checked={_item.processId === value} />
