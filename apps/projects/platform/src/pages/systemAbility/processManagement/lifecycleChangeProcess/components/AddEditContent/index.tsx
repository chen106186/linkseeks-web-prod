/**
 * 系统能力 - 流程引擎 - 质量管理流程规则配置 新增/编辑/详情公共组件
 * @author: Crayon
 */
...
import {
  getMemberLifeCycleProcessGet,
  getMemberLifeCycleProcessBaseList,
  postMemberLifeCycleProcessSave,
  postMemberLifeCycleProcessSaveDefault,
  postMemberLifeCycleProcessUpdate,
} from '@apps/apis'
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, btnCode, title, pageType }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      // 基本的流程规则修改或新增
      let requestApi = pageType === PAGE_TYPE.EDIT ? postMemberLifeCycleProcessUpdate : postMemberLifeCycleProcessSave
      // 修改默认流程规则比较特殊，需要调整参数以及特定的接口
      if (isDefault) {
        params = { processId: values.baseProcessId }
        requestApi = postMemberLifeCycleProcessSaveDefault
      }
      setLoading?.(true)
      requestApi(params)
        .then(({ code }) => {
  ...
  const getProcessBaseList = async (params: any = {}) => {
    const { code, data } = await getMemberLifeCycleProcessBaseList(params, { ctlType: 'none' })
    if (code === 1000) {
      setProcessBaseList(data)
      if (pageType === PAGE_TYPE.ADD) {
        form.setFieldsValue({
          baseProcessId: data[0]?.baseProcessId,
        })
      }
  ...
  const getDetail = async () => {
    if (processId) {
      const { code, data } = await getMemberLifeCycleProcessGet({ processId })
      if (code === 1000) {
      ...
      <BaseInfo cols={1}>
        <ProcessBaseRadio
          ...
          onValueChange={(v, item, lastValue) => {
            ...
            // 设置选择生命周期弹窗接口基本参数
            processRulesRef?.current?.setDrawerSelectFetchParams({
              [Select_Content_Type.SelectLifeCycle]: {
                processType: item?.processType,
              },
            })
            processRulesRef?.current?.setProcessType(item?.processType)
            // 由于字段数据变化，流程规则配置需要重置
