/**
 * 系统能力 - 流程引擎 - 物料管理流程规则配置 新增/编辑/详情公共组件
 * @author: Crayon
 */
...
import {
  ...
  postProductMaterialProcessSaveDefault,
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, btnCode, title, pageType }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      // 基本的流程规则修改或新增
      let requestApi = pageType === PAGE_TYPE.EDIT ? postProductMaterialProcessUpdate : postProductMaterialProcessSave
      // 修改默认流程规则比较特殊，需要调整参数以及特定的接口
      if (isDefault) {
        params = { processId: values.baseProcessId }
        requestApi = postProductMaterialProcessSaveDefault
      }
      setLoading?.(true)
      requestApi(params)
        .then(({ code, data }) => {
  ...
  const getProcessBaseList = async (params: any = {}) => {
    const { code, data } = await getProductMaterialProcessBaseList(params)
    if (code === 1000) {
  ...
  const getDetail = async () => {
    if (processId) {
      const { code, data } = await getProductMaterialProcessGet({ processId })
      if (code === 1000) {
