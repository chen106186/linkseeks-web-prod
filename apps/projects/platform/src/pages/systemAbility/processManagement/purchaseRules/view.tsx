...
const PurchaseRules: React.FC<{}> = () => {
  ...
  const fetchData = (params: any) => {
    ...
    return new Promise((resolve, reject) => {
      getOrderPurchaseProcessPage(params).then((res) => {
        const { data } = res
        resolve(data)
      })
    })
  ...
  const confirm = (record: any) => {
    postOrderPurchaseProcessStatusUpdate({ processId: record.processId, status: record.status ? 0 : 1 }).then((res) => {
  ...
  const handelDelete = (record: any) => {
    postOrderPurchaseProcessDelete({ processId: record.processId }).then((res) => {
