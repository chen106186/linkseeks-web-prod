...
import {
  getOrderQualityProcessGet,
  postOrderQualityProcessBaseList,
  postOrderQualityProcessSave,
  postOrderQualityProcessSaveDefault,
  postOrderQualityProcessUpdate,
} from '@apps/apis'
...
const AddEditContent: React.FC<PropsType> = ({ id: processId, btnCode, title, pageType }) => {
  ...
  const handleSubmit = (setLoading: Function, handleLeave: Function) => {
    form.validateFields().then((values) => {
      ...
      // 基本的流程规则修改或新增
      let requestApi = pageType === PAGE_TYPE.EDIT ? postOrderQualityProcessUpdate : postOrderQualityProcessSave
      // 修改默认流程规则比较特殊，需要调整参数以及特定的接口
      if (isDefault) {
        params = { processId: values.baseProcessId }
        requestApi = postOrderQualityProcessSaveDefault
      }
      setLoading?.(true)
      requestApi(params)
        .then(({ code, data }) => {
  ...
  const getProcessBaseList = async (params: any = {}) => {
    const { code, data } = await postOrderQualityProcessBaseList(params, { ctlType: 'none' })
    if (code === 1000) {
  ...
  const getDetail = async () => {
    if (processId) {
      const { code, data } = await getOrderQualityProcessGet({ processId })
      if (code === 1000) {
