...
const columns_6 = ({ dataSource, setDataSource, handleDelete, form }) => {
  ...
  return [
    ...
    {
      title: (
        <Tooltip placement="top" title={intl.formatMessage({ id: 'transaction_components.diyigeyonghubangkanjia' })}>
              ...
              validator: (_rule, value) => {
                ...
                if (!value) {
                  return Promise.reject(
                    new Error(intl.formatMessage({ id: 'transaction_components.qingshuruqishijiage' })),
                  )
                }
    ...
    },
    {
      title: (
        <Tooltip
          placement="top"
          title={intl.formatMessage({ id: 'transaction_components.kanjiaguochengzhongzuihouyi' })}
        >
            ...
            ({ getFieldValue }) => ({
              validator: (_rule, value) => {
                ...
                if (!(Number(value) < Number(plummetPrice))) {
                  return Promise.reject(
                    new Error(intl.formatMessage({ id: 'transaction_components.bixudayu0qiexiaoyu3' })),
                  )
                }
                return Promise.resolve()
              },
