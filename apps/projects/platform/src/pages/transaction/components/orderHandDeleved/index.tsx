...
const OrderHandDeleveModal: React.FC<OrderHandDeleveModalProps> = (props) => {
  ...
  const initData = async () => {
    try {
      const orderId = props.orderId || context?.formContext?.data?.orderId
      if (!orderId) {
        message.error('订单ID不能为空')
        return
      }

      const res = await getOrderVendorValidateDeliveryProduct({ orderId, deliveryNo: null }, { penetrateError: true })

      const { code }: any = res
      const _data = res.data
      if (code === 1000) {
        // 获取库存模式
        const _res = await postProductFreightGetInventoryPattern({ idList: _data.map((item) => item.skuId) })
        message.destroy()
    ...
    >
      <div style={{ margin: '8px 16px' }}>
        <NiceForm
          ...
          expressionScope={{
            productColumns: _productColumns,
            productComponents,
          }}
        />
      </div>
