...
import { postOrderVendorValidateDeliveryConfirm } from '@apps/apis'
...
const PlatformDeliveryModal: React.FC<PlatformDeliveryModalProps> = ({
  ...
  const filteredProducts = useMemo(
    () => selectedProducts.filter((item) => selectedOrderProductIds.includes(Number(item.orderProductId))),
    [selectedProducts, selectedOrderProductIds],
  )
    ...
    if (visible) {
      form.setFieldsValue({
        products: filteredProducts.map((item) => ({
          ...item,
          deliveryCount: Number(item.leftCount) || 0,
        })),
      })
    } else {
      form.resetFields()
    }
  }, [visible, filteredProducts, form])
    ...
    try {
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            setLoading(true)
            const { code } = await postOrderVendorValidateDeliveryConfirm({
              orderId,
