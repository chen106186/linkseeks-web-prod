...
import { mapTextComponent, mapStyledProps, normalizeCol } from '@apps/formily'
import { Select as AntSelect, SelectProps as AntSelectProps } from '@linkseeks/ui'
import styled from 'styled-components'
import { isArr, FormPath } from '@apps/formily'
export * from '@apps/formily'
...
export const Select: React.FC<SelectProps> = styled((props: SelectProps) => {
  ...
  const children = createEnum(dataSource).map((item) => {
    ...
    return (
      <AntSelect.Option key={value} {...others} title={label as string} value={value}>
        {label}
      </AntSelect.Option>
    )
  })
...
export const createMatchUpdate =
  (name: string, path: string) => (targetName: string, targetPath: string, callback: () => void) => {
