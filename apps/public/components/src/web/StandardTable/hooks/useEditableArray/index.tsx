...
import { useDeepCompareEffectDebounce } from '../useDeepCompareEffect'
...
const CancelEditableAction: React.FC<ActionRenderConfig<any> & { row: any }> = (props) => {
  const { recordKey, tableName, newLineConfig, editorType, onCancel, cancelEditable, row, cancelText } = props
  const context = useContext(ProFormContext)
  const form = Form.useFormInstance()
  return (
    <a
