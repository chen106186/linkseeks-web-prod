...
import { postDataPipeline, runFunction } from './utils'
...
const useFetchData = <DataSource extends RequestData<any>>(
   ...
   * @property {number} totalCount 数据总量
  ...
  const fetchList = async (isPolling: boolean) => {
    ...
    try {
      ...
      const { data = [], success, totalCount = 0, ...rest } = (await getData?.(pageParams)) || {}
      // 如果失败了，直接返回，不走剩下的逻辑了
  ...
  return {
     ...
     * @prop {number} [totalCount] - 新的总数据数量。
