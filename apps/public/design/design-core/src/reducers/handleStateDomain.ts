...
export function setStateDomain(state: StateType, payload: ComponentKeyPayload): StateType {
  const { key } = payload
...
export function restStateDomain(state: StateType, payload: ComponentKeyPayload): StateType {
  const { key } = payload
  const { pageConfig, undo, redo } = state
  undo.push({ pageConfig })
  redo.length = 0
