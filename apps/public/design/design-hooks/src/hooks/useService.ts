...
import { ApiType, fetchData, FetcherType } from '@apps/design-utils'
...
const getFetchData = async (
  ...
  if (!nextApi) return
  const result = await fetchData(fetcher, prevApi, nextApi, prevData, nextData, isFirst, options)
...
export function useService(state: any, api?: ApiType) {
  ...
  useEffect(() => {
    if (prevState.current) {
      getFetchData(prevApi.current, api, prevState.current, state, false, options)
    }
