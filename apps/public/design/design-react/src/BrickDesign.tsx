...
import { BrickStore, StaticContextProvider } from '@apps/design-hooks'
...
const controlUpdate = (prevState: BrickdHookState, nextState: BrickdHookState) => {
  ...
  return !isEqual(pageConfig, prevPageConfig)
...
function BrickDesign(brickdProps: BrickDesignProps) {
  const { onLoadEnd, pageName, initState, options, themeName, onlyEidt, ...props } = brickdProps
  const { pageConfig = {} } = useSelector<BrickdHookState, STATE_PROPS>(stateSelector, controlUpdate)
  ...
  const renderComponent = useCallback((pageConfig: PageConfigType) => {
    ...
    return <StateDomainWrapper {...props} onMouseLeave={onMouseLeave} specialProps={specialProps} />
  }, [])

  const designPage = useMemo(() => renderComponent(pageConfig), [pageConfig])
  const divContainer = useRef(null)
  const operateStore = useRef<BrickStore<OperateStateType>>(new BrickStore<OperateStateType>()).current
