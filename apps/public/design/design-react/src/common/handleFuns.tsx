...
import { FunParamContextProvider } from '@apps/design-hooks'
...
function renderNodes(
  ...
  const resultChildNodes = map(childNodes, (key) => {
    const { componentName, isStateDomain, loop, props } = pageConfig[key] || {}
    ...
    return <ContainerDiffWrapper isContainer={isCon} key={key} specialProps={specialProps} {...props} />
  })
...
export function handleChildNodes(
  ...
  if (!nodePropsConfig) {
    nodeProps.children = renderNodes(children as string[], specialProps, pageConfig, allState, undefined, isOnlyNode)
  } else {
    each(children, (nodes: string[], propName: string) => {
      const { isOnlyNode, isRequired, childNodesRule } = nodePropsConfig![propName]
      if (isEmpty(nodes)) {
        return isRequired && (nodeProps[propName] = renderRequiredChildNodes(childNodesRule))
      }
        ...
        nodeProps[realPropName] = (...funParams) => {
          return (
            <FunParamContextProvider value={funParams}>
              {renderNodes(nodes, specialProps, pageConfig, allState, propName, isOnlyNode)}
            </FunParamContextProvider>
          )
        }
      } else {
        nodeProps[propName] = renderNodes(nodes, specialProps, pageConfig, allState, propName, isOnlyNode)
      }
    })
  }
