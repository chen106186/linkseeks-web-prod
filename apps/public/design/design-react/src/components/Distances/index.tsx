...
import { generateCSS, getDragKey, getElementInfo, getIframe, setPosition } from '../../utils'
import { useOperate } from '../../hooks/useOperate'

interface ClientRectType {
  width: any
  height: any
  left: any
  top: any
...
function handleDistances(selectRect: ClientRectType, hoverRect: ClientRectType) {
  const {
    left: selectLeft,
    top: selectTop,
    bottom: selectBottom,
  ...
  useEffect(() => {
    const renderDistances = () => {
      ...
      if (hoverNode && selectedNode && !getDragKey()) {
        const selectRect = getElementInfo(selectedNode, iframe, isModal)
        const { width, height, top, left } = selectRect
        const hoverRect = getElementInfo(hoverNode, iframe, isModal)
        const {
          leftGuide,
          leftDistance,
          rightDistance,
