...
import { DragSourceType, DropTargetType, SelectedInfoType, STATE_PROPS } from '@apps/design-core'
import get from 'lodash/get'
import styles from './index.less'
import { useSelector } from '../../hooks/useSelector'
import { generateCSS, getElementInfo, getIframe, getSelectedNode, setPosition } from '../../utils'
...
function Guidelines() {
  ...
  const { hoverKey, dropTarget, selectedInfo } = useSelector<SelectState, STATE_PROPS>([
    'hoverKey',
    'dropTarget',
    'selectedInfo',
  ])
  ...
  useEffect(() => {
    ...
    const renderGuideLines = () => {
      ...
      if (node) {
        ...
        setPosition(
          [hoverNodeRef.current, leftRef.current, rightRef.current, topRef.current, bottomRef.current],
          isModal,
        )
      }
      if (dropNode) {
  ...
  const hoverNodeClass =
    dropNode || hoverKey ? (dropNode ? styles['drop-node'] : styles['hover-node']) : styles['guide-hidden']
  return (
    <>
