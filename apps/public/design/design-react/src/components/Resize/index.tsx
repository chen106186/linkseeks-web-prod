...
import { PageConfigType, resizeChange, ResizePayload, ROOT, SelectedInfoType, STATE_PROPS } from '@apps/design-core'
...
const controlUpdate = (prevState: ResizeState, nextState: ResizeState) => {
  ...
  return (
    ...
    (selectedInfo && (prevState.pageConfig !== pageConfig || prevState.hoverKey !== hoverKey))
  )
}
...
function Resize(resizeProps: ResizeProps) {
  const { onlyEidt } = resizeProps
  const iframe = getIframe()
  //selectedInfo, hoverKey, pageConfig
  const { selectedInfo, hoverKey, pageConfig } = useSelector<ResizeState, STATE_PROPS>(
    ['selectedInfo', 'pageConfig', 'hoverKey'],
    controlUpdate,
  )
  ...
  const currentConfig = pageConfig[selectedKey] || {}
  let { width, height } = get(props, 'style', { width: 'auto', height: 'auto' }) as { width: string; height: string }
  width = width || 'auto'
  height = height || 'auto'
  ...
  const setSelectedBorder = (css = '') => {
    ...
    if (selectedNode) {
      const { left, top, width, height } = getElementInfo(selectedNode, iframe, isModal)
      if (top <= 14 && isOut) {
        setIsOut(false)
    ...
    <>
      <div className={styles['border-container']} ref={resizeRef}>
        <ActionSheet
          ...
          onlyEidt={onlyEidt}
          hideActions={currentConfig['hideActions'] || false}
        />
        {map(Direction, (direction) => (
          <Item onResizeStart={onResizeStart} direction={direction} key={direction} />
        ))}
        <div className={hoverKey || width === 'auto' ? styles['tip-hidden'] : styles['size-tip-width']} ref={widthRef}>
          {width}
        </div>
        <div
          className={hoverKey || height === 'auto' ? styles['tip-hidden'] : styles['size-tip-height']}
          ref={heightRef}
        >
          {height}
        </div>
