...
import { DragSourceType, SelectedInfoBaseType, SelectedInfoType, STATE_PROPS } from '@apps/design-core'
...
export function useSelect(specialProps: SelectedInfoBaseType, isModal?: boolean): UseSelectType {
  ...
  const controlUpdate = useCallback((prevState: SelectType, nextState: SelectType) => {
    ...
    if (prevSelectedKey && selectedKey) {
      if (prevSelectedKey !== selectedKey) {
        ...
        return isEqualKey(key, prevSelectedKey) || isEqualKey(key, selectedKey)
      } else {
        return (
          propName !== prevPropName || selfDomTreeKeys.includes(prevDragKey) || selfDomTreeKeys.includes(nextDragKey)
        )
      }
    } else {
  ...
  const { selectedInfo } = useSelector<SelectType, STATE_PROPS>(['selectedInfo', 'dragSource'], controlUpdate)
