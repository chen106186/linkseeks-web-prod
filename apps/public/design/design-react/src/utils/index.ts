...
import { Edge, IE11OrLess, resolveMapping, isPureVariable } from '@apps/design-utils'
...
export const iframeSrcDoc = (themeName?: string) => {
  return `<!DOCTYPE html>
    <html lang="en">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=0.5, maximum-scale=2.0, user-scalable=yes" />
    <body ${themeName ? `class='${themeName}'` : ''}>
...
export const getSelectedNode = (key?: string | null, iframe?: HTMLIFrameElement): HTMLElement | undefined => {
  if (iframe && key) {
    ...
    return contentDocument!.getElementsByClassName(selectClassTarget + key)[0] as HTMLElement
  }
}
...
export function getIsModalChild(pageConfig: PageConfigType, domTreeKeys?: string[]) {
  if (domTreeKeys) {
    for (const key of domTreeKeys) {
      const { mirrorModalField } = getComponentConfig(get(pageConfig, [key, 'componentName'])) || {}
...
export const getNodeRealRect = (element: Element) => {
  ...
  const realWidth = parseInt(eleCSS.marginLeft) + parseInt(eleCSS.marginRight) + width
  const realHeight = parseInt(eleCSS.marginTop) + parseInt(eleCSS.marginBottom) + height
...
export const dragSort = (
  ...
  const { realWidth: parentWidth, realHeight: parentHeight } = getParentNodeRealRect(parentNode)
...
export const getPropParentNodes = (childNodes: ChildNodesType, parentNodes: PropParentNodes, index = 0) => {
...
export const getDragComponentName = (dragKey?: string) =>
  get(getSelector(['pageConfig']), ['pageConfig', dragKey || getDragKey(), 'componentName'])
...
export function isVertical(el) {
  const elCSS = css(el),
    ...
    firstChildWidth =
      firstChildCSS &&
      parseInt(firstChildCSS.marginLeft) + parseInt(firstChildCSS.marginRight) + child1.getBoundingClientRect().width,
    secondChildWidth =
      secondChildCSS &&
      parseInt(secondChildCSS.marginLeft) + parseInt(secondChildCSS.marginRight) + child2.getBoundingClientRect().width
...
export const resolveMappingPageConfig = (pageConfig: PageConfigType, state: any) => {
  const newPageConfig: PageConfigType = {}
  const resolveConfig = (childNodes: ChildNodesType, parentKey?: string) => {
    if (Array.isArray(childNodes)) {
      childNodes.forEach((key) => {
        const itemConfig = pageConfig[key]
        if (state) {
          if (key === ROOT) {
            itemConfig['state'] = state
          }
        }

        const { loop, props } = itemConfig
        if ((typeof loop === 'string' && isPureVariable(loop)) || Array.isArray(loop)) {
          const newItemKeys: string[] = []
          delete itemConfig.loop
          const loopList = Array.isArray(loop) ? loop : resolveMapping(loop, state)
          map(loopList, (item, index) => {
            const itemKey = `${key}-${index}`
            newItemKeys.push(itemKey)
            const newItemConfig = produce(itemConfig, (oldItemConfig) => {
              const { title, explain, props } = oldItemConfig
              const source = {
                item,
              }
              if (typeof title === 'string' && isPureVariable(title)) {
                oldItemConfig['title'] = resolveMapping(title, source)
              }
              if (typeof explain === 'string' && isPureVariable(explain)) {
                oldItemConfig['explain'] = resolveMapping(explain, source)
              }

              if (props) {
                const newProps: any = {}
                Object.keys(props).forEach((propsItemKey) => {
                  const propsItem = props[propsItemKey]
                  if (typeof propsItem === 'string' && isPureVariable(propsItem)) {
                    newProps[propsItemKey] = resolveMapping(propsItem, source)
                  } else {
                    newProps[propsItemKey] = propsItem
                  }
                })
                oldItemConfig['props'] = newProps
              }

              return oldItemConfig
            })

            newPageConfig[itemKey] = newItemConfig
          })
          if (parentKey) {
            newPageConfig[parentKey]['childNodes'] = newItemKeys
          }
        } else {
          if (props) {
            const newProps: any = {}
            Object.keys(props).forEach((propsItemKey) => {
              const propsItem = props[propsItemKey]
              if (typeof propsItem === 'string' && isPureVariable(propsItem)) {
                newProps[propsItemKey] = resolveMapping(propsItem, state)
              } else {
                newProps[propsItemKey] = propsItem
              }
            })
            itemConfig['props'] = newProps
          }

          newPageConfig[key] = itemConfig
        }
        if (itemConfig.childNodes) {
          resolveConfig(itemConfig.childNodes, key)
        }
      })
    }
  }
