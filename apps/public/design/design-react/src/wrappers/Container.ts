import { createElement, forwardRef, memo, useCallback, useEffect, useMemo, useRef, useState } from 'react'
import {
  ChildNodesType,
  DropTargetType,
  getComponentConfig,
...
import { useCommon } from '@apps/design-hooks'
import { getChildrenFields, VirtualDOMType } from '@apps/design-utils'
...
function Container(allProps: CommonPropsType, ref: any) {
  ...
  const { pageConfig: PageDom, dropTarget } = useSelector<ContainerState, STATE_PROPS>(
    ['dropTarget', 'pageConfig'],
    controlUpdate,
  )
  ...
  const { mirrorModalField, nodePropsConfig, childNodesRule } = useMemo(() => getComponentConfig(componentName), [])
  const nodePropNames = keys(nodePropsConfig)
  const prevPropName = useRef(nodePropNames.includes(defaultPropName) ? defaultPropName : nodePropNames[0])
  ...
  const { selectedDomKeys, isSelected, propName } = useSelect(specialProps, !!mirrorModalField)
  ...
  useEffect(() => {
    ...
    if (isSelected && (isEmpty(funParams || item) || (isEmpty(funParams || item) && selectedIndex === index))) {
  ...
  const parentDragOver = useCallback(
    (event: DragEvent) => {
      event.preventDefault()
      const dragKey = getDragKey()
      const { isLock } = getOperateState()
  ...
  const onParentDragEnter = useCallback(
    (e: DragEvent) => {
      ...
      if (nodePropsConfig) {
        ...
        isDropAble =
          isAllowDrop(childNodesRule) && (!isNeedJudgeFather() || isAllowAdd(`${componentName}.${selectedPropName}`))
      } else {
  ...
  useEffect(() => {
    ...
    containerRootNode.addEventListener('dragover', parentDragOver)
  ...
  return createElement(getComponent(componentName), {
    ...
    ...handleChildNodes(specialProps, pageConfig, { ...pageState, ...pageState.getPageState() }, children),
    draggable: false,
    /**
     * 设置组件id方便抓取图片
     */
