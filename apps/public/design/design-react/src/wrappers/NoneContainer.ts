import { createElement, forwardRef, memo, useCallback, useEffect, useMemo, useRef } from 'react'
import { clearDropTarget, ROOT, STATE_PROPS } from '@apps/design-core'
...
function NoneContainer(allProps: CommonPropsType, ref: any) {
  ...
  const { pageConfig: PageDom } = useSelector<HookState, STATE_PROPS>(stateSelector, (prevState, nextState) =>
  ...
  const { setSelectedNode, ...events } = useEvents(parentRootNode, specialProps, isSelected, props)
  ...
  return createElement(getComponent(componentName), {
    ...restProps,
    className: handlePropsClassName(uniqueKey, domTreeKeys.includes(dragKey), className, animateClass),
    onDragEnter,
    ...events,
