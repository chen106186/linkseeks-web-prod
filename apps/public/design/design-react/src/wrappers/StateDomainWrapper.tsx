...
import { useService, useRedux, PropsProvider, BrickStoreProvider, useGetProps } from '@apps/design-hooks'
import { STATE_PROPS, isContainer } from '@apps/design-core'
...
function StateDomainWrapper(props: any) {
  ...
  const { pageConfig } = useSelector<HookState, STATE_PROPS>(stateSelector, (prevState, nextState) =>
    controlUpdate(prevState, nextState, specialProps.key),
  )
  const { state, api, componentName } = pageConfig[specialProps.key] || getDragSourceVDom()[specialProps.key]
  ...
  return (
    <PropsProvider value={childProps}>
      <BrickStoreProvider value={brickdStore}>
        <ContainerDiffWrapper isContainer={isContainerNode} specialProps={specialProps} {...rest} />
      </BrickStoreProvider>
    </PropsProvider>
  )
}
