...
const ShopHeader: React.FC<ShopHeaderProps> = (props: ShopHeaderProps) => {
  const {
    type,
    replaceArr,
    title,
  ...
  const classNameString = cx(
    styles['lingxi-marketingCard-shop-header'],
    className,
  )
  ...
  const renderComponent = (locale: MobileLocale) => {
    ...
    return (
      <div className={classNameString} {...other}>
        <div className={styles[`lingxi-marketingCard-shop-header-title`]}>
          <img
            src={_icon()}
            className={styles[`lingxi-marketingCard-shop-header-title-icon`]}
          />
          {_titleText()}
        </div>
      </div>
