...
import { isObjectShallowModified, hasFile, object2formData, qsstringify, cloneObject, createObject } from './helper'
...
export function buildApi(
  ...
  if (~idx) {
    ...
    const params = qs.parse(api.url.substring(idx + 1, ~hashIdx ? hashIdx : undefined))
    api.url =
  ...
  if (api.method === 'get') {
    ...
    } else if (api.attachDataToQuery === false && api.data && !raw.includes('$') && autoAppend) {
      const idx = api.url.indexOf('?')
      if (~idx) {
...
function responseAdaptor(ret: any, api: ApiObject) {
  ...
  if (payload.ok && api.responseData) {
    payload.data = dataMapping(api.responseData, createObject({ api }, payload.data || {}))
  }
  return payload
}
...
export function wrapFetcher(
  ...
  return async function (api, data, options) {
    ...
    if (typeof api.cache === 'number' && api.cache > 0) {
      ...
      return wrapAdaptor(apiCache ? (apiCache as ApiCacheConfig).result : setApiCache(api, result), api)
    }
...
export function isApiOutdated(
  ...
  const nextUrl = get(nextApi, 'url', nextApi) as string
  const prevUrl = get(prevApi, 'url', prevApi) as string
...
export function isValidApi(api: string) {
  return api && /^(?:(https?|wss?|taf):\/\/[^\/]+)?(\/?[^\s\/\?]*){1,}(\?.*)?$/.test(api)
}
...
export const fetchData = async (
  ...
  if (fetcher && nextApi) {
    if (isArray(nextApi)) {
      ...
      if (isApiOutdated(prevApi as Api, nextApi as any, prevData, nextData, isFirst)) {
        const result = await wrapFetcher(fetcher)(nextApi as any, nextData, options)
        if (result) {
