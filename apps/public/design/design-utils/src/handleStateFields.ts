...
export function getStateFields(data: any, fieldsRet?: string[]) {
  ...
  each(data, (value, key) => {
    ...
    } else if (typeof value === 'string' && /(\\)?\$(?:([a-z0-9_\.]+|&|\$)|{([^}{]+?)})/gi.test(value)) {
      value.replace(/(\\)?\$(?:([a-z0-9_\.]+|&|\$)|{([^}{]+?)})/gi, (_, escape) => {
...
export function getChildrenFields(pageConfig: PageConfigType, childNodes: ChildNodesType) {
  ...
  const getFields = (childNodes: string[]) => {
    each(childNodes, (nodeKey) => {
      const { loop } = (pageConfig && pageConfig[nodeKey]) || {}
...
export const resolveFieldFromExpression = (expression: string, fieldsRet?: string[]) => {
...
export const resolveVariableAndFilterField = (path?: string, fieldsRet?: string[], defaultFilter = '| html'): any => {
  if (!path) {
    return
  }
...
function getStrOrField(value: string) {
  return /(^('|")(.*)\1$)|(^-?\d+$)|(^(-?\d+)\.\d+?$)|(^\[.*\]$)|(,)/.test(value) ? false : value
}
