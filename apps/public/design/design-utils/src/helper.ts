...
export function isObjectShallowModified(prev: any, next: any, strictMode = true, ignoreUndefined = false): boolean {
  if (Array.isArray(prev) && Array.isArray(next)) {
    return prev.length !== next.length
      ? true
      : prev.some((prev, index) => isObjectShallowModified(prev, next[index], strictMode, ignoreUndefined))
  } else if (null == prev || null == next || !isObject(prev) || !isObject(next)) {
