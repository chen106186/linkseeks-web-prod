...
import { createObject, setVariable, qsstringify, string2regExp } from './helper'
...
function makeSorter(key: string, method?: 'alpha' | 'numerical', order?: 'desc' | 'asc') {
...
export const filterDate = (value: string, data: object = {}, format = 'X', utc = false): moment.Moment => {
  let m
  const mm = utc ? moment.utc : moment
...
export const filters: {
  ...
  dateModify: (input: any, modifier: 'add' | 'subtract' | 'endOf' | 'startOf' = 'add', amount = 0, unit = 'days') => {
  ...
  sortBy: (input: any, key: string, method: 'alpha' | 'numerical' = 'alpha', order?: 'asc' | 'desc') =>
    Array.isArray(input) ? input.sort(makeSorter(key, method, order)) : input,
  unique: (input: any, key?: string) => (Array.isArray(input) ? (key ? uniqBy(input, key) : uniq(input)) : input),
  topAndOther: (input: any, len = 10, labelField = 'name', restLabel = '其他') => {
      ...
      return Object.keys(grouped).map((key, index) => {
        ...
        const obj = group.reduce((obj, item) => {
          Object.keys(item).forEach((key) => {
            ...
            } else if (typeof item[key] === 'string' && typeof obj[key] === 'string') {
              obj[key] += `, ${item[key]}`
            } else {
              obj[key] = item[key]
            }
  ...
  pick: (input, path = '&') =>
    Array.isArray(input) && !/^\d+$/.test(path)
      ? input.map((item, index) => pickValues(path, createObject({ index }, item)))
...
export function registerFilter(name: string, fn: (input: any, ...args: any[]) => any): void {
  filters[name] = fn
}
...
export function pickValues(names: string, data: object) {
  ...
  if (!names || ((arr = names.split(',')) && arr.length < 2)) {
    ...
    if (~idx) {
      const key = names.substring(0, idx)
      const target = names.substring(idx + 1)
      return {
...
export const isPureVariable = (path?: any) =>
  typeof path === 'string' ? /^\$(?:([a-z0-9_.]+)|{[^}{]+})$/.test(path) : false

export const resolveVariableAndFilter = (path?: string, data: object = {}, defaultFilter = '| html'): any => {
  if (!path) {
    return undefined
  }
  ...
  const originalKey = finalKey
  finalKey = paths.shift() as string

  const ret = resolveVariable(finalKey, data)

  let prevConInputChanged = false // 前一个类三元过滤器生效，则跳过后续类三元过滤器

  return ret == null && !originalKey.includes('default') && !originalKey.includes('now')
    ? ret
    : paths.reduce((input, filter) => {
        ...
        if (['isTrue', 'isFalse', 'isMatch', 'isEquals', 'notMatch', 'notEquals'].includes(key)) {
...
export const tokenize = (str: string, data: object, defaultFilter = '| raw') => {
...
export function resolveMapping(value: any, data: PlainObject, defaultFilter = '| raw') {
...
export function dataMapping(
  ...
  Object.keys(to).forEach((key) => {
    ...
    if (typeof ignoreFunction === 'function' && ignoreFunction(key, value)) {
      ...
      const v =
        ...
        Array.isArray(from[keys[0].substring(1)])
          ? from[keys[0].substring(1)].map((raw: object) =>
              dataMapping(value[keys[0]], createObject(from, raw), ignoreFunction),
            )
      ...
      ret[key] = arr.map((raw: object) => dataMapping(mapping, createObject(from, raw), ignoreFunction))
    } else if (isPlainObject(value)) {
      ret[key] = dataMapping(value, from, ignoreFunction)
    } else if (Array.isArray(value)) {
      ret[key] = value.map((value: any) =>
        isPlainObject(value) ? dataMapping(value, from, ignoreFunction) : resolveMapping(value, from),
      )
...
export function registerBuiltin(): Enginer & { name: string } {
  return {
    name: 'builtin',
