export class BaseService {
  private cache: Record<string, { data?: any; error?: any }> = {}
   ...
   */
  protected async getData<T>(key: string, request: () => Promise<T>): Promise<T> {
    if (this.cache[key]) {
  ...
  protected throwError(msg?: string) {
    ...
    const errorMessage = `[${className}] Error: ${msg || 'An error occurred.'}`
