...
const LayoutHeader: React.FC<IProps> = (props) => {
  ...
  const userInfo = authLocalStorage.getItem()
  const handleClickMenu = () => {
    setCollapsed(!collapsed)
  }

  const showMenu = useMemo(() => location.pathname !== '/home', [location])
  ...
  return (
    <Header className={style['header']}>
      {showMenu && <RenderIcon size={24} className={style['icon']} onClick={handleClickMenu} />}
