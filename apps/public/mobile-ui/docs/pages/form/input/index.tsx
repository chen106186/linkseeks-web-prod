...
import { Form as GodForm, Input as GodInput } from '@apps/mobile-ui'
...
const TestInput: FC<any> = () => {
  const [_value, setValue] = useState('')
  const handleChange = (e) => {
    const { value } = e.detail
    if (Number(value) > 10) {
      console.log('value Change')
      setValue((value === '10' ? 10 : '10') as string)
    } else {
      setValue(value)
    }
  }
  console.log('component rerender')
  return <Input placeholder="测试输入" value={_value} onInput={handleChange} />
}
...
export default class Index extends React.Component<{}, IndexState> {
  ...
  handleChangeInput(value) {
    if (Number(value) > 10) {
      this.setState({
        value: (value === '10' ? 10 : '10') as string,
      })
    } else {
      this.setState({
        value: value,
