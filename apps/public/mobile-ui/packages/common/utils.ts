...
function pxTransform(size: number): string {
  ...
  return IS_WEB ? pxTransformTaro(size) : `${size * deviceRatio[designWidth]}rpx`
}
...
function onAppHide(fn: any) {
  if (ENV === ENV_TYPE.WEAPP) {
    onAppHideTaro(fn)
  }
}

function offAppShow(fn: any) {
  if (ENV === ENV_TYPE.WEAPP) {
    offAppShowTaro(fn)
  }
}

function offAppHide(fn: any) {
  if (ENV === ENV_TYPE.WEAPP) {
    offAppHideTaro(fn)
  } else {
    fn()
