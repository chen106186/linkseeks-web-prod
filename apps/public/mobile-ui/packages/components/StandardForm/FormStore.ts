...
import type { FieldEntity, NotifyInfo, Store, ValuedNotifyInfo, Callbacks, ValidateRule } from './typings'
...
export interface InternalHooks {
  ...
  setRules: typeof FormStore.prototype.setRules
}
...
export class FormStore {
    ...
    return new Promise<Boolean>((resolve, reject) => {
      Object.keys(this.rules).forEach((key) => {
        ...
        this.rules[key].forEach((rule) => {
          ...
          if (rule.min) {
            if (value && typeof value === 'number' && value < rule.min) {
              showToast(rule.message)
              resolve(false)
              return false
            }
          ...
          if (rule.max) {
            if (value && typeof value === 'number' && value > rule.max) {
              showToast(rule.message)
              resolve(false)
              return false
            }
  ...
  public setRules = (rules: Record<string, ValidateRule> | undefined) => {
    this.rules = rules || {}
  }
