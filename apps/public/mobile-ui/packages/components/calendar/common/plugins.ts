...
export function handleActive(args: PluginArg, item: Calendar.Item): Calendar.Item {
  ...
  item.isSelected =
    _value.isSame(dayjsEnd) || _value.isSame(dayjsStart) || (_value.isAfter(dayjsStart) && _value.isBefore(dayjsEnd))
...
export function handleMarks(args: PluginArg, item: Calendar.Item): Calendar.Item {
  const { options } = args
  const { _value } = item
  const { marks } = options
...
export function handleDisabled(args: PluginArg, item: Calendar.Item): Calendar.Item {
  const { options } = args
  const { _value } = item
...
export function handleValid(args: PluginArg, item: Calendar.Item): Calendar.Item {
  const { options } = args
  const { _value } = item
