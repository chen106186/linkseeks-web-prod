...
import { getEnv, ENV_TYPE, chooseImage } from '@tarojs/taro'
import { GodImagePickerProps, File } from '../../types/image-picker'
...
// 生成 jsx 二维矩阵
const generateMatrix = (files: MatrixFile[], col: number, showAddBtn: boolean): MatrixFile[][] => {
...
export default class GodImagePicker extends React.Component<GodImagePickerProps> {
  public static propTypes: InferProps<GodImagePickerProps>
  public static defaultProps: GodImagePickerProps

  private chooseFile = (): void => {
    const { files = [], multiple, count, sizeType, sourceType } = this.props
    const filePathName = ENV === ENV_TYPE.ALIPAY ? 'apFilePaths' : 'tempFiles'
        ...
        const targetFiles = res.tempFilePaths.map((path, i) => ({
          ...
          fileName: path
            ? `${
                ...
                (res[filePathName][i]?.originalFileObj
                  ? `.${res[filePathName][i]?.originalFileObj?.name?.split('.')?.pop()}`
                  : '')
              }`
            : '',
        }))
  ...
  public render(): JSX.Element {
    const { className, customStyle, files, mode, length = 4, showAddBtn = true, max } = this.props
          ...
          <View className="at-image-picker__flex-box" key={i + 1}>
            {row.map((item, j) =>
              item.url ? (
                ...
                <View className="at-image-picker__flex-item" key={'empty_' + i * length + j}>
                  {item.type === 'btn' && (
                    <View className="at-image-picker__item at-image-picker__choose-btn" onClick={this.chooseFile}>
                      <View className={`add-bar ${max !== undefined ? 'add-bar__tip' : ''}`}></View>
                      <View className={`add-bar ${max !== undefined ? 'add-bar__tip' : ''}`}></View>
                      {max && (
                        <View className="add-tip">{translate('mobile.common.zuiduoshangchuan', { max: max })}</View>
                      )}
                    </View>
                  )}
                </View>
