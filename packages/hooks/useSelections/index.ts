...
export default function useSelections<T>(items: T[], defaultSelected: T[] = []) {
  ...
  const { selectedSet, isSelected, select, unSelect, clearSelect, toggle } = useMemo(() => {
    ...
    return { selectedSet, isSelected, select, unSelect, clearSelect, toggle }
  }, [selected])

  const { selectAll, unSelectAll, noneSelected, allSelected, partiallySelected, toggleAll } = useMemo(() => {
    const selectAll = () => {
      items.forEach((o) => {
        selectedSet.add(o)
      })
