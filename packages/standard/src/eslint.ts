...
const fs = require("fs");
const tsEslintConfig = require("./tsEslintConfig.ts");
...
const isJsMoreTs = async (p = "src") => {
  const fg = require("fast-glob");
  const jsFiles = await fg(`${p}/src/**/*.{js,jsx}`, { deep: 3 });
  const tsFiles = await fg(`${p}/src/**/*.{ts,tsx}`, { deep: 3 });
