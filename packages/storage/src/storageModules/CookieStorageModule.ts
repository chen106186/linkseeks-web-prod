...
export class CookieStorageModule implements StorageModuleProps {
  /**
   * 储存key的前缀，通常为应用名
   */
  ...
  constructor(options: StorageModuleProps) {
    ...
    this.storageInstance = storageInstance || new CookieStorage()
    this.cryptoType = cryptoType
    this.transformResult = transformResult
  }

  setItem<T extends any>(value: T, options?: any) {
    let dispatchValue = this.setDispatchValue(this.formatValue(value))
    this.storageInstance.setItem(this.storageKey, dispatchValue, options)
  }

  getItem(options?: any) {
    const result = this.storageInstance.getItem(this.storageKey, options)
    let dispatchValue = this.formatValue(this.getDispatchValue(result), true)
    return this.transformResult ? this.transformResult(dispatchValue) : dispatchValue
  }

  removeItem(options?: any) {
    this.storageInstance.removeItem(this.storageKey, options)
  }
  ...
  private getDispatchValue(value: string) {
    if (!value) {
      return value
    }
    switch (this.cryptoType) {
